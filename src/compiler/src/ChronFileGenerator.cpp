...
BytecodeInstruction::BytecodeInstruction(BytecodeNodeType type)
    : type(type), id(0), hasId(false) {}
...
bool ChronFileGenerator::writeFile(const std::filesystem::path &outputPath) {
  ...
  outFile.write(reinterpret_cast<const char *>(bytecode.data()),
                bytecode.size());
  outFile.close();
...
void ChronFileGenerator::registerNodeId(const AstNode *node, uint32_t id) {
  nodeIds[node] = id;
}
...
void ChronFileGenerator::addInstruction(
...
void ChronFileGenerator::visitProgramNode(ProgramNode *node) {
  ...
  programInstr.addField("numDeclarations",
                        static_cast<int32_t>(node->getDeclarations().size()));
...
void ChronFileGenerator::visitLiteralExprNode(LiteralExprNode *node) {
  ...
  if (!node->getToken().hasValue()) {
    ...
    if (std::holds_alternative<int>(value)) {
      ...
      literalInstr.addField("value",
                            static_cast<int32_t>(std::get<int>(value)));
    } else if (std::holds_alternative<double>(value)) {
...
void ChronFileGenerator::visitCallExprNode(CallExprNode *node) {
  ...
  callInstr.addField("numArgs",
                     static_cast<int32_t>(node->getArguments().size()));
...
void ChronFileGenerator::visitBlockStmtNode(BlockStmtNode *node) {
  ...
  blockInstr.addField("numStmts",
                      static_cast<int32_t>(node->getStatements().size()));
...
void ChronFileGenerator::visitResourceStmtNode(ResourceStmtNode *node) {
  ...
  BytecodeInstruction resourceStmtInstr(BytecodeNodeType::RESOURCE_STMT,
                                        resourceStmtId);

  // Add resource operation type
  resourceStmtInstr.addField("operation",
                             static_cast<int32_t>(node->getOperation()));
...
void ChronFileGenerator::visitParallelExecutionStmtNode(
...
void ChronFileGenerator::visitBranchTimelineStmtNode(
  ...
  BytecodeInstruction branchInstr(
      static_cast<BytecodeNodeType>(0x25),
  ...
  branchInstr.addField("branchCount",
                       static_cast<int32_t>(1)); // Default to 1 branch
