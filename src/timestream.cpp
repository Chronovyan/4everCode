...
std::shared_ptr<TimeAnchor> Timestream::getSourceAnchor() const { return m_source_anchor; }

std::shared_ptr<TimeAnchor> Timestream::createAnchor(double stability,
...
std::vector<std::shared_ptr<TimeAnchor>> Timestream::getAnchors() const { return m_anchors; }
...
std::shared_ptr<TimeAnchor> Timestream::getLatestAnchor() const {
    ...
    return *std::max_element(
        ...
        [](const std::shared_ptr<TimeAnchor>& a, const std::shared_ptr<TimeAnchor>& b) {
            return a->getCreationTime() < b->getCreationTime();
        });
}

std::shared_ptr<TimeAnchor> Timestream::findAnchor(const std::string& id) const {
    auto it = std::find_if(
        m_anchors.begin(), m_anchors.end(),
        [&id](const std::shared_ptr<TimeAnchor>& anchor) { return anchor->getId() == id; });
...
double Timestream::calculateStability() const {
    ...
    for (const auto& anchor : m_anchors) {
        ...
        double time_diff = static_cast<double>(latest_time - anchor->getCreationTime()) + 1.0;
...
double Timestream::calculateDivergence(const std::shared_ptr<Timestream> other) const {
    if (!other) {
    ...
    if (isDerivedFrom(other)) {
        ...
        while (current && !common_ancestor) {
            auto it = std::find(this_ancestry.begin(), this_ancestry.end(), current->getId());
    ...
    // For simplicity, we'll just use the total number of anchors as an approximation
    ...
    double temporal_distance = this_latest->calculateTemporalDistance(*other_latest);
    ...
    double count_diff = std::abs(this_anchor_count - other_anchor_count) /
