...
std::shared_ptr<ResourceSnapshot> ResourceVisualization::takeSnapshot(
    ...
    if (m_runtime && m_debt_tracker) {
        ...
        snapshot->m_resource_stats["cycle"] = std::to_string(m_runtime->getCycleNumber());
    }
...
std::string ResourceVisualization::generateResourceReport(ReportType report_type,
...
std::string ResourceVisualization::generateReportFilename(ReportType report_type,
                                                          VisualizationFormat format) const {
    return "report_" + std::to_string(static_cast<int>(report_type)) + ".txt";
}

std::string ResourceVisualization::visualizeAethelEfficiencyMetrics(VisualizationFormat format) {
    return "AethelEfficiencyMetrics: metrics data";
}

std::string ResourceVisualization::visualizeTemporalResourceFlow(int steps,
                                                                 VisualizationFormat format) {
    return "TemporalResourceFlow: flow data";
}

std::string ResourceVisualization::visualizePredictiveUsage(int projection_cycles,
...
void ResourceVisualization::updateDashboard(const std::string& dashboard_id,
    ...
    // Debug output
    std::cout << "Dashboard " << dashboard_id << " updated with format " << static_cast<int>(format)
              << std::endl;
}
...
std::string ResourceVisualization::generateDashboardHeader(VisualizationFormat format) {
    ...
    switch (format) {
        ...
        case VisualizationFormat::HTML:
            ...
            ss << "  .header { background-color: #f8f9fa; padding: 10px; border-bottom: 1px solid "
                  "#ddd; }\n";
            ss << "</style>\n</head>\n<body>\n";
...
std::string ResourceVisualization::generateDashboardFooter(VisualizationFormat format) {
    ...
    switch (format) {
        case VisualizationFormat::TEXT:
            ss << "=======================================" << std::endl;
            ss << "End of Dashboard - Next update in " << m_refresh_rate << " ms" << std::endl;
            break;
...
std::string ResourceVisualization::generateResourcesSection(VisualizationFormat format) {
    std::stringstream ss;
    ...
    switch (format) {
        ...
        case VisualizationFormat::HTML:
            ...
            for (const auto& resource : m_resources) {
                ss << "  <li><strong>" << resource.first << ":</strong> " << resource.second
                   << "</li>\n";
            }
...
std::string ResourceVisualization::generateTrendsSection(VisualizationFormat format) {
    ...
    switch (format) {
        case VisualizationFormat::TEXT:
            ...
            if (!m_history.empty() && m_history.size() > 1) {
                ...
                for (const auto& resource : m_resources) {
                    ...
                    if (previous.resources.find(resource.first) != previous.resources.end()) {
        ...
        case VisualizationFormat::JSON:
            ...
            if (!m_history.empty() && m_history.size() > 1) {
                ...
                for (const auto& resource : m_resources) {
                    ...
                    double change = resource.second - prev_value;
                    std::string trend = (change > 0) ? "up" : ((change < 0) ? "down" : "stable");
        ...
        case VisualizationFormat::HTML:
            ...
            if (!m_history.empty() && m_history.size() > 1) {
                ss << "<table border=\"1\" style=\"width:100%; border-collapse: collapse;\">\n";
                ...
                for (const auto& resource : m_resources) {
                    ...
                    ss << "  <tr><td>" << resource.first << "</td><td>" << std::fixed
                       << std::setprecision(2) << change << "</td><td style=\"color:" << color
                       << "\">" << trend << "</td></tr>\n";
                }
...
std::string ResourceVisualization::generateDebtSection(VisualizationFormat format) {
    ...
    switch (format) {
        ...
        case VisualizationFormat::JSON:
            ...
            ss << "        \"data\": {\n";
            ss << "          \"overall_debt_level\": " << m_debt_tracker->getOverallDebtLevel()
               << ",\n";
...
std::string ResourceVisualization::generateMetricsSection(VisualizationFormat format) {
    std::stringstream ss;
    ...
    switch (format) {
        case VisualizationFormat::TEXT:
            ...
            ss << "Resource Utilization: " << getDoubleStat("resource_utilization") << "%"
               << std::endl;
            ss << "Operation Throughput: " << getDoubleStat("operation_throughput") << " ops/s"
               << std::endl;
            ss << "Timeline Stability: " << getDoubleStat("timeline_stability") << std::endl;
            ss << std::endl;
            break;
        ...
        case VisualizationFormat::JSON:
            ...
            ss << "        \"data\": {\n";
            ss << "          \"resource_utilization\": " << getDoubleStat("resource_utilization")
               << ",\n";
            ss << "          \"operation_throughput\": " << getDoubleStat("operation_throughput")
               << ",\n";
            ss << "          \"timeline_stability\": " << getDoubleStat("timeline_stability")
               << "\n";
            ss << "        }\n";
...
std::string ResourceVisualization::generateDetailedReport(VisualizationFormat format,
    ...
    if (m_optimizer) {
        ...
        ss << "  "
           << const_cast<ResourceVisualization*>(this)->visualizeOptimizationOpportunities(
    ...
    switch (report_type) {
        ...
        case ReportType::DEBT_IMPACT:
            ...
            if (m_debt_tracker) {
                ...
                content +=
                    "Total Aethel Debt: " + std::to_string(m_debt_tracker->getTotalAethelDebt()) +
                    "\n";
            }
            break;
...
std::string ResourceVisualization::visualizeTrends(int steps, VisualizationFormat format) {
    std::stringstream ss;
...
std::string ResourceVisualization::visualizeOptimizationOpportunities(VisualizationFormat format) {
    ...
    if (m_optimizer) {
        ss << "Optimization recommendations not implemented in compatibility version.\n";
    } else {
...
std::string ResourceVisualization::visualizeDebtStatus(VisualizationFormat format) {
    ...
    if (m_debt_tracker) {
        ...
        ss << "\nDEBT PROJECTIONS\n";
        ss << "- Projected in 3 cycles: " << m_debt_tracker->getOverallDebtLevel() * 1.1 << "\n";
        ss << "- Projected in 5 cycles: " << m_debt_tracker->getOverallDebtLevel() * 1.2 << "\n";
        ss << "- Projected in 10 cycles: " << m_debt_tracker->getOverallDebtLevel() * 1.4 << "\n";

        // Add minimum repayment information
        ss << "\nMINIMUM SAFE REPAYMENT\n";
        ss << "- Chronons: " << m_debt_tracker->getTotalChrononsDebt() * 0.1 << " per cycle\n";
        ss << "- Aethel: " << m_debt_tracker->getTotalAethelDebt() * 0.1 << " per cycle\n";
...
void ResourceVisualization::clearHistory() {
    ...
    // This is a compatibility version with no history to clear, but we should add a log
    // to indicate that the method was called and ensure tests can verify this happened
}
        ...
        case VisualizationFormat::TEXT:
            ...
            if (m_debt_tracker) {
                ...
                if (m_operation_tracker) {
                    ...
                    ss << "  Current throughput: " << m_operation_tracker->getCurrentThroughput()
                       << " ops/s" << std::endl;
...
// Export the debt impact report (alias for exportDebtImpactVisualization for compatibility)
bool ResourceVisualization::exportDebtImpactReport(VisualizationFormat format,
                                                   int projection_cycles,
                                                   const std::string& filename) {
