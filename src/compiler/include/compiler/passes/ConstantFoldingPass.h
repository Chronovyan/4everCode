...
public:
    ...
    StmtNode* visitParallelExecutionStmtNode(ParallelExecutionStmtNode& node) override;
    ...
    void replaceExpressionNode(ExprNode* oldNode, std::unique_ptr<ExprNode> newNode);
    void replaceStatementNode(StmtNode* oldNode, std::unique_ptr<StmtNode> newNode);
...
private:
    ...
    std::unordered_map<ExprNode*, std::unique_ptr<ExprNode>> expressionReplacements;
    std::unordered_map<StmtNode*, std::unique_ptr<StmtNode>> statementReplacements;

    // Helper methods for evaluating literals
    std::variant<int64_t, double, std::string, bool> evaluateLiteral(const LiteralExprNode* node);
};
