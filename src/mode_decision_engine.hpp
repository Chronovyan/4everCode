...
public:
  ModeDecisionEngine()
      : force_mode_for_testing_(false), forced_mode_(PerformanceMode::Balanced),
        forced_reason_("Mode forced for testing"),
        had_previous_source_failure_(false), cpu_was_unavailable_(false),
        memory_was_unavailable_(false), gpu_was_unavailable_(false) {}
  ...
  void
  setForceModeForTesting(PerformanceMode mode, bool enable,
  ...
  void setSourceRecoveryTestingState(bool cpu_unavailable,
    ...
    had_previous_source_failure_ =
        cpu_unavailable || memory_unavailable || gpu_unavailable;
  }
...
private:
  ...
  // Helper methods
  bool
  is_metric_stale(const std::chrono::system_clock::time_point &timestamp) const;
  bool should_apply_hysteresis(const ModeDecision &new_decision) const;
  ModeDecision make_conservative_decision(const std::string &reason,
