...
WhisperingStoneSensorImpl::WhisperingStoneSensorImpl()
    : m_sensorState(SENSOR_STATE_UNINITIALIZED)
...
PressureReading WhisperingStoneSensorImpl::getPressureReading() {
    ...
    // Apply calibration
    float calibratedReading = rawReading * m_calibrationData.scale + m_calibrationData.offset;

    // Store the reading in the buffer
    if (m_pressureReadingsBuffer.size < m_pressureReadingsBuffer.capacity) {
        m_pressureReadingsBuffer.data[m_pressureReadingsBuffer.size++] = calibratedReading;
    }
...
bool WhisperingStoneSensorImpl::validateSensorState() {
    ...
    if (testReading < MINIMUM_VALID_READING || testReading > MAXIMUM_VALID_READING) {
        return false;
    }
...
void WhisperingStoneSensorImpl::setLastSurgeMagnitude(float magnitude) {
    ...
    Logger::info("WhisperingStoneSensor: Surge magnitude set to " + std::to_string(magnitude));
}
...
bool WhisperingStoneSensorImpl::performSelfTest() {
    ...
    if (testReading < MINIMUM_VALID_READING || testReading > MAXIMUM_VALID_READING) {
        Logger::error("WhisperingStoneSensor: Self-test failed - reading out of range");
        return false;
    }
    ...
    if (!m_primaryBuffer.isValid() || !m_secondaryBuffer.isValid() ||
        !m_pressureReadingsBuffer.isValid()) {
        Logger::error("WhisperingStoneSensor: Self-test failed - buffer validation failed");
        return false;
    }
...
bool WhisperingStoneSensorImpl::resetAfterSurge() {
    ...
    Logger::info("WhisperingStoneSensor: Performing comprehensive reset after surge");
    ...
    if (!reinitializeSensorHardware()) {
        ...
        Logger::error("WhisperingStoneSensor: Failed to reinitialize sensor hardware");
        return false;
    }
    ...
    Logger::info("WhisperingStoneSensor: Reset successful after surge of magnitude " +
