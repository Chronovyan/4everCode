 ...
 * @brief Implementation of temporal manipulation functions for the Chronovyan
 * Standard Library
 *
...
bool initialize_temporal() {
  ...
  try {
    ...
    registered_anchors.insert(std::make_pair(
        "@Origin",
        TimeAnchor("@Origin", 0.0, "Initial temporal reference point")));
    registered_anchors.insert(std::make_pair(
        "@Fork", TimeAnchor("@Fork", 1.0, "First branching point")));
...
bool rewind_to(const TIMESTAMP &anchor) {
  std::lock_guard<std::mutex> lock(temporal_mutex);
  if (!initialized) {
    std::cerr << "Temporal manipulation subsystem not initialized."
              << std::endl;
    return false;
  }
...
// Define TemporalState struct at namespace scope to avoid incomplete type
// errors
...
bool TemporalManipulator::merge_branch(const std::string &branch_name) {
  ...
  std::cout << "Merging branch " << branch_name << " into "
            << state->current_branch << std::endl;

  // Remove the merged branch from the list
  auto it =
      std::find(state->branches.begin(), state->branches.end(), branch_name);
