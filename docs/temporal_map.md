---
title: TEMPORAL_MAP Type in 4ever
description: Documentation for temporal_map.md
weight: 100
draft: true
---

# TEMPORAL_MAP Type in 4ever

## Overview

The TEMPORAL_MAP type in 4ever is an advanced key-value container with temporal semantics, designed for efficient storage and retrieval of key-value pairs across time. It provides extensive functionality for tracking changes, detecting patterns, and predicting future states, making it ideal for temporal analytics, state management, and simulation in temporal environments.
...
- **Flexible types**: Both keys and values can be of any type supported by 4ever
...
// Set key-value pairs
map.set(Value("name"), Value("4ever"));
map.set(Value("version"), Value(1.0));

// Access values
Value name = map.at(Value("name"));  // Returns "4ever"
...
// Print the map
std::cout << map.toString() << std::endl;  // Outputs: {name: 4ever}
```
...
The TEMPORAL_MAP type is registered with 4ever's custom type system, allowing it to be used seamlessly in 4ever scripts and programs:
...
- [4everVector](./vector_type.md): Documentation for the VECTOR type
