...
int main(int argc, char* argv[]) {
    ...
    if (!GraphicalResourceVisualizer::isAvailable()) {
        std::cout << "ERROR: Graphical visualization is not available." << std::endl;
        std::cout
            << "This feature requires the Matplot++ library, which was not found during build."
            << std::endl;
        std::cout
            << "Please install Matplot++ and rebuild the project to enable graphical visualization."
            << std::endl;
        return 1;
    }
    ...
    auto graphical_viz = std::make_shared<GraphicalResourceVisualizer>(visualization);
    ...
    debt_tracker->accrueRebelDebt(RebelOperationType::REWIND_FLOW, "test_rebel_op");
    ...
    std::string chronon_chart = graphical_viz->generateResourceChart(
        ...
        ChartOptions("Chronon Usage Over Time", "Time", "Chronons"), GraphOutputFormat::PNG,
        "resource_viz_output/chronon_usage.png");
    ...
    std::string aethel_chart = graphical_viz->generateResourceChart(
        ...
        ChartOptions("Aethel Flux Over Time", "Time", "Aethel"), GraphOutputFormat::PNG,
        "resource_viz_output/aethel_usage.png");
    ...
    std::string stability_chart = graphical_viz->generateResourceChart(
        ...
        ChartOptions("Temporal Stability", "Time", "Stability"), GraphOutputFormat::PNG,
        "resource_viz_output/stability.png");
    ...
    std::string comparison_chart = graphical_viz->generateResourceComparisonChart(
        ChartType::LINE, 20, ChartOptions("Resource Comparison", "Time", "Resource Level"),
        GraphOutputFormat::PNG, "resource_viz_output/resource_comparison.png");
    std::cout << "Resource comparison chart saved to: " << comparison_chart << std::endl;
    ...
    RebelOperationType operations[] = {
        RebelOperationType::REWIND_FLOW, RebelOperationType::QUANTUM_MANIPULATION,
        RebelOperationType::SUSPEND_TIMELINE, RebelOperationType::TEMPORAL_ECHO_LOOP,
    ...
    for (auto op : operations) {
        ...
        std::string rebel_chart = graphical_viz->generateRebelOperationImpactChart(
            op, 15, 2, ChartType::LINE, ChartOptions("", "", "Impact"), GraphOutputFormat::PNG,
            "resource_viz_output/rebel_" + op_name + ".png");
        std::cout << "Rebel operation impact chart for " << op_name << " saved to: " << rebel_chart
                  << std::endl;
    }
    ...
    std::cout << "\nAll visualizations have been generated in the 'resource_viz_output' directory."
              << std::endl;
    std::cout << "You can open these files in any image viewer or web browser (for HTML files)."
              << std::endl;
