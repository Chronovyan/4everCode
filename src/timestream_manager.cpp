...
std::vector<std::shared_ptr<Timestream>> TimestreamManager::getAllTimestreams() const {
...
std::shared_ptr<Timestream> TimestreamManager::findTimestream(const std::string& id) const {
...
std::shared_ptr<Timestream> TimestreamManager::createEcho(const std::string& name,
    ...
    if (!source_timestream) {
        throw std::runtime_error("Source timestream not found: " + source_timestream_id);
    }
    ...
    auto new_timestream = std::make_shared<Timestream>(new_id, name, source_timestream, anchor);
...
std::shared_ptr<TimeAnchor> TimestreamManager::createAnchor(double stability,
...
bool TimestreamManager::harmonizeTimestreams(const std::string& source_id,
    ...
    // For example, we might want to prevent harmonizing if the divergence is too high
    ...
    double new_stability =
        std::min(source_timestream->calculateStability(), target_timestream->calculateStability()) *
        (1.0 - divergence);
    ...
    m_debt_tracker.applyRepaymentStrategy(strategy, available_chronons, available_aethel);

    return true;
}

double TimestreamManager::calculateHarmonizationImpact(const std::string& source_id,
...
double TimestreamManager::calculateEchoParadoxRisk(const std::string& timestream_id,
    ...
    double time_diff =
        static_cast<double>(latest_anchor->getCreationTime() - anchor->getCreationTime());
...
double TimestreamManager::recordEchoDebt(const std::shared_ptr<Timestream> source_timestream,
    ...
    double time_diff =
        static_cast<double>(latest_anchor->getCreationTime() - anchor->getCreationTime());
    double age_factor = std::min(2.0, time_diff / 5000.0 + 1.0);  // 1.0 - 2.0 range
    ...
    int echo_count = 0;
    for (const auto& ts : getAllTimestreams()) {
        if (ts->getParent() && ts->getParent()->getId() == source_timestream->getId()) {
            echo_count++;
        }
    }

    // Calculate echo count factor
    double echo_factor = 1.0 + (echo_count * 0.1);  // Increases by 0.1 for each echo
    ...
    std::string operation_id = "Echo_" + source_timestream->getName() + "_" + anchor->getId();
    std::string description =
        "Created Echo from " + source_timestream->getName() + " at anchor " + anchor->getId();
...
double TimestreamManager::recordHarmonizationDebt(
    ...
    std::string operation_id =
        "Harmonize_" + source_timestream->getName() + "_" + target_timestream->getName();
    std::string description =
        "Harmonized " + source_timestream->getName() + " into " + target_timestream->getName();
    ...
    [[maybe_unused]] double interest_rate = 0.08 + (divergence * 0.12);  // 0.08 - 0.20 range
