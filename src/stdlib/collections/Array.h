...
public:
  ...
  void add(ValuePtr value) override {
    elements_.push_back(value ? value : NIL);
  }
  ...
  ArrayPtr slice(int64_t start, int64_t end = -1) const {
    ...
    auto result = create();
    result->elements_ =
        ElementList(elements_.begin() + start, elements_.begin() + end);
    return result;
  }
  ...
  ValuePtr reduce(const FunctionPtr &reducer,
                  ValuePtr initial = nullptr) const {
    if (!reducer)
      return NIL;

    ValuePtr accumulator =
        initial ? initial : (elements_.empty() ? NIL : elements_[0]);
    size_t start = initial ? 0 : 1;

    for (size_t i = start; i < elements_.size(); ++i) {
      accumulator = reducer->call(
          {accumulator, elements_[i], Number::create(static_cast<double>(i))});
    }
  ...
  std::string toString() const override {
    return chronovyan::stdlib::Array::toString();
  }
