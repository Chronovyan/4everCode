...
std::string readAndConvertFile(const std::string &filename) {
  ...
  if (buffer.size() >= 2 && static_cast<unsigned char>(buffer[0]) == 0xFF &&
    ...
    for (size_t i = 2; i < buffer.size() - 1; i += 2) {
      ...
      // This is a simplistic conversion - a proper implementation would handle
      // all Unicode
...
// Simple implementation of a semantic analyzer that handles basic Chronovyan
// constructs
...
// Simple implementation of a code generator that produces C code for basic
// Chronovyan programs
  ...
  void visitCallExprNode(CallExprNode *node) override {
    ...
    if (varExpr && varExpr->getName().getLexeme() == "print") {
      ...
      if (node->getArguments().size() == 1) {
        ...
        setNodeCode(node,
                    "chronovyan_print(\"Error: print requires 1 argument\")");
      }
    } else {
...
int main(int argc, char *argv[]) {
  ...
  for (const auto &pair : tokenCounts) {
    std::cout << tokenTypeToString(pair.first) << ": " << pair.second
              << std::endl;
  }
