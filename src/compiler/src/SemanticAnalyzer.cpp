...
void SemanticAnalyzer::visitVariableExprNode(VariableExprNode* node) {
    ...
    if (!symbol->initialized) {
        reportWarning("Variable '" + name + "' may be used before initialization", node->getName());
    }
...
void SemanticAnalyzer::visitUnaryExprNode(UnaryExprNode* node) {
    ...
    // Check the operation
    Type resultType = typeChecker.checkUnaryOperation(node->getOperator().getType(), rightType);
...
void SemanticAnalyzer::visitBinaryExprNode(BinaryExprNode* node) {
    ...
    // Check the operation
    Type resultType =
        typeChecker.checkBinaryOperation(node->getOperator().getType(), leftType, rightType);
    setExprType(node, resultType);
}
...
void SemanticAnalyzer::visitAssignExprNode(AssignExprNode* node) {
    ...
    if (symbol->type) {
        ...
        if (!typeChecker.checkAssignment(varType, valueType)) {
            reportError("Cannot assign value of type '" + valueType.toString() + "' to variable '" +
                            name + "' of type '" + varType.toString() + "'",
                        node->getName());
        }
...
void SemanticAnalyzer::visitVarDeclNode(VarDeclNode* node) {
    ...
    if (node->getInitializer()) {
        ...
        if (node->getType()) {
            ...
            if (!typeChecker.checkAssignment(declaredType, initType)) {
                reportError("Cannot initialize variable of type '" + declaredType.toString() +
    ...
    symbolTable.addSymbol(name, Symbol::Kind::VARIABLE, node->getType(), initialized);
}
...
void SemanticAnalyzer::visitParallelExecutionStmtNode(ParallelExecutionStmtNode* node) {
...
void SemanticAnalyzer::visitBranchTimelineStmtNode(BranchTimelineStmtNode* node) {
...
void SemanticAnalyzer::reportError(const std::string& message, const Token& token) {
    diagnostics.addError(token, message);
}

void SemanticAnalyzer::reportWarning(const std::string& message, const Token& token) {
    diagnostics.addWarning(token, message);
}

void SemanticAnalyzer::reportError(const std::string& message, const SourceLocation& location) {
    diagnostics.addError(location, message);
}

void SemanticAnalyzer::reportWarning(const std::string& message, const SourceLocation& location) {
