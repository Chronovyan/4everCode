...
public:
  ...
  explicit BytecodeConstant(double value)
      : type(Type::FLOAT), floatValue(value) {}
  explicit BytecodeConstant(const std::string &value)
      : type(Type::STRING), stringValue(value) {}
  explicit BytecodeConstant(bool value)
  ...
  std::vector<uint8_t> serialize() const {
    ...
    case Type::FLOAT: {
      // Convert double to bytes (this is a simplification - real implementation
      // would handle endianness)
      uint64_t bits;
...
public:
  ...
  const std::unordered_map<std::string, std::shared_ptr<BytecodeFunction>> &
  getFunctions() const {
    return functions;
  }

  void addConstant(const BytecodeConstant &constant) {
    constantPool.push_back(constant);
  }

  size_t getConstantPoolSize() const { return constantPool.size(); }

  const std::vector<BytecodeConstant> &getConstantPool() const {
    return constantPool;
  }
