...
using AdaptiveOptimizationConfig =
    chronovyan::sync::TemporalSynchronizer::AdaptiveOptimizationConfig;
...
protected:
    void SetUp() override { synchronizer = std::make_unique<TemporalSynchronizer>(); }

    void TearDown() override { synchronizer.reset(); }
...
TEST_F(TemporalSynchronizerTest, MetricCorrelationTest) {
    ...
    EXPECT_TRUE(is_within_range(overall_sync, std::min(stability[0], coherence[0]),
...
TEST_F(TemporalSynchronizerTest, CallbackTest) {
    ...
    synchronizer->set_error_callback([&](const std::exception&) { error_called = true; });
...
TEST_F(TemporalSynchronizerTest, CallbackOrderTest) {
    ...
    synchronizer->set_sync_callback([&](double) { callback_order.push_back("sync"); });

    synchronizer->set_error_callback(
        [&](const std::exception&) { callback_order.push_back("error"); });
    ...
    synchronizer->set_recovery_callback([&](bool) {
        if (callback_order.size() == 2 && callback_order[0] == "sync" &&
            callback_order[1] == "error") {
            callback_order.push_back("recovery");
        }
    });
...
TEST_F(TemporalSynchronizerTest, RecoveryStrategyTest) {
    ...
    std::vector<RecoveryStrategy> strategies = {RecoveryStrategy::Automatic,
                                                RecoveryStrategy::Manual, RecoveryStrategy::Custom};
...
TEST_F(TemporalSynchronizerTest, PatternRecognitionTest) {
    ...
    // Verify pattern consistency
    for (size_t i = 1; i < pattern_history.size(); ++i) {
        EXPECT_NEAR(pattern_history[i].confidence, pattern_history[i - 1].confidence, 0.2);
    }
}
...
TEST_F(TemporalSynchronizerTest, AdvancedPatternAnalysisTest) {
    ...
    for (size_t i = 1; i < metrics_history.size(); ++i) {
        EXPECT_NEAR(metrics_history[i].stability_score, metrics_history[i - 1].stability_score,
                    0.2);
        EXPECT_NEAR(metrics_history[i].coherence_score, metrics_history[i - 1].coherence_score,
                    0.2);
    }
}
...
TEST_F(TemporalSynchronizerTest, AdvancedPatternMatchingTest) {
    ...
    std::vector<std::vector<double>> variations = {
        {0.7, 0.8, 0.9}, {0.9, 1.0, 1.0}, {0.8, 0.8, 0.9}};
