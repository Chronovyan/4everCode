...
SourceFile::SourceFile(const std::string &filename)
    : m_name(filename), m_path(filename) {
  std::cout << "SourceFile constructor called with filename: " << filename
            << std::endl;
  ...
  if (!file) {
    std::cerr << "Failed to open file in SourceFile constructor: " << filename
              << std::endl;
    // Instead of using the filename as content, throw an exception
    throw std::runtime_error("Could not open file: " + filename);
  }

  std::cout << "Successfully opened file in SourceFile constructor"
            << std::endl;
  ...
  std::cout << "Final source content size: " << m_source.size() << " bytes"
            << std::endl;
  if (!m_source.empty()) {
    std::cout << "First 20 chars: [" << m_source.substr(0, 20) << "]"
              << std::endl;
  }
...
std::pair<size_t, size_t> SourceFile::getLineAndColumn(size_t position) const {
  if (position >= m_source.length()) {
    // If position is at the end of the file, return the last line and its
    // length
  ...
  auto it =
      std::upper_bound(m_lineOffsets.begin(), m_lineOffsets.end(), position);
