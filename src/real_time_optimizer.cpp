...
RealTimeOptimizer::RealTimeOptimizer(double initial_threshold, double min_threshold,
                                     double max_threshold, size_t window_size)
...
double RealTimeOptimizer::get_current_threshold() const { return current_threshold; }
...
void RealTimeOptimizer::update_thresholds() {
    ...
    double avg_efficiency =
        calculate_moving_average([](const OptimizationMetrics& m) { return m.sync_efficiency; });
    double avg_stability =
        calculate_moving_average([](const OptimizationMetrics& m) { return m.stability; });
    ...
    double adjustment_factor =
        0.4 * efficiency_factor + 0.3 * stability_factor + 0.3 * coherence_factor;

    // Apply adaptive threshold adjustment
    double threshold_change = adjustment_factor * 0.1;  // Scale adjustment
    current_threshold =
        std::clamp(current_threshold + threshold_change, min_threshold, max_threshold);
}
...
void RealTimeOptimizer::calculate_performance_metrics() {
    ...
    performance_metrics.stability =
        calculate_weighted_moving_average([](const OptimizationMetrics& m) { return m.stability; });
...
double RealTimeOptimizer::calculate_moving_average(
    std::function<double(const OptimizationMetrics&)> metric_selector) const {
    if (metrics_history.empty()) {
        return 0.0;
    }
