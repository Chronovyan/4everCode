 ...
 * @brief Implementation of exploration and loot functions for the Chronovyan Standard Library
 *
...
extern bool consume_aethel(AETHEL_RESERVE amount);  // From resource_management.cpp
extern void increase_paradox_level(double amount);  // From paradox_management.cpp
...
std::unordered_map<std::string, std::unordered_map<std::string, std::string>> discovered_regions;
...
const std::vector<std::string> temporal_events = {
    "Temporal_Echo", "Chronon_Storm",   "Reality_Shift",        "Time_Dilation",    "Paradox_Wave",
    "Memory_Ripple", "Causality_Break", "Timeline_Convergence", "Temporal_Anomaly", "Aethel_Surge"};
}  // namespace
...
bool initialize_exploration() {
    ...
    try {
        ...
        std::cerr << "Failed to initialize exploration subsystem: " << e.what() << std::endl;
        return false;
    }
}
...
bool shutdown_exploration() {
    ...
    try {
        ...
        std::cerr << "Failed to shut down exploration subsystem: " << e.what() << std::endl;
        return false;
    }
}
...
TEMPORAL_SIGNATURE deepen_exploration(const std::string& target) {
    ...
    try {
        ...
        std::cout << "Generated temporal signature: " << signature << " from " << target
                  << std::endl;
        ...
        if (discovery_chance(rng) > stability) {
            ...
            std::string new_region = target + "_Branch_" + std::to_string(int(random_factor * 100));

            // Set properties for the new region
            discovered_regions[new_region] = {
                {"stability", std::to_string(stability * 0.8)},          // Less stable than parent
                {"resources", discovered_regions[target]["resources"]},  // Same resources initially
                {"danger",
                 std::to_string(std::min(1.0, std::stod(discovered_regions[target]["danger"]) *
                                                  1.2))},  // More dangerous
...
CHRONON_STREAM loot_drop(const LOOT_TABLE& table) {
    ...
    try {
        ...
        for (const auto& entry : table) {
            if (roll <= entry.first) {
                std::cout << "Loot roll: " << roll << " yields: " << entry.second << std::endl;
                return entry.second;
            }
        }

        // If no match was found (should not happen with a properly constructed table)
...
Explorer::~Explorer() {
    ...
    delete static_cast<std::unordered_map<std::string, std::string>*>(internal_state);
}

std::unordered_map<std::string, std::string> Explorer::analyze_signature(
    ...
    try {
        ...
        if (first_underscore == std::string::npos) {
            std::cerr << "Invalid temporal signature format: " << signature << std::endl;
            return properties;
        }
        ...
        if (discovered_regions.find(base_region) == discovered_regions.end()) {
            std::cerr << "Unknown base region in signature: " << base_region << std::endl;
            return properties;
        }
        ...
        properties["signature_timestamp"] = signature.substr(
            first_underscore + 1, signature.find('_', first_underscore + 1) - first_underscore - 1);
        properties["signature_variant"] = signature.substr(signature.find_last_of('_') + 1);
...
bool Explorer::create_team(const std::string& name, int size) {
    ...
    try {
        ...
        std::cout << "Consuming " << cost << " Aethel to create team..." << std::endl;
        ...
        std::cout << "Created exploration team: " << name << " with " << size << " members."
                  << std::endl;
        return true;
    } catch (const std::exception& e) {
...
TEMPORAL_SIGNATURE Explorer::send_team(const std::string& team_name, const std::string& target,
                                       int duration) {
    std::lock_guard<std::mutex> lock(exploration_mutex);
    if (!initialized) {
        std::cerr << "Exploration subsystem not initialized." << std::endl;
    ...
    try {
        ...
        if (team.active) {
            std::cerr << "Team is already on a mission: " << team.current_mission << std::endl;
            return "";
        }

        // Validate duration
        if (duration < 1 || duration > 100) {
            std::cerr << "Invalid mission duration. Must be between 1 and 100 ticks." << std::endl;
            return "";
        }

        // Apply Aethel cost based on team size and duration
        int cost = team.size * duration * 5;  // 5 Aethel per team member per tick
        std::cout << "Consuming " << cost << " Aethel to send team on mission..." << std::endl;
        if (!consume_aethel(cost)) {
        ...
        std::cout << "Team " << team_name << " dispatched to " << target << " for " << duration
                  << " ticks. Signature: " << signature << std::endl;
        ...
        if (discovered_regions[target]["mapped"] == "false") {
            ...
            if (discovery_chance(rng) < mapping_chance) {
                ...
                std::cout << "Region " << target << " has been successfully mapped!" << std::endl;
            }
        }
...
// Add a simple internal test function that can be called to verify the module works
void _exploration_internal_test() {
    std::cout << "Running internal exploration module test..." << std::endl;

    // Initialize the exploration subsystem
    if (!initialize_exploration()) {
        std::cerr << "Failed to initialize exploration subsystem for test." << std::endl;
        return;
    }
    ...
    if (signature.empty()) {
        ...
        std::cout << "Successfully generated temporal signature: " << signature << std::endl;
    }
    ...
    if (!shutdown_exploration()) {
        std::cerr << "Failed to shut down exploration subsystem after test." << std::endl;
    }
