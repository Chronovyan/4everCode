...
void TemporalVisitor::executeForChronon(const TemporalOpStmtNode &stmt) {
  ...
  if (stmt.getArguments().size() < 2) {
    throw RuntimeError(
        "FOR_CHRONON requires at least two arguments: iterator name and range.",
        stmt.getLocation());
  }
  ...
  if (!stmt.getArguments()[0]->isLiteral()) {
    throw RuntimeError(
        "FOR_CHRONON first argument must be a literal string (iterator name).",
        stmt.getLocation());
  }

  const auto &literalExpr =
      static_cast<const LiteralExprNode &>(*stmt.getArguments()[0]);
  if (!literalExpr.getValue().isString()) {
    throw RuntimeError(
        "FOR_CHRONON first argument must be a string literal (iterator name).",
        stmt.getLocation());
  }
  ...
  if (rangeValue.isInteger()) {
    ...
    throw RuntimeError(
        "FOR_CHRONON range argument must be a number or an array.",
        stmt.getLocation());
  }
  ...
  if (stmt.getArguments().size() >= 3) {
    ...
    if (stepValue.isInteger()) {
      ...
      throw RuntimeError("FOR_CHRONON step argument must be a number.",
                         stmt.getLocation());
    }

    if (step == 0) {
      throw RuntimeError("FOR_CHRONON step cannot be zero.",
                         stmt.getLocation());
    }
  }
...
void TemporalVisitor::executeWhileEvent(const TemporalOpStmtNode &stmt) {
  ...
  if (stmt.getArguments().empty()) {
    throw RuntimeError("WHILE_EVENT requires a condition argument.",
                       stmt.getLocation());
  }
...
void TemporalVisitor::executeRewindFlow(const TemporalOpStmtNode &stmt) {
  ...
  if (!stmt.getArguments().empty()) {
    ...
    if (targetValue.isInteger()) {
      ...
      throw RuntimeError("REWIND_FLOW target cycle must be a number.",
                         stmt.getLocation());
    }
  }
  ...
  if (stmt.getArguments().size() >= 2) {
    ...
    if (attemptsValue.isInteger()) {
      ...
      throw RuntimeError("REWIND_FLOW max attempts must be a number.",
                         stmt.getLocation());
    }
  }
...
void TemporalVisitor::executeMergeTimelines(const TemporalOpStmtNode &stmt) {
  ...
  for (const auto &arg : stmt.getArguments()) {
    ...
    if (idValue.isString()) {
      ...
      throw RuntimeError("MERGE_TIMELINES timeline IDs must be strings.",
                         stmt.getLocation());
    }
  }
...
void TemporalVisitor::executeTemporalEchoLoop(const TemporalOpStmtNode &stmt) {
  ...
  if (stmt.getArguments().empty()) {
    throw RuntimeError(
        "TEMPORAL_ECHO_LOOP requires an iteration count argument.",
        stmt.getLocation());
  }
...
void TemporalVisitor::visitForChrononStmt(const ForChrononStmtNode &stmt) {
  ...
  if (startValue.isInteger()) {
    ...
    throw RuntimeError("FOR_CHRONON range start must be a number.",
                       stmt.getLocation());
  }
  ...
  if (endValue.isInteger()) {
    ...
    throw RuntimeError("FOR_CHRONON range end must be a number.",
                       stmt.getLocation());
  }
  ...
  if (stmt.hasStep()) {
    ...
    if (stepValue.isInteger()) {
      ...
      throw RuntimeError("FOR_CHRONON step must be a number.",
                         stmt.getLocation());
    }
...
void TemporalVisitor::visitRewindFlowStmt(const RewindFlowStmtNode &stmt) {
  ...
  if (stmt.hasMaxRewinds()) {
    ...
    if (rewindsValue.isInteger()) {
      ...
      throw RuntimeError("REWIND_FLOW max rewinds must be a number.",
                         stmt.getLocation());
    }
  }
...
void TemporalVisitor::visitTemporalLoopStmt(const TemporalLoopStmtNode &stmt) {
  ...
  if (stmt.hasDuration()) {
    ...
    if (durationValue.isInteger()) {
      ...
      throw RuntimeError("TEMPORAL_LOOP duration must be a number.",
                         stmt.getLocation());
    }
  }
  ...
  if (stmt.hasVariance()) {
    ...
    if (varianceValue.isNumber()) {
      ...
      throw RuntimeError("TEMPORAL_LOOP variance must be a number.",
                         stmt.getLocation());
    }
  }
  ...
  try {
    ...
    for (int i = 0; i < iterations; i++) {
      ...
      if (variance > 0.0) {
        ...
        double random =
            (std::rand() / static_cast<double>(RAND_MAX) * 2.0 - 1.0) *
            variance;
...
void TemporalVisitor::visitExpendResourcesStmt(
    const ExpendResourcesStmtNode &stmt) {
  ...
  if (aethelCost > 0.0 &&
      m_interpreter.getRuntime()->getAethelLevel() < aethelCost) {
    sufficientResources = false;
  }

  if (chrononCost > 0.0 &&
      m_interpreter.getRuntime()->getChrononsLevel() < chrononCost) {
    sufficientResources = false;
  }
  ...
  if (sufficientResources) {
    ...
    m_interpreter.trackResourceUsage(aethelCost, chrononCost,
                                     "Explicit resource expenditure");
...
void TemporalVisitor::visitResourceInsufficiencyHandler(
  ...
  auto handlerEnv =
      std::make_shared<Environment>(m_interpreter.getEnvironment());
  ...
  try {
    ...
    errorObj->setProperty("message",
                          Value("Insufficient resources for operation"));
...
void TemporalVisitor::visitTemporalProgram(const TemporalProgramNode &program) {
  ...
  for (const auto &sectionType :
       {TemporalSectionType::NAME, TemporalSectionType::TYPE,
        TemporalSectionType::AUTHOR, TemporalSectionType::VERSION,
        TemporalSectionType::DESCRIPTION,
        TemporalSectionType::RESOURCES_REQUIRED,
        TemporalSectionType::TIMELINE_BEHAVIOR,
        TemporalSectionType::STABILITY_TARGET,
        TemporalSectionType::DEFAULT_FLOW,
        TemporalSectionType::PARADOX_HANDLING, TemporalSectionType::VARIABLES,
        TemporalSectionType::INITIALIZATION_PHASE,
        TemporalSectionType::EXECUTION_WEAVE,
