...
MultiDimensionalVector::MultiDimensionalVector(
...
MultiDimensionalVector::MultiDimensionalVector(
    std::shared_ptr<ChronovyanVector> flatVector,
    const std::vector<size_t> &dimensions,
    std::shared_ptr<TemporalRuntime> runtime)
  ...
  if (flatVector->size() != expectedSize) {
    throw std::invalid_argument(
        "Flat vector size doesn't match the product of dimensions");
  }
...
size_t MultiDimensionalVector::getDimensionCount() const {
  return m_dimensions.size();
}

// Get dimension sizes
const std::vector<size_t> &MultiDimensionalVector::getDimensions() const {
  return m_dimensions;
}

// Get total number of elements
size_t MultiDimensionalVector::getTotalSize() const {
  return m_flatVector->size();
}

// Get element at multi-dimensional indices (const)
const Value &
MultiDimensionalVector::at(const std::vector<size_t> &indices) const {
...
void MultiDimensionalVector::set(const std::vector<size_t> &indices,
                                 const Value &value) {
  if (!validateIndices(indices)) {
    throw std::out_of_range("Vector indices out of range");
  }
...
MultiDimensionalVector::slice(size_t dimension, size_t index) const {
  ...
  return std::make_shared<MultiDimensionalVector>(sliceVector, newDimensions,
                                                  m_runtime);
}
...
std::shared_ptr<MultiDimensionalVector>
MultiDimensionalVector::transpose() const {
  ...
  return std::make_shared<MultiDimensionalVector>(transposedVector,
                                                  newDimensions, m_runtime);
}
...
std::shared_ptr<MultiDimensionalVector> MultiDimensionalVector::matrixMultiply(
  ...
  if (cols1 != rows2) {
    throw std::runtime_error(
        ...
        std::to_string(rows1) + "x" + std::to_string(cols1) + " and " +
        std::to_string(rows2) + "x" + std::to_string(cols2));
  }
  ...
  auto resultMatrix = std::make_shared<MultiDimensionalVector>(
      resultVector, resultDimensions, m_runtime);
    ...
    for (size_t j = 0; j < cols2; ++j) {
      ...
      for (size_t k = 0; k < cols1; ++k) {
        ...
        if (sum.isInteger() && product.isInteger()) {
          sum = Value(
              static_cast<int64_t>(sum.asInteger() + product.asInteger()));
        } else {
  ...
  // Track resource usage
  const_cast<MultiDimensionalVector *>(this)->trackResourceUsage(
      "matrixMultiply");
...
std::shared_ptr<MultiDimensionalVector> MultiDimensionalVector::elementWise(
  ...
  if (m_dimensions.size() != other.getDimensions().size()) {
    throw std::runtime_error("Vectors must have same number of dimensions for "
  ...
  for (size_t i = 0; i < m_flatVector->size(); ++i) {
    resultVector->push_back(
        op(m_flatVector->at(i), other.getFlatVector()->at(i)));
  }
  ...
  return std::make_shared<MultiDimensionalVector>(resultVector, m_dimensions,
                                                  m_runtime);
}

// Map function over elements
std::shared_ptr<MultiDimensionalVector>
MultiDimensionalVector::map(std::function<Value(const Value &)> func) const {
...
std::shared_ptr<MultiDimensionalVector> MultiDimensionalVector::reduce(
  ...
  // Create the result multi-dimensional vector
  auto resultMDVector = std::make_shared<MultiDimensionalVector>(
      resultVector, newDimensions, m_runtime);
...
// Get flat vector
std::shared_ptr<ChronovyanVector>
MultiDimensionalVector::getFlatVector() const {
  return m_flatVector;
}
...
Value MultiDimensionalVector::getAtCycle(const std::vector<size_t> &indices,
                                         int cycle) const {
  if (!validateIndices(indices)) {
    throw std::out_of_range("Vector indices out of range");
  }
...
// Temporal transform
std::shared_ptr<MultiDimensionalVector>
MultiDimensionalVector::temporalTransform(
    int startCycle, int endCycle,
    std::function<Value(const Value &, const Value &, double)> transformFunc)
    const {
  // Create a new flat vector for the result
  auto resultVector = std::make_shared<ChronovyanVector>(m_runtime);
  ...
  // Create the result multi-dimensional vector
  auto resultMDVector = std::make_shared<MultiDimensionalVector>(
      resultVector, m_dimensions, m_runtime);
  ...
  for (size_t i = 0; i < totalSize; ++i) {
    ...
    Value transformedValue = transformFunc(
        startValue, endValue, static_cast<double>(endCycle - startCycle));
...
// Temporal gradient
std::shared_ptr<MultiDimensionalVector>
MultiDimensionalVector::temporalGradient(int startCycle, int endCycle) const {
  ...
  auto gradientFunc = [](const Value &start, const Value &end,
...
std::shared_ptr<MultiDimensionalVector>
MultiDimensionalVector::predictFutureState(
  ...
  for (size_t i = 0; i < totalSize; ++i) {
    ...
    if (history.size() >= 2 && history.back().second.isNumeric() &&
      ...
      double endVal = history.back().second.isInteger()
                          ? history.back().second.asInteger()
      ...
      // For non-numeric values or insufficient history, use the most recent
      // value
...
size_t MultiDimensionalVector::indicesToFlatIndex(
...
std::vector<size_t>
MultiDimensionalVector::flatIndexToIndices(size_t flatIndex) const {
...
bool MultiDimensionalVector::validateIndices(
    const std::vector<size_t> &indices) const {
...
int64_t MultiDimensionalVector::getChrononsForOperation(
    const std::string &operation) const {
  // Base cost for any operation
  int64_t baseCost = 1;

  // Scaling factor based on total size (logarithmic to be efficient for large
  // tensors)
  ...
  if (operation == "init" || operation == "init_with_elements") {
    ...
    if (m_dimensions.size() == 2) {
      ...
      return baseCost +
             static_cast<int64_t>(n * n * n /
                                  10); // Divide by 10 to make it reasonable
    }
    ...
    return baseCost + static_cast<int64_t>(getTotalSize());
             ...
             operation == "restoreFromCycle") {
    return baseCost + static_cast<int64_t>(getTotalSize() * 2);
  } else if (operation == "temporalTransform" ||
             operation == "temporalGradient" ||
...
void registerMultiDimensionalVectorType(CustomTypeSystem &typeSystem) {
  ...
  auto vectorProps = std::map<std::string, Value>{
      ...
      {"description", Value("Multi-dimensional vector type with temporal "
                            "features and matrix operations")}};
  ...
  auto vectorType = std::make_shared<CustomTypeDefinition>(
      "MULTI_DIMENSIONAL_VECTOR", CustomTypeKind::STRUCT, vectorProps,
      vectorParams);
