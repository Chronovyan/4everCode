...
BreakpointManager::BreakpointManager() : nextBreakpointId(1), hitBreakpointId(-1) {
...
std::map<int, std::pair<std::string, int>> BreakpointManager::getAllBreakpoints() const {
...
bool BreakpointManager::isSpecificBreakpointHit(int breakpointId, const Breakpoint& bp,
...
bool BreakpointManager::evaluateCondition(const BreakpointCondition& condition,
    ...
    switch (condition.type) {
        ...
        case BreakpointType::RESOURCE_THRESHOLD: {
            ...
            if (nameIt == condition.parameters.end() || thresholdIt == condition.parameters.end()) {
                return false;
            }
        ...
        case BreakpointType::TEMPORAL_OPERATION: {
            ...
            // For demonstration, we'll just check if we're in a function with a matching name
