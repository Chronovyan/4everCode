...
ResourceVisualizer::ResourceVisualizer(const ResourceTracker& tracker) : tracker_(tracker) {}

std::string ResourceVisualizer::generateAsciiGraph(size_t width, size_t height) const {
...
std::string ResourceVisualizer::generateDetailedReport() const {
    ...
    report << "  Chronon Efficiency: " << std::fixed << std::setprecision(2)
           << (stats["current_chronon_usage"] / (stats["max_chronon_usage"] + 0.0001) * 100) << "%"
           << std::endl;
    report << "  Aethel Efficiency: " << std::fixed << std::setprecision(2)
           << (stats["current_aethel_usage"] / (stats["max_aethel_usage"] + 0.0001) * 100) << "%"
           << std::endl;
    report << "  Temporal Stability: " << std::fixed << std::setprecision(2)
           << (100 - (stats["current_temporal_debt"] / (stats["max_temporal_debt"] + 0.0001) * 100))
           << "%" << std::endl;
    report << "  Paradox Containment: " << std::fixed << std::setprecision(2)
           << (100 - (stats["current_paradox_risk"] / (stats["max_paradox_risk"] + 0.0001) * 100))
           << "%" << std::endl;
    report << std::endl;
...
std::string ResourceVisualizer::generateAsciiHistogram(const std::vector<double>& values,
...
std::string ResourceVisualizer::generateAsciiTimeline(const std::vector<ResourceDataPoint>& data,
                                                      size_t width) const {
    if (data.size() < 2) {
...
std::vector<size_t> ResourceVisualizer::normalizeValues(const std::vector<double>& values,
                                                        size_t height) const {
    ...
    for (double value : values) {
        ...
        size_t scaled =
            value > 0 ? std::max(size_t(1), static_cast<size_t>((value / max_value) * height)) : 0;
...
std::string ResourceVisualizer::generateAsciiBar(size_t height, size_t max_height) const {
    std::string bar;
