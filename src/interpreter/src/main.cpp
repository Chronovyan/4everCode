...
bool deserializeBytecodeModule(const std::string &inputFile,
  ...
  char header[4];
  in.read(header, 4);
  if (in.gcount() != 4 || header[0] != 'C' || header[1] != 'H' ||
      header[2] != 'R' || header[3] != 'O') {
    std::cerr << "Error: Invalid bytecode file format. Not a Chronovyan "
                 "bytecode file."
              << std::endl;
    return false;
  }
  ...
  if (in.gcount() != sizeof(version) || version != 1) {
    std::cerr << "Error: Unsupported bytecode version: "
  ...
  in.read(reinterpret_cast<char *>(&constantPoolSize),
          sizeof(constantPoolSize));
  ...
  for (uint32_t i = 0; i < functionCount; ++i) {
    ...
    if (in.gcount() != sizeof(paramCount)) {
      std::cerr << "Error: Failed to read function parameter count"
                << std::endl;
      return false;
    }
    ...
    in.read(reinterpret_cast<char *>(&instructionCount),
    ...
    auto function =
        std::make_shared<compiler::bytecode::BytecodeFunction>(name);
...
bool runBytecodeFile(const std::string &inputFile, bool verbose) {
  ...
  if (verbose) {
    ...
    for (const auto &pair : module.getFunctions()) {
      ...
      std::cout << "  Function '" << function->getName()
                ...
                << " instructions, " << function->getParameterCount()
                << " parameters" << std::endl;
    }
  }
  ...
  try {
    ...
    if (verbose) {
      std::cout << "Program executed successfully with result: "
                << result.toString() << std::endl;
      std::cout << "Final PARADOX_LEVEL: " << stdlib::get_paradox_level()
                << std::endl;
    }
...
int main(int argc, char *argv[]) {
  ...
  if (inputFile.empty()) {
    std::cerr << "Usage: " << argv[0] << " [options] bytecode_file"
              << std::endl;
