...
void printTimestreamInfo(const std::shared_ptr<Timestream>& timestream) {
    ...
    std::cout << Color::BOLD << "Timestream: " << Color::GREEN << timestream->getName()
              << Color::RESET << " (ID: " << timestream->getId() << ")" << std::endl;
    ...
    if (source_anchor) {
        std::cout << "  Source Anchor: " << source_anchor->getId() << " (Stability: " << std::fixed
                  << std::setprecision(2) << source_anchor->getStability() << ")" << std::endl;
    }

    std::cout << "  Anchors: " << std::endl;
    for (const auto& anchor : timestream->getAnchors()) {
        std::cout << "    - " << Color::CYAN << anchor->getId() << Color::RESET
                  << " (Stability: " << std::fixed << std::setprecision(2) << anchor->getStability()
...
void printDebtInfo(const TemporalDebtTracker& tracker) {
    std::cout << Color::BOLD << "Temporal Debt Status:" << Color::RESET << std::endl;
    ...
    std::cout << "  Stability: " << std::fixed << std::setprecision(2) << stability;
    if (stability > 0.8) {
    ...
    std::cout << "  Debt Limit: " << std::fixed << std::setprecision(2) << debt_limit << std::endl;
    std::cout << "  Total Chronon Debt: " << std::fixed << std::setprecision(2)
              << total_chronon_debt << std::endl;
    std::cout << "  Total Aethel Debt: " << std::fixed << std::setprecision(2) << total_aethel_debt
              << std::endl;
    ...
    if (debts.empty()) {
        ...
        for (const auto& debt : debts) {
            std::cout << "    * Operation " << debt.operation_id << " - Chronons: " << std::fixed
                      << std::setprecision(2) << debt.chronon_debt
                      << ", Aethel: " << debt.aethel_debt;
...
void visualizeTimestreams(const TimestreamManager& manager, TimeAnchor* selected_anchor = nullptr) {
    ...
    std::cout << Color::BOLD << Color::CYAN << "Main Timestream: " << Color::RESET
              << main_timestream->getName() << " (ID: " << main_timestream->getId() << ")"
              << std::endl;
    ...
    for (size_t i = 0; i < main_anchors.size(); i++) {
        ...
        std::cout << "├── " << Color::YELLOW << "Anchor: " << Color::RESET
                  ...
                  << std::setprecision(2) << anchor->getStability() << ")" << std::endl;
        ...
        if (branch_it != children.end() && i < main_anchors.size() - 1) {
            ...
            for (const auto& child : branch_it->second) {
                ...
                if (!anchors.empty()) {
                    const auto& origin_anchor = anchors[0];  // Get the first anchor as origin
                    if (origin_anchor && origin_anchor->getId() == anchor->getId()) {
                        std::cout << "│   │" << std::endl;
                        std::cout << "│   ├── " << Color::MAGENTA << "Branch: " << Color::RESET
                        ...
                        for (const auto& child_anchor : child_anchors) {
                            std::cout << "│   │   │" << std::endl;
                            std::cout
                                << "│   │   ├── " << Color::YELLOW << "Anchor: " << Color::RESET
                                << child_anchor->getDescription() << " (Stability: " << std::fixed
                                << std::setprecision(2) << child_anchor->getStability() << ")"
                                << std::endl;
                        }
                        std::cout << "│   │" << std::endl;
                    }
...
void simulateDeveloperWorkflow(TimestreamManager& manager, TemporalDebtTracker& debt_tracker) {
    printHeader("Chronovyan Timestream Version Control System");
    std::cout << "This demonstration simulates a development workflow using the Chronovyan "
                 "Timestream system."
              << std::endl;
    std::cout << "We'll create anchors, branch into Echoes, and harmonize changes while managing "
                 "temporal debt."
              << std::endl;
    std::cout << std::endl;
    ...
    std::cout << "First, we'll create an anchor for our initial implementation:" << std::endl;

    auto anchor1 = manager.createAnchor(0.95, "Initial implementation");

    simulateProcessing("Creating anchor", 1500);

    std::cout << "Anchor created: " << anchor1->getDescription() << " (ID: " << anchor1->getId()
              << ")" << std::endl;
    std::cout << "Stability: " << std::fixed << std::setprecision(2) << anchor1->getStability()
              << std::endl;
    std::cout << std::endl;

    // Create a bug fix branch
    printSection("Creating Bug Fix Branch");
    std::cout << "Now, let's create an Echo (branch) to fix a critical bug:" << std::endl;

    std::string bugfix_name = "Bug Fix Branch";
    auto bugfix_branch = manager.createEcho(
        bugfix_name, manager.getMainTimestream()->getId(),
        static_cast<int>(TimestreamType::BUGFIX)  // Cast to int if the API expects an int
    );
    ...
    std::cout << "Echo created: " << bugfix_branch->getName() << " (ID: " << bugfix_branch->getId()
              << ")" << std::endl;
    ...
    std::cout << "Meanwhile, in the main timestream, we'll create another anchor for a new feature:"
              << std::endl;
    ...
    std::cout << "Anchor created: " << anchor2->getDescription() << " (ID: " << anchor2->getId()
              << ")" << std::endl;
    std::cout << "Stability: " << std::fixed << std::setprecision(2) << anchor2->getStability()
              << std::endl;
    std::cout << std::endl;

    // Create feature branch
    printSection("Creating Feature Branch");
    std::cout << "Now, we'll create another Echo for developing the new feature:" << std::endl;

    std::string feature_name = "Feature Branch";
    auto feature_branch = manager.createEcho(
        feature_name, manager.getMainTimestream()->getId(),
        static_cast<int>(TimestreamType::FEATURE)  // Cast to int if the API expects an int
    );
    ...
    std::cout << "Creating multiple Echoes increases our temporal debt:" << std::endl;
    printDebtInfo(debt_tracker);
    ...
    std::cout << "Let's harmonize (merge) the bug fix into the main timestream:" << std::endl;

    bool success =
        manager.harmonizeTimestreams(bugfix_branch->getId(), manager.getMainTimestream()->getId(),
                                     RepaymentStrategyType::CRITICAL_FIRST);

    simulateProcessing("Harmonizing bug fix branch", 2000);

    if (success) {
        std::cout << Color::GREEN << "Harmonization successful!" << Color::RESET << std::endl;
    } else {
        std::cout << Color::RED << "Harmonization failed! Timestreams too divergent."
    ...
    std::cout << "Now, let's assess the risk of harmonizing our experimental feature branch:"
              << std::endl;

    double risk =
        manager.calculateEchoParadoxRisk(manager.getMainTimestream()->getId(), anchor2->getId());

    simulateProcessing("Calculating paradox risk", 1500);

    std::cout << "Paradox risk: " << std::fixed << std::setprecision(2) << risk << std::endl;
    if (risk < 0.3) {
        std::cout << Color::GREEN << "Low risk - safe to proceed with harmonization."
                  << Color::RESET << std::endl;
    } else if (risk < 0.7) {
        std::cout << Color::YELLOW << "Moderate risk - proceed with caution." << Color::RESET
                  << std::endl;
    } else {
        std::cout << Color::RED << "High risk - harmonization may cause instability."
                  << Color::RESET << std::endl;
    }

    // Apply debt repayment
    printSection("Debt Repayment");
    std::cout << "Before proceeding further, let's allocate resources to repay some temporal debt:"
              << std::endl;

    double chronon_resources = 20.0;
    double aethel_resources = 25.0;

    std::cout << "Available resources - Chronon: " << std::fixed << std::setprecision(2)
              << chronon_resources << ", Aethel: " << aethel_resources << std::endl;

    double stability_before = debt_tracker.getStability();
    std::cout << "Stability before repayment: " << std::fixed << std::setprecision(2)
              << stability_before << std::endl;

    simulateProcessing("Applying balanced repayment strategy", 2000);

    debt_tracker.applyRepaymentStrategy(RepaymentStrategyType::BALANCED, chronon_resources,
                                        aethel_resources);

    double stability_after = debt_tracker.getStability();
    std::cout << "Stability after repayment: " << std::fixed << std::setprecision(2)
              << stability_after << std::endl;
    ...
    std::cout << "This demonstration showed key features of the Chronovyan Timestream Version "
                 "Control System:"
              << std::endl;
    std::cout << "  1. Creating anchors to mark significant points in development" << std::endl;
    std::cout << "  2. Creating Echoes (branches) for parallel development" << std::endl;
    std::cout << "  3. Harmonizing (merging) changes between timestreams" << std::endl;
    ...
    std::cout << "Total timestreams: " << manager.getAllTimestreams().size() << std::endl;
    std::cout << std::endl;

    std::cout << "Thank you for exploring the Chronovyan Timestream Version Control System!"
              << std::endl;
}
...
void simulateDemo() {
    ...
    auto bugFixBranch = manager.createEcho(bugFixName, manager.getMainTimestream()->getId(),
                                           static_cast<int>(TimestreamType::BUGFIX));

    // Display the new branch
    std::cout << "Bug fix branch created: " << bugFixBranch->getName() << std::endl;

    // Create feature branch
    std::cout << "Creating feature branch..." << std::endl;
    std::string featureName = "feature_" + std::to_string(std::time(nullptr));
    auto featureBranch = manager.createEcho(featureName, manager.getMainTimestream()->getId(),
                                            static_cast<int>(TimestreamType::FEATURE));

    // Display the new branch
    std::cout << "Feature branch created: " << featureBranch->getName() << std::endl;
