...
ChronovyanVector::ChronovyanVector(std::shared_ptr<TemporalRuntime> runtime)
    : m_runtime(runtime) {
...
std::shared_ptr<ChronovyanVector> ChronovyanVector::slice(size_t start,
                                                          size_t end) const {
  ...
  std::vector<Value> sliced_elements(m_elements.begin() + start,
...
ChronovyanVector::concat(const ChronovyanVector &other) const {
  // Use a mutable function to track resource usage
  const_cast<ChronovyanVector *>(this)->trackResourceUsage("concat");

  std::vector<Value> combined = m_elements;
  combined.insert(combined.end(), other.getData().begin(),
                  other.getData().end());
...
std::shared_ptr<ChronovyanVector>
ChronovyanVector::map(std::function<Value(const Value &)> func) const {
  // Use a mutable function to track resource usage
...
std::shared_ptr<ChronovyanVector>
ChronovyanVector::filter(std::function<bool(const Value &)> pred) const {
  // Use a mutable function to track resource usage
...
int64_t ChronovyanVector::indexOf(const Value &value,
...
bool ChronovyanVector::contains(const Value &value) const {
...
const std::vector<Value> &ChronovyanVector::getData() const {
  return m_elements;
}
...
void ChronovyanVector::setTemporalRuntime(
    std::shared_ptr<TemporalRuntime> runtime) {
  m_runtime = runtime;
}

// Get temporal runtime
std::shared_ptr<TemporalRuntime> ChronovyanVector::getTemporalRuntime() const {
  return m_runtime;
}

// Resource costs
int64_t ChronovyanVector::getChrononsForOperation(const std::string &operation,
                                                  size_t vectorSize) {
  // Base cost for any operation
  int64_t baseCost = 1;

  // Scaling factor based on vector size (logarithmic to be efficient for large
  // vectors)
...
void registerVectorType(CustomTypeSystem &typeSystem) {
  ...
  auto vectorType = std::make_shared<CustomTypeDefinition>(
      "VECTOR", CustomTypeKind::STRUCT, vectorProps, vectorParams);
...
void registerMultiDimensionalVectorType(CustomTypeSystem &typeSystem) {
  ...
  auto multiVectorType = std::make_shared<CustomTypeDefinition>(
      "MULTIVECTOR",
      CustomTypeKind::ALIAS, // It's essentially an alias for a specialized
                             // vector
