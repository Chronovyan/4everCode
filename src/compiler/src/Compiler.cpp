...
public:
    ConcreteSemanticAnalyzer(DiagnosticReporter& diagnostics) : SemanticAnalyzer(diagnostics) {}
...
public:
    void visitParallelExecutionStmtNode(ParallelExecutionStmtNode* node) override {
        ...
        BytecodeInstruction parallelInstr(BytecodeNodeType::PARALLEL_EXECUTION, parallelId);
        // Add thread count as a field
        parallelInstr.addField("threadCount", static_cast<int32_t>(1));  // Default to 1 thread
    ...
    void visitBranchTimelineStmtNode(BranchTimelineStmtNode* node) override {
        ...
        BytecodeInstruction branchInstr(BytecodeNodeType::BRANCH_TIMELINE, branchId);
...
public:
    void visitParallelExecutionStmtNode(ParallelExecutionStmtNode* node) override {
        ...
        printLine(
            "ChronovyanParallelExecution* parallel_ctx = "
    ...
    void visitBranchTimelineStmtNode(BranchTimelineStmtNode* node) override {
        ...
        printLine("ChronovyanTimeline* timeline_branch = chronovyan_branch_timeline(runtime, " +
                  branchCountCode + ");");
...
bool Compiler::compile(const std::filesystem::path& inputPath,
                       const std::filesystem::path& outputPath, bool generateChron) {
    // Check if input file exists
    if (!std::filesystem::exists(inputPath)) {
        SourceLocation location;
        diagnostics.addError(location, "Input file does not exist: " + inputPath.string());
        return false;
    }
    ...
    if (!inFile.is_open()) {
        SourceLocation location;
        diagnostics.addError(location, "Failed to open input file: " + inputPath.string());
        return false;
    }
...
bool Compiler::compileString(const std::string& sourceCode, const std::filesystem::path& outputPath,
                             bool generateChron) {
    // Reset diagnostics
    ...
    if (generateChron) {
        ...
        if (!ast || parser.hasErrors()) {
            ...
            const auto& parserDiagnostics = parser.getDiagnosticReporter().getDiagnostics();
...
bool Compiler::saveToFile(const std::string& generatedCode,
    ...
    if (!outFile.is_open()) {
        SourceLocation location;
        diagnostics.addError(location, "Failed to open output file: " + outputPath.string());
        return false;
    }
...
bool Compiler::generateChronFile(std::unique_ptr<AstNode>& ast,
    ...
    if (!programNode) {
        SourceLocation location;
        diagnostics.addError(location, "Internal error: AST root is not a ProgramNode");
        return false;
    }
    ...
    if (!success) {
        SourceLocation location;
        diagnostics.addError(location, "Failed to generate .chron file: " + outputPath.string());
        return false;
    }
