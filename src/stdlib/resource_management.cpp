 ...
 * @brief Implementation of resource management functions for the Chronovyan
 * Standard Library
 *
...
bool check_aethel(AETHEL_RESERVE &reserve, int amount) {
  if (reserve < amount) {
    std::cout << "Insufficient Aethel: " << reserve << " < " << amount
              << std::endl;
    return false;
  }
  return true;
...
bool consume_aethel(AETHEL_RESERVE &reserve, int amount) {
  ...
  reserve -= amount;
  std::cout << "Consumed " << amount << " Aethel, remaining: " << reserve
            << std::endl;
  return true;
}
...
bool check_chronons(CHRONON_POOL &pool, int amount) {
  if (pool < amount) {
    std::cout << "Insufficient Chronons: " << pool << " < " << amount
              << std::endl;
    return false;
  }
  return true;
...
bool consume_chronons(CHRONON_POOL &pool, int amount) {
  ...
  pool -= amount;
  std::cout << "Consumed " << amount << " Chronons, remaining: " << pool
            << std::endl;
  return true;
}
...
void replenish_aethel(AETHEL_RESERVE &reserve, int amount) {
  reserve += amount;
  std::cout << "Replenished " << amount << " Aethel, new total: " << reserve
            << std::endl;
}
...
void replenish_chronons(CHRONON_POOL &pool, int amount) {
  pool += amount;
  std::cout << "Replenished " << amount << " Chronons, new total: " << pool
            << std::endl;
}
...
AETHEL_RESERVE ResourceManager::refine_aethel(const AETHEL_RESERVE &reserve,
  ...
  AETHEL_RESERVE refined_amount =
      static_cast<AETHEL_RESERVE>(reserve * refinement_efficiency);

  std::cout << "Refined " << reserve << " Aethel to quality level "
            << quality_target << " resulting in " << refined_amount
            << " high-quality Aethel" << std::endl;
