...
const std::vector<Value> &ChronovyanArray::getElements() const {
  return m_elements;
}
...
Value::Value(std::shared_ptr<ChronovyanArray> value)
    : m_value(std::move(value)) {}

Value::Value(std::shared_ptr<ChronovyanMap> value)
    : m_value(std::move(value)) {}

Value::Value(std::shared_ptr<NativeFunction> value)
    : m_value(std::move(value)) {}

Value::Value(std::shared_ptr<ChronovyanFunction> value)
    : m_value(std::move(value)) {}
...
Value::Type Value::getType() const {
  ...
  } else if (std::holds_alternative<std::shared_ptr<ChronovyanFunction>>(
                 m_value)) {
    return Type::CHRONOVYAN_FUNCTION;
  } else if (std::holds_alternative<std::shared_ptr<ChronovyanArray>>(
                 m_value)) {
    return Type::ARRAY;
...
bool Value::equals(const Value &other) const {
  ...
  case Type::ARRAY:
    if (std::holds_alternative<std::vector<ValuePtr>>(m_value) &&
      ...
      return true;
                   ...
                   m_value) &&
               std::holds_alternative<std::shared_ptr<ChronovyanArray>>(
                   other.m_value)) {
  ...
  case Type::MAP:
    if (std::holds_alternative<std::map<std::string, ValuePtr>>(m_value) &&
        std::holds_alternative<std::map<std::string, ValuePtr>>(
            other.m_value)) {
      ...
      for (const auto &[key, value] : a) {
        auto it = b.find(key);
        if (it == b.end() || !value || !it->second ||
            !value->equals(*it->second)) {
          return false;
        }
      }

      return true;
    } else if (std::holds_alternative<std::shared_ptr<ChronovyanMap>>(
                   m_value) &&
               std::holds_alternative<std::shared_ptr<ChronovyanMap>>(
                   other.m_value)) {
...
std::vector<Value> Value::asArray() const {
  ...
  throw std::runtime_error(
      "Value is not an array or cannot be converted to one");
}
...
std::shared_ptr<ChronovyanArray> Value::asChronovyanArray() const {
  ...
  throw std::runtime_error(
      "Value is not a ChronovyanArray or convertible to one");
}
...
std::shared_ptr<ChronovyanMap> Value::asChronovyanMap() const {
  ...
  throw std::runtime_error(
      "Value is not a ChronovyanMap or convertible to one");
}
