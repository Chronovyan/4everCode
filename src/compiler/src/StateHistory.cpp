...
std::vector<std::string> StateHistory::getVariableHistory(const std::string& variableName) const {
...
std::vector<double> StateHistory::getResourceHistory(const std::string& resourceName) const {
...
std::map<int, int> StateHistory::getTimelineBranchingHistory() const { return timelineBranching; }
...
void StateHistory::updateTimelineBranchingHistory(const DebuggerState& state) {
    ...
    if (timelineId > 0 && timelineBranching.find(timelineId) == timelineBranching.end()) {
