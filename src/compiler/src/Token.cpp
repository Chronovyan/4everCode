...
std::ostream& operator<<(std::ostream& os, const Token& token) { return os << token.toString(); }
...
bool isKeyword(const std::string& identifier, TokenType& keywordType) {
    ...
    for (char c : identifier) {
        upperIdentifier.push_back(static_cast<char>(std::toupper(static_cast<unsigned char>(c))));
    }
