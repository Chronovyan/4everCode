...
TimeAnchor::TimeAnchor(const std::string& id, double stability, const std::string& description)
    : m_id(id)
    , m_stability(std::max(0.0, std::min(1.0, stability)))
    ,  // Clamp between 0.0 and 1.0
    m_creation_time(getCurrentTimestamp())
...
void TimeAnchor::setDescription(const std::string& description) { m_description = description; }
...
double TimeAnchor::calculateTemporalDistance(const TimeAnchor& other) const {
    ...
    double time_diff =
        std::abs(static_cast<double>(m_creation_time) - static_cast<double>(other.m_creation_time));
