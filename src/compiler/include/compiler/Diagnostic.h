...
/**
 * DiagnosticMessage represents a diagnostic message with severity, location, and message.
 */
...
public:
    ...
    DiagnosticMessage(DiagnosticSeverity severity, const Token& token, std::string message)
        : severity(severity), location(token.getLocation()), message(std::move(message)) {}
...
/**
 * DiagnosticReporter is responsible for collecting and reporting diagnostic messages.
 */
