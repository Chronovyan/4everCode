...
StatementVisitor::StatementVisitor(Interpreter &interpreter)
...
void StatementVisitor::visitFunctionDeclStmt(const FunctionDeclStmtNode &stmt) {
  ...
  auto function = std::make_shared<Function>(stmt.getName(), stmt.getBody(),
                                             m_interpreter.getEnvironment());
...
void StatementVisitor::visitTypeDefStmt(const TypeDefStmtNode &stmt) {
  ...
  if (stmt.isStruct()) {
    ...
    auto builder =
        typeSystem->createTypeBuilder(stmt.getName(), CustomTypeKind::STRUCT);
    ...
    try {
      ...
      throw RuntimeError("Failed to define struct type '" + stmt.getName() +
                             "': " + e.what(),
                         stmt.getLocation());
    }
  } else if (stmt.isEnum()) {
    // Create an enum type
    auto builder =
        typeSystem->createTypeBuilder(stmt.getName(), CustomTypeKind::ENUM);
    ...
    try {
      ...
      throw RuntimeError("Failed to define enum type '" + stmt.getName() +
                             "': " + e.what(),
                         stmt.getLocation());
    }
    ...
    // Create a union type
    auto builder =
        typeSystem->createTypeBuilder(stmt.getName(), CustomTypeKind::UNION);
    ...
    try {
      ...
      throw RuntimeError("Failed to define union type '" + stmt.getName() +
                             "': " + e.what(),
                         stmt.getLocation());
    }
    ...
    // Create a type alias
    auto builder =
        typeSystem->createTypeBuilder(stmt.getName(), CustomTypeKind::ALIAS);
    ...
    try {
      ...
      throw RuntimeError("Failed to define type alias '" + stmt.getName() +
                             "': " + e.what(),
                         stmt.getLocation());
    }
  }
