...
std::string Clock::formatTime(double timestamp, const std::string& format) const {
    using namespace std::chrono;

    // Convert timestamp to time_point
    system_clock::time_point tp{duration_cast<system_clock::duration>(duration<double>(timestamp))};
