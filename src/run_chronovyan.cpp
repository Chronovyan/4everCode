...
int main(int argc, char **argv) {
  ...
  try {
    ...
    if (!compileSuccess || diagnostics.hasErrors()) {
      ...
      for (const auto &diag : diagnostics.getDiagnostics()) {
        std::cerr << inputFile << ":" << diag.getLocation().line << ":"
                  << diag.getLocation().column << ": " << diag.getMessage()
                  << std::endl;
      }
      return 1;
    }
    ...
    if (!fs::exists(outputFile)) {
      std::cerr << "Error: Expected output file not created: " << outputFile
                << std::endl;
      return 1;
    }

    // For now, we'll just print a message since we don't have a way to load the
    // module from a file directly. The interpreter would need to be updated to
    // support this.
    std::cout << "Successfully compiled to " << outputFile << std::endl;
    std::cout
        << "Note: Direct execution of compiled bytecode is not yet implemented."
        << std::endl;
    return 0;
