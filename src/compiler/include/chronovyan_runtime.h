...
ChronovyanRuntime* chronovyan_init() {
    ChronovyanRuntime* runtime = (ChronovyanRuntime*)malloc(sizeof(ChronovyanRuntime));
    if (runtime) {
...
// Resource operations
void chronovyan_ALLOCATE_resources(ChronovyanRuntime* runtime, int chronons, int aethel) {
    if (runtime) {
...
void chronovyan_EXPEND_resources(ChronovyanRuntime* runtime, int chronons, int aethel) {
    if (runtime) {
        if (runtime->chronons_available >= chronons && runtime->aethel_available >= aethel) {
            runtime->chronons_available -= chronons;
            runtime->aethel_available -= aethel;
...
void chronovyan_BORROW_resources(ChronovyanRuntime* runtime, int chronons, int aethel) {
    if (runtime) {
...
void chronovyan_REPAY_resources(ChronovyanRuntime* runtime, int chronons, int aethel) {
    if (runtime) {
        if ((runtime->chronons_borrowed && chronons > 0) ||
            ...
            if (runtime->chronons_available >= chronons && runtime->aethel_available >= aethel) {
                runtime->chronons_available -= chronons;
                runtime->aethel_available -= aethel;
                runtime->chronons_borrowed = chronons > 0 ? false : runtime->chronons_borrowed;
                runtime->aethel_borrowed = aethel > 0 ? false : runtime->aethel_borrowed;
