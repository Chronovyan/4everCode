 ...
 *
 * This class orchestrates the entire compilation process, from parsing source
 * code to generating bytecode. It manages the different compiler components and
 * provides a simple interface for compiling Chronovyan programs.
 */
class CompilerDriver {
public:
  /**
   ...
   * @param level The minimum optimization level at which to run this pass.
   */
  void registerTemporalOptimizationPass(
      std::unique_ptr<OptimizationPass> pass,
   ...
   * @param reportFile Path to save the report to (empty for console output
   * only).
   * @param format The report format (text, html, json).
   */
  void enableValidationReporting(bool enable,
  ...
  getProvingGroundsFramework() const;
...
private:
  ...
  bool provingGroundsEnabled; ///< Whether Temporal Proving Grounds validation
                              ///< is enabled
  ...
  std::string validationReportFormat; ///< Format for validation reports (text,
                                      ///< html, json)
  std::shared_ptr<proving_grounds::TemporalTestFramework>
