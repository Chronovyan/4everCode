...
// We no longer need the extern declaration since we're including the header
...
TEST(FileExtensionTest, InvalidExtension) {
    ...
    EXPECT_FALSE(hasValidExtension(
        "t.cvy"));  // This should be valid, but our implementation requires at least 5 chars
}
...
TEST(FileExtensionTest, EdgeCases) {
    ...
    EXPECT_FALSE(
        hasValidExtension("test.txt.cvy"));  // Should be valid but depends on implementation
