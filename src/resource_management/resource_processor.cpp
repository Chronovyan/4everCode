 ...
 */
bool serializeTimePoint(std::ofstream& file,
                        const std::chrono::time_point<std::chrono::system_clock>& timePoint) {
    // Convert time_point to a simple representation (time since epoch in microseconds)
    auto micros =
 ...
 * @return true if successful, false otherwise
 */
bool deserializeTimePoint(std::ifstream& file,
    ...
    timePoint = std::chrono::system_clock::time_point(std::chrono::microseconds(micros));
    return true;
}
 ...
 * @return true if successful, false otherwise
 */
bool serializeResourceDataPoint(std::ofstream& file, const ResourceDataPoint& point) {
    // Serialize each field individually for safety
    if (!serializeTimePoint(file, point.timestamp)) {
        return false;
    }

    file.write(reinterpret_cast<const char*>(&point.chronon_usage), sizeof(point.chronon_usage));
    if (!file.good())
        return false;

    file.write(reinterpret_cast<const char*>(&point.aethel_usage), sizeof(point.aethel_usage));
    if (!file.good())
        return false;

    file.write(reinterpret_cast<const char*>(&point.temporal_debt), sizeof(point.temporal_debt));
    if (!file.good())
        return false;

    file.write(reinterpret_cast<const char*>(&point.paradox_risk), sizeof(point.paradox_risk));
    if (!file.good())
        return false;
 ...
 * @return true if successful, false otherwise
 */
bool deserializeResourceDataPoint(std::ifstream& file, ResourceDataPoint& point) {
    // Deserialize each field individually for safety
    if (!deserializeTimePoint(file, point.timestamp)) {
        return false;
    }

    file.read(reinterpret_cast<char*>(&point.chronon_usage), sizeof(point.chronon_usage));
    if (!file.good())
        return false;

    file.read(reinterpret_cast<char*>(&point.aethel_usage), sizeof(point.aethel_usage));
    if (!file.good())
        return false;

    file.read(reinterpret_cast<char*>(&point.temporal_debt), sizeof(point.temporal_debt));
    if (!file.good())
        return false;

    file.read(reinterpret_cast<char*>(&point.paradox_risk), sizeof(point.paradox_risk));
    if (!file.good())
        return false;
...
ResourceProcessor::ResourceProcessor(size_t maxHistorySize)
    ...
    // Create the optimizer with the tracker (ResourceOptimizer expects a const ResourceTracker&)
    ...
    visualizer_ =
        std::make_unique<chronovyan::ResourceVisualization>(tracker_, runtime, debt_tracker_);
...
bool ResourceProcessor::processCurrentUsage(double chrononUsage, double aethelUsage,
                                            double temporalDebt, double paradoxRisk) {
    // Record the current usage
    tracker_->recordCurrentUsage(chrononUsage, aethelUsage, temporalDebt, paradoxRisk);

    // Check if within safe thresholds
    return !checkResourceThresholds(chrononUsage, aethelUsage, temporalDebt, paradoxRisk);
}

std::string ResourceProcessor::getSummary() const {
    return visualizer_->generateSummaryReport(chronovyan::VisualizationFormat::TEXT);
}

std::string ResourceProcessor::getDetailedReport() const {
    return visualizer_->generateDetailedReport(chronovyan::VisualizationFormat::TEXT,
                                               20);  // Default history length
}

std::string ResourceProcessor::getVisualization() const {
    // Use updateDashboard method instead, which produces output containing "Chronovyan Resource
    // Visualization"
    return visualizer_->updateDashboard(chronovyan::VisualizationFormat::TEXT);
}

const ResourceTracker& ResourceProcessor::getTracker() const { return *tracker_; }
...
bool ResourceProcessor::isResourceStateHealthy(unsigned int timeout_ms) const {
    if (timeout_ms == 0) {
        ...
        double debt = debt_tracker_->getTotalChrononsDebt() + debt_tracker_->getTotalAethelDebt();
        ...
        std::packaged_task<bool()> task([this]() {
            ...
            // Calculate debt and risk factors
            double debt =
                debt_tracker_->getTotalChrononsDebt() + debt_tracker_->getTotalAethelDebt();
            double risk = 1.0 - debt_tracker_->getStability();
        ...
        try {
            if (future.wait_for(std::chrono::milliseconds(timeout_ms)) ==
                ...
                if (thread.joinable()) {
                    thread.join();  // Ensure thread is joined even on timeout before returning
                }
                // Log timeout?
    ...
    try {
        ...
        for (size_t i = 0; i < size; ++i) {
            if (!utils::serializeResourceDataPoint(file, data[i])) {
                std::cerr << "Error: Failed to write data point " << i << " to file" << std::endl;
                return false;
            }
        }
...
bool ResourceProcessor::loadFromLog(const std::string& filename) {
    try {
        ...
        if (fileSize < static_cast<std::streamsize>(sizeof(size_t))) {
            std::cerr << "Error: File is too small to contain valid data" << std::endl;
            return false;
        }
        ...
        if (size > 1000000) {  // Arbitrary large number as safety limit
            std::cerr << "Error: Unreasonable data size in file: " << size << std::endl;
            return false;
        }

        // Estimate expected file size (approximate, since we're using safer serialization)
        // This is a rough estimate based on the size of each field
        const size_t estimatedPointSize = sizeof(int64_t) + 4 * sizeof(double);
        std::streamsize estimatedMinimumSize = sizeof(size_t) + (estimatedPointSize * size);
        if (fileSize < estimatedMinimumSize) {
            std::cerr << "Error: File size is smaller than expected for " << size << " records"
                      << std::endl;
            return false;
        }
...
std::vector<OptimizationRecommendation> ResourceProcessor::getOptimizationRecommendations() const {
    ...
    if (metrics["chronon_efficiency"] < 0.5) {
        ...
        result.push_back(
            OptimizationRecommendation(OptimizationRecommendation::Type::RESOURCE_ALLOCATION,
...
std::map<std::string, double> ResourceProcessor::getEfficiencyMetrics() const {
    ...
    metrics["chronon_efficiency"] = 1.0 - (stats["current_chronon_usage"] / 100.0);
...
void ResourceProcessor::setMinimumDataPointsForOptimization(size_t min_data_points) {
    // This is a placeholder for future functionality
    // Mark as intentionally unused
    (void)min_data_points;
}

bool ResourceProcessor::processChronons(double amount, const std::string& operation_id,
    ...
    if (available >= amount) {
        // Record the usage
        tracker_->recordCurrentUsage(available - amount, stats["current_aethel_usage"],
    ...
    if (success) {
        ...
        tracker_->recordCurrentUsage(
            available + amount - amount,  // net effect is 0, but we record the transaction
...
bool ResourceProcessor::processAethel(double amount, const std::string& operation_id,
                                      bool allow_debt, bool critical) {
    // Get current resource statistics
    auto stats = tracker_->getResourceStatistics();
    ...
    if (available >= amount) {
        // Record the usage
        tracker_->recordCurrentUsage(stats["current_chronon_usage"], available - amount,
                                     stats["current_temporal_debt"], stats["current_paradox_risk"]);
        return true;
    }
    ...
    if (success) {
        // Record the change in resource state
        tracker_->recordCurrentUsage(stats["current_chronon_usage"],
                                     available + amount -
...
double ResourceProcessor::repayChrononsDebt(double amount, const std::string& operation_id) {
    // Get current resource statistics
    auto stats = tracker_->getResourceStatistics();
    double available = stats["current_chronon_usage"];
    ...
    // Record the change in resource state
    if (repaid > 0) {
        tracker_->recordCurrentUsage(available - repaid, stats["current_aethel_usage"],
                                     stats["current_temporal_debt"] - repaid,
                                     stats["current_paradox_risk"]);
    }
...
double ResourceProcessor::repayAethelDebt(double amount, const std::string& operation_id) {
    // Get current resource statistics
    auto stats = tracker_->getResourceStatistics();
    double available = stats["current_aethel_usage"];
    double to_repay = std::min(amount, available);
    ...
    // Record the change in resource state
    if (repaid > 0) {
        tracker_->recordCurrentUsage(stats["current_chronon_usage"], available - repaid,
                                     stats["current_temporal_debt"] - repaid,
                                     stats["current_paradox_risk"]);
    }
...
std::map<std::string, double> ResourceProcessor::getDebtStatus() const {
    ...
    status["past_due_count"] = static_cast<double>(debt_tracker_->getPastDueDebts().size());
...
ResourceProcessor::getDetailedDebtInfo() const {
    std::vector<std::map<std::string, std::variant<double, std::string, int, bool>>> result;
    ...
    for (const auto& debt : debts) {
        std::map<std::string, std::variant<double, std::string, int, bool>> debt_map;
...
std::string ResourceProcessor::generateResourceSummary() const {
    ...
    ss << visualizer_->generateSummaryReport(chronovyan::VisualizationFormat::TEXT);
    ...
    ss << "\n===== Optimization Recommendations =====\n";
    for (const auto& rec : recommendations) {
        ss << rec.description << " (Improvement: " << (rec.estimated_improvement * 100.0) << "%)\n";
    }
...
std::string ResourceProcessor::generateDetailedReport() const {
    ...
    ss << visualizer_->generateDetailedReport(chronovyan::VisualizationFormat::TEXT,
                                              20);  // Default history length
    ...
    if (!debts.empty()) {
        ...
        for (size_t i = 0; i < debts.size(); ++i) {
            ...
            ss << "  Interest Rate: " << (std::get<double>(debt.at("interest_rate")) * 100.0)
               << "%\n";
            ss << "  Due Cycle: " << std::get<int>(debt.at("due_cycle")) << "\n";
            ss << "  Critical: " << (std::get<bool>(debt.at("critical")) ? "Yes" : "No") << "\n";
        }
    }
...
bool ResourceProcessor::saveData(const std::string& filename) const { return saveToLog(filename); }

bool ResourceProcessor::loadData(const std::string& filename) { return loadFromLog(filename); }

void ResourceProcessor::updateComponents() {
    // This is a placeholder for future functionality
}

bool ResourceProcessor::checkResourceThresholds(double chrononUsage, double aethelUsage,
    ...
    if (chrononUsage > chronon_threshold_ || aethelUsage > aethel_threshold_ ||
        temporalDebt > temporal_debt_threshold_ || paradoxRisk > paradox_risk_threshold_) {
        return true;
    }
    return false;
}
