...
TemporalSequence::TemporalSequence(std::shared_ptr<TemporalRuntime> runtime)
    : m_runtime(runtime) {
...
bool TemporalSequence::hasCycle(int cycle) const {
  return findIndexByCycle(cycle) != -1;
}
...
std::shared_ptr<TemporalSequence> TemporalSequence::slice(int startCycle,
                                                          int endCycle) const {
  if (startCycle > endCycle) {
    throw std::invalid_argument(
        "Start cycle must be less than or equal to end cycle");
  }
...
Value TemporalSequence::interpolate(int cycle,
  ...
  size_t afterIndex = 0;
  while (afterIndex < m_timePoints.size() &&
         m_timePoints[afterIndex].cycle < cycle) {
    afterIndex++;
  }
  ...
  if (method == "step") {
    ...
    if (before.value.getType() == Value::Type::NUMBER &&
      ...
      double ratio = static_cast<double>(cycle - before.cycle) /
...
std::shared_ptr<TemporalSequence>
TemporalSequence::resample(int interval, const std::string &method) const {
...
std::vector<std::shared_ptr<TemporalSequence>>
TemporalSequence::detectPatterns() const {
  ...
  // In a real implementation, this would use autocorrelation, Fourier analysis,
  // etc.

  // For now, just return the full sequence as a "pattern"
  patterns.push_back(
      std::make_shared<TemporalSequence>(m_timePoints, m_runtime));
...
std::shared_ptr<TemporalSequence> TemporalSequence::project(int cycles) const {
  ...
  std::vector<TimePoint> pattern(m_timePoints.end() - patternSize,
                                 m_timePoints.end());
  ...
  for (int i = 0; i < cycles; i++) {
    for (const auto &point : pattern) {
      int newCycle =
          lastCycle + (point.cycle - pattern.front().cycle) + i * patternLength;
...
std::shared_ptr<TemporalSequence>
TemporalSequence::map(std::function<Value(int, const Value &)> func) const {
...
// Filter sequence
std::shared_ptr<TemporalSequence>
TemporalSequence::filter(std::function<bool(int, const Value &)> pred) const {
...
Value TemporalSequence::reduce(
    std::function<Value(const Value &, int, const Value &)> func,
...
std::map<std::string, Value> TemporalSequence::statistics() const {
  ...
  stats["cycle_range"] =
      Value(static_cast<int64_t>(getLatestCycle() - getEarliestCycle()));
...
std::shared_ptr<TemporalSequence>
TemporalSequence::merge(const TemporalSequence &other,
...
void TemporalSequence::setTemporalRuntime(
    std::shared_ptr<TemporalRuntime> runtime) {
  m_runtime = runtime;
}

// Get temporal runtime
std::shared_ptr<TemporalRuntime> TemporalSequence::getTemporalRuntime() const {
  return m_runtime;
}
...
int64_t TemporalSequence::getChrononsForOperation(const std::string &operation,
  ...
  // Scaling factor based on sequence size (logarithmic to be efficient for
  // large sequences)
...
void TemporalSequence::ensureSorted() {
  std::sort(m_timePoints.begin(), m_timePoints.end());
}
...
int TemporalSequence::findIndexByCycle(int cycle) const {
  ...
  auto it = std::lower_bound(
      m_timePoints.begin(), m_timePoints.end(), TimePoint(cycle, Value()),
...
void registerTemporalSequenceType(CustomTypeSystem &typeSystem) {
  ...
  auto seqType = std::make_shared<CustomTypeDefinition>(
      "TEMPORAL_SEQUENCE", CustomTypeKind::STRUCT, props, params);
