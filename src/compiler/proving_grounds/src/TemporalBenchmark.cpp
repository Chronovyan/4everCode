...
void TemporalBenchmark::markOperationEnd(const std::string& operationName) {
    if (isMeasuring && operationStartTimes.find(operationName) != operationStartTimes.end()) {
        auto endTime = std::chrono::high_resolution_clock::now();
...
double TemporalBenchmark::getOperationDuration(const std::string& operationName) const {
...
std::map<std::string, double> TemporalBenchmark::getAllOperationDurations() const {
    return operationDurations;
}

void TemporalBenchmark::recordResourceUsage(const std::string& resourceName, double usage) {
    resourceUsages[resourceName] = usage;
}

double TemporalBenchmark::getResourceUsage(const std::string& resourceName) const {
...
double TemporalBenchmark::calculateEfficiency(const std::string& operationName,
...
std::map<std::string, double> TemporalBenchmark::calculateAllEfficiencies(
    ...
    for (const auto& [resourceName, usage] : resourceUsages) {
        efficiencies[resourceName] = calculateEfficiency(operationName, resourceName);
    }

    return efficiencies;
}

TemporalBenchmark::BenchmarkResult TemporalBenchmark::getBenchmarkResult(
    const std::string& operationName) const {
...
std::string TemporalBenchmark::generateReport(const std::string& format) const {
    ...
    if (format == "json") {
        ...
        for (const auto& [operationName, duration] : operationDurations) {
            report << "    <tr><td>" << operationName << "</td><td>" << duration << "</td></tr>\n";
        }
        report << "  </table>\n";
        ...
        for (const auto& [resourceName, usage] : resourceUsages) {
            report << "    <tr><td>" << resourceName << "</td><td>" << usage << "</td></tr>\n";
        }
        report << "  </table>\n";
        ...
        report << "  <table>\n";
        report << "    <tr><th>Operation</th><th>Resource</th><th>Efficiency</th></tr>\n";
        for (const auto& [operationName, duration] : operationDurations) {
            const auto& efficiencies = calculateAllEfficiencies(operationName);
            for (const auto& [resourceName, efficiency] : efficiencies) {
...
bool TemporalBenchmark::saveReport(const std::string& report, const std::string& filePath) const {
