...
void Lexer::debugPrint(const std::string& message) {
    if (diagnosticReporter && diagnosticReporter->getDebugOutput()) {
        diagnosticReporter->addInfo(SourceLocation(filename, line, column), message);
    }
}
...
Token Lexer::handleCharacter() {
    ...
    debugPrint("Found opening quote at position " + std::to_string(tokenStart) + ", line " +
               std::to_string(tokenStartLine) + ", column " + std::to_string(tokenStartColumn));

    // Debug log
    if (diagnosticReporter && diagnosticReporter->getDebugOutput()) {
        std::string debugMsg = "handleCharacter: Starting at position " + std::to_string(position) +
                               ", char='" + (isAtEnd(0) ? "<EOF>" : escapeChar(peek(0))) + "'";
        ...
        diagnosticReporter->addInfo(SourceLocation(filename, tokenStartLine, tokenStartColumn),
                                    debugMsg);
    }
    ...
    debugPrint("After skipping opening quote, position: " + std::to_string(position) +
    ...
    if (peek() == '\\') {
        ...
        switch (escapeChar) {
            ...
            case 'x': {
                ...
                debugPrint("Hex escape sequence value: " + std::to_string(static_cast<int>(c)));
                break;
            }
            default:
                debugPrint("Error: Unrecognized escape sequence: \\" + std::string(1, escapeChar));
                return errorToken("Unrecognized escape sequence: \\" + std::string(1, escapeChar));
        }
    ...
    // Create the token
    std::string lexeme = source.substr(static_cast<size_t>(tokenStart), position - tokenStart);

    debugPrint("Successfully parsed character literal: " + lexeme + ", value: '" + escapeChar(c) +
               "' (" + std::to_string(static_cast<int>(c)) + ")");
...
bool Lexer::isAtEnd(int offset) const {
    ...
    size_t newPosition = static_cast<size_t>(position) + static_cast<size_t>(offset);
...
Token Lexer::makeToken(TokenType type, Token::ValueType value, const std::string& lexeme) {
    SourceLocation location(filename, tokenStartLine, tokenStartColumn);
    return Token(type, lexeme, location, value);
}
...
std::vector<Token> Lexer::tokenize() {
    ...
    while (true) {
        ...
        if (token.getType() == TokenType::END_OF_FILE || token.getType() == TokenType::ERROR) {
            break;
        }
    }
