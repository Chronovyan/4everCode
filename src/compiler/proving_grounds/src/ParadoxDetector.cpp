...
const std::string RULE_VARIABLE_STATE_INCONSISTENCIES =
    "variable_state_inconsistencies";
...
void ParadoxDetector::setSeverityThreshold(int threshold) {
  if (threshold >= 1 && threshold <= 10) {
    ...
    diagnostics.addWarning(SourceLocation(), "Invalid severity threshold: " +
...
void ParadoxDetector::detectResourceViolations(
    const debugger::DejaVuDebugger &debugger,
  ...
  if (chrononsIt != resourceUsage.end()) {
    ...
    if (chronons > 1000.0) {
      ...
      paradoxes.push_back({"Excessive Chronon usage detected: " +
                               std::to_string(chronons) + " chronons",
                           8, // High severity
                           debugger.getCurrentState().getSourceLocation()});
    } else if (chronons > 500.0) {
      // High chronon usage is a moderate paradox
      paradoxes.push_back({"High Chronon usage detected: " +
                               std::to_string(chronons) + " chronons",
                           5, // Moderate severity
                           debugger.getCurrentState().getSourceLocation()});
    }
  ...
  if (aethelIt != resourceUsage.end()) {
    ...
    if (aethel < 0.0) {
      ...
      paradoxes.push_back(
          {"Negative Aethel detected: " + std::to_string(aethel) + " aethel",
           9, // Very high severity
           debugger.getCurrentState().getSourceLocation()});
    } else if (aethel > 1000.0) {
      // Very high Aethel usage is a moderate paradox
      paradoxes.push_back({"Excessive Aethel usage detected: " +
                               std::to_string(aethel) + " aethel",
  ...
  if (debtIt != resourceUsage.end()) {
    ...
    if (debt > 100.0) {
      ...
      paradoxes.push_back(
          {"Critical temporal debt detected: " + std::to_string(debt),
           10, // Maximum severity
           debugger.getCurrentState().getSourceLocation()});
    } else if (debt > 50.0) {
      // Moderate temporal debt is a moderate paradox
      paradoxes.push_back(
          {"Significant temporal debt detected: " + std::to_string(debt),
           7, // High severity
           debugger.getCurrentState().getSourceLocation()});
    }
  }
...
void ParadoxDetector::detectTimelineInconsistencies(
    const debugger::DejaVuDebugger &debugger,
    std::vector<Paradox> &paradoxes) const {
...
void ParadoxDetector::detectCausalityViolations(
    const debugger::DejaVuDebugger &debugger,
    std::vector<Paradox> &paradoxes) const {
  ...
  // We can check if the current state is explicitly marked as a potential
  // paradox
...
void ParadoxDetector::detectVariableStateInconsistencies(
    const debugger::DejaVuDebugger &debugger,
    std::vector<Paradox> &paradoxes) const {
...
void ParadoxDetector::detectThreadSynchronizationParadoxes(
    const debugger::DejaVuDebugger &debugger,
    std::vector<Paradox> &paradoxes) const {
