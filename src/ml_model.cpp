...
MLModel::MLModel(const std::string &model_type,
                 const std::vector<std::string> &feature_columns,
                 double learning_rate, std::optional<unsigned int> seed)
    : model_type(model_type), feature_columns(feature_columns),
...
void MLModel::update_hyperparameters(const OptimizationMetrics &metrics) {
  ...
  double coherence_factor =
      1.0 - metrics.error_rate / 100.0; // Convert error rate to coherence score
  ...
  hyperparameters["learning_rate"] =
      std::clamp(hyperparameters["learning_rate"], 0.001, 0.1);
  hyperparameters["regularization"] =
      std::clamp(hyperparameters["regularization"], 0.001, 0.1);
  hyperparameters["momentum"] =
      std::clamp(hyperparameters["momentum"], 0.5, 0.99);
}
...
void MLModel::train_model(const std::vector<double> &features) {
  ...
  for (size_t i = 0; i < features.size(); ++i) {
    ...
    // This ensures that features with higher values (like 1.0) get higher
    // weights
...
void MLModel::update_feature_importance() {
  ...
  for (size_t i = 0; i < feature_weights.size(); ++i) {
    ...
    // importance
    weight_magnitudes[i] =
        std::abs(feature_weights[i]) * (1.0 + std::abs(feature_weights[i]));
  ...
  if (total_weight > 0.0) {
    ...
    for (size_t i = 0; i < feature_weights.size(); ++i) {
      ...
      double feature_value_importance =
          std::abs(feature_weights[i]) /
          (std::abs(feature_weights[i]) +
           1.0); // Add 1.0 to prevent division by zero

      // Adjust the importance calculation to put more emphasis on the weight
      // magnitude
      feature_importance[i] =
          0.8 * weight_importance + 0.2 * feature_value_importance;
    }
  }
}
