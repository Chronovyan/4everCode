...
OptimizationManager::OptimizationManager() : level(OptimizationLevel::STANDARD) {}
...
void OptimizationManager::registerTemporalPass(std::unique_ptr<OptimizationPass> pass,
                                               OptimizationLevel minLevel) {
    std::string passName = pass->getName();
...
void OptimizationManager::setOptimizationLevel(OptimizationLevel newLevel) { level = newLevel; }

OptimizationLevel OptimizationManager::getOptimizationLevel() const { return level; }

bool OptimizationManager::setPassEnabled(const std::string& passName, bool enabled) {
    auto it = passIndices.find(passName);
    if (it != passIndices.end()) {
...
bool OptimizationManager::optimize(AstNode* node, DiagnosticReporter& diagnostics) {
    if (!node) {
        return false;
    }
        ...
        if (shouldRunPass(passInfo)) {
            try {
                ...
                if (passResult) {
                    ...
                    std::string message = "Applied optimization pass: " + passInfo.pass->getName();
