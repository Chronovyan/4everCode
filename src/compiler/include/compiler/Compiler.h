...
public:
     ...
     * @param outputPath The path to the output file. If empty, uses <inputPath>.c or
     * <inputPath>.chron.
    ...
    bool compile(const std::filesystem::path& inputPath,
                 const std::filesystem::path& outputPath = {}, bool generateChron = false);
     ...
     * @return True if compilation was successful, false otherwise.
     */
    bool compileString(const std::string& sourceCode, const std::filesystem::path& outputPath,
...
private:
     ...
     */
    bool generateChronFile(std::unique_ptr<AstNode>& ast, const std::filesystem::path& outputPath);
     ...
     */
    bool saveToFile(const std::string& generatedCode, const std::filesystem::path& outputPath);
