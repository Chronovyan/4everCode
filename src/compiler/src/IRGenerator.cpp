...
void IRGenerator::visitLiteralExprNode(LiteralExprNode* node) {
    ...
    value = std::visit(
        overloaded{[](int i) -> IRValue { return IRValue::createInteger(i); },
                   ...
                   [](const std::string& s) -> IRValue { return IRValue::createString(s); },
...
void IRGenerator::visitCallExprNode(CallExprNode* node) {
    ...
    if (auto varExpr = dynamic_cast<VariableExprNode*>(&node->getCallee())) {
        ...
        // Handle other types of callees (e.g., member access, function pointer, etc.)
        diagnostics.addError(node->getLocation(), "Unsupported callee type in function call");
        hasError = true;
        return;
    }
...
void IRGenerator::visitVarDeclNode(VarDeclNode* node) {
    ...
    if (node->getInitializer()) {
        ...
        addInstruction(IROpCode::LOAD_CONST, IRValue::createVoid(), node->getLocation());
    }
...
void IRGenerator::visitIfStmtNode(IfStmtNode* node) {
    ...
    addInstruction(IROpCode::JUMP_IF_FALSE, IRValue::createString(elseLabel), node->getLocation());

    // Generate code for the then branch
    node->getThenBranch().accept(*this);

    // Jump to the end of the if statement
    addInstruction(IROpCode::JUMP, IRValue::createString(endLabel), node->getLocation());
...
void IRGenerator::visitWhileStmtNode(WhileStmtNode* node) {
    ...
    addInstruction(IROpCode::JUMP_IF_TRUE, IRValue::createString(bodyLabel), node->getLocation());
    ...
    addInstruction(IROpCode::JUMP, IRValue::createString(condLabel), node->getLocation());
...
void IRGenerator::visitResourceStmtNode(ResourceStmtNode* node) {
    ...
    if (node->getChronons()) {
        ...
        addInstruction(IROpCode::LOAD_CONST, IRValue::createFloat(0.0), node->getLocation());
    }
...
void IRGenerator::visitParallelExecutionStmtNode(ParallelExecutionStmtNode* node) {
...
void IRGenerator::visitStmtDeclNode(StmtDeclNode* node) {
    ...
    // This is a wrapper around a statement that needs to be treated as a declaration
...
IRBasicBlock& IRGenerator::createBasicBlock(const std::string& label) {
    ...
    if (!currentFunction) {
        ...
        diagnostics.addError(SourceLocation(), "Cannot create basic block outside of a function");
        hasError = true;
    ...
    return currentFunction->getBasicBlock(currentFunction->getBasicBlocks().size() - 1);
}
...
void IRGenerator::addInstruction(IROpCode opcode, const SourceLocation& location) {
    // Add an instruction to the current basic block
    if (!currentBlock) {
        // This should not happen
        diagnostics.addError(location, "Cannot add instruction outside of a basic block");
        hasError = true;
        return;
    }
...
void IRGenerator::addInstruction(IROpCode opcode, const IRValue& operand1, const IRValue& operand2,
                                 const SourceLocation& location) {
...
void IRGenerator::addInstruction(IROpCode opcode, const IRValue& operand1, const IRValue& operand2,
                                 const IRValue& operand3, const SourceLocation& location) {
