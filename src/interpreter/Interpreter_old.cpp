...
Interpreter::Interpreter()
    : m_globals(std::make_shared<Environment>(nullptr)),
      m_environment(m_globals), m_runtime(std::make_shared<TemporalRuntime>()),
  ...
  m_resourceManager =
      std::make_unique<ResourceManager>(m_runtime, m_optimizer, m_debt_tracker);
...
bool Interpreter::consumeResources(double amount) {
  ...
  // Delegate to resource manager
  return m_resourceManager->consumeResources(aethelAmount, chrononsAmount,
                                             "Generic operation");
}

Value Interpreter::handleVariableInteraction(const Value &left,
                                             const Value &right,
                                             TokenType operation) {
...
void Interpreter::updateParadoxLevel(const Value &left, const Value &right,
                                     TokenType operation) {
...
void Interpreter::visitIfStmt(const IfStmtNode &stmt) {
  m_statementVisitor->visitIfStmt(stmt);
}
