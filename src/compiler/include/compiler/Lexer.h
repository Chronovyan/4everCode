...
/**
 * Lexer is responsible for converting a string of source code into a sequence of tokens.
 */
class Lexer {
public:
    /**
     * Constructs a lexer with the given source code, filename, and diagnostic reporter.
     *
    ...
    /**
     * Returns the next token in the source code without advancing the current position.
     *
     * @return The next token
     */
    Token peekToken();
     ...
     * @return The diagnostic reporter
     */
    const DiagnosticReporter& getDiagnosticReporter() const { return *diagnosticReporter; }
...
private:
    ...
    Token makeToken(TokenType type, Token::ValueType value, const std::string& lexeme);
