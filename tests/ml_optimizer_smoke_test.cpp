...
protected:
    void SetUp() override {
        ...
        std::vector<std::string> features = {"latency", "throughput", "error_rate",
                                             "resource_usage"};
...
TEST_F(MLOptimizerSmokeTest, MLModelNumericalStability) {
    ...
    std::vector<double> nan_features = {std::numeric_limits<double>::quiet_NaN(), 0.0, 0.0, 0.0};
...
TEST_F(MLOptimizerSmokeTest, MLModelHyperparameterBounds) {
    ...
    OptimizationMetrics extreme_metrics(std::numeric_limits<double>::max(),  // sync_efficiency
                                        0.0,                                 // stability_score
