...
ExpressionVisitor::ExpressionVisitor(Interpreter &interpreter)
...
void ExpressionVisitor::initializeBinaryOps() {
  ...
  m_binaryOps[TokenType::PLUS] = [this](const Value &left,
    ...
    if ((left.isNumber() || left.isInteger()) &&
        (right.isNumber() || right.isInteger())) {
  ...
  m_binaryOps[TokenType::MINUS] = [this](const Value &left,
                                         const Value &right) -> Value {
    if ((left.isNumber() || left.isInteger()) &&
        (right.isNumber() || right.isInteger())) {
      // Check if either is a float
    ...
    throw RuntimeError("Operands must be numbers for subtraction.",
                       left.getSourceLocation());
  };

  m_binaryOps[TokenType::STAR] = [this](const Value &left,
                                        const Value &right) -> Value {
    if ((left.isNumber() || left.isInteger()) &&
        (right.isNumber() || right.isInteger())) {
      // Check if either is a float
  ...
  m_binaryOps[TokenType::SLASH] = [this](const Value &left,
                                         const Value &right) -> Value {
    if ((left.isNumber() || left.isInteger()) &&
        (right.isNumber() || right.isInteger())) {
    ...
    throw RuntimeError("Operands must be numbers for division.",
                       left.getSourceLocation());
  };

  // Comparison operations
  m_binaryOps[TokenType::GREATER] = [this](const Value &left,
                                           const Value &right) -> Value {
    if ((left.isNumber() || left.isInteger()) &&
        (right.isNumber() || right.isInteger())) {
      return Value(getNumericValue(left) > getNumericValue(right));
  ...
  m_binaryOps[TokenType::GREATER_EQUAL] = [this](const Value &left,
                                                 const Value &right) -> Value {
    if ((left.isNumber() || left.isInteger()) &&
        (right.isNumber() || right.isInteger())) {
      return Value(getNumericValue(left) >= getNumericValue(right));
    ...
    throw RuntimeError(
        "Operands must be comparable types for greater than or equal.",
        left.getSourceLocation());
  };

  m_binaryOps[TokenType::LESS] = [this](const Value &left,
                                        const Value &right) -> Value {
    if ((left.isNumber() || left.isInteger()) &&
        (right.isNumber() || right.isInteger())) {
      return Value(getNumericValue(left) < getNumericValue(right));
  ...
  m_binaryOps[TokenType::LESS_EQUAL] = [this](const Value &left,
                                              const Value &right) -> Value {
    if ((left.isNumber() || left.isInteger()) &&
        (right.isNumber() || right.isInteger())) {
      return Value(getNumericValue(left) <= getNumericValue(right));
    ...
    throw RuntimeError(
        "Operands must be comparable types for less than or equal.",
        left.getSourceLocation());
  };

  // Equality operations
  m_binaryOps[TokenType::EQUAL_EQUAL] = [](const Value &left,
                                           const Value &right) -> Value {
    return Value(left == right);
  };

  m_binaryOps[TokenType::BANG_EQUAL] = [](const Value &left,
                                          const Value &right) -> Value {
    return Value(left != right);
  };

  // Logical operations (implemented in visitBinaryExpr for short-circuit
  // behavior)
}
...
void ExpressionVisitor::visitUnaryExpr(const UnaryExprNode &expr) {
  ...
  case TokenType::MINUS:
    ...
    if (right.isNumber()) {
      ...
      throw RuntimeError("Operand must be a number for unary minus.",
                         expr.getLocation());
    }
    break;
  ...
  case TokenType::PLUS_PLUS:
    ...
    if (right.isNumber()) {
      ...
      throw RuntimeError("Operand must be a number for increment.",
                         expr.getLocation());
    }
    ...
    if (expr.getRight().isVariable()) {
      const auto &varExpr =
          static_cast<const VariableExprNode &>(expr.getRight());
  ...
  case TokenType::MINUS_MINUS:
    ...
    if (right.isNumber()) {
      ...
      throw RuntimeError("Operand must be a number for decrement.",
                         expr.getLocation());
    }
...
void ExpressionVisitor::visitCallExpr(const CallExprNode &expr) {
  ...
  if (callable->arity() != -1 && arguments.size() != callable->arity()) {
    throw RuntimeError("Expected " + std::to_string(callable->arity()) +
                           " arguments but got " +
...
Value ExpressionVisitor::lookUpVariable(const std::string &name,
...
Value ExpressionVisitor::handleVariableInteraction(const Value &left,
                                                   const Value &right,
                                                   TokenType operation) {
...
void ExpressionVisitor::updateParadoxLevel(const Value &left,
                                           const Value &right,
                                           TokenType operation) {
  // Delegate to the interpreter's implementation
