...
bool Parser::isAtEnd() const {
  return current_.getType() == TokenType::END_OF_FILE;
}
...
std::unique_ptr<DeclNode> Parser::parseDeclaration() {
  if (match(TokenType::LET) || match(TokenType::CONST)) {
    try {
      ...
      reporter_.addError(
          current_, std::string("Error in variable declaration: ") + e.what());
      synchronize();
      return nullptr;
    }
  }
...
std::unique_ptr<StmtNode> Parser::parseStatement() {
  try {
    ...
    if (match({TokenType::BRANCH_TIMELINE, TokenType::MERGE_TIMELINES,
               TokenType::REWIND_FLOW, TokenType::REVERSE_FLOW})) {
    ...
    reporter_.addError(current_,
                       std::string("Error in statement: ") + e.what());
    synchronize();
    return nullptr;
  }
}
...
std::unique_ptr<StmtNode> Parser::parseBlockStatement() {
  ...
  return std::make_unique<BlockStmtNode>(std::move(leftBrace),
                                         std::move(statements));
}
...
std::unique_ptr<StmtNode> Parser::parseIfStatement() {
  ...
  if (!thenBranch) {
    ...
    lexer.getDiagnosticReporter().addError(
        currentToken, "Invalid or missing then branch in if statement");
...
std::unique_ptr<StmtNode> Parser::parseWhileStatement() {
  ...
  if (!condition) {
    // If condition parsing failed, try to recover
    lexer.getDiagnosticReporter().addError(
        currentToken, "Invalid or missing condition in while statement");
    condition = std::make_unique<LiteralExprNode>(
  ...
  if (!body) {
    ...
    lexer.getDiagnosticReporter().addError(
        currentToken, "Invalid or missing body in while statement");
    body = std::make_unique<BlockStmtNode>(
        Token(TokenType::LEFT_BRACE, "{", whileToken.getLocation()),
        std::vector<std::unique_ptr<StmtNode>>());
  }

  return std::make_unique<WhileStmtNode>(std::move(whileToken),
...
std::unique_ptr<StmtNode> Parser::parseResourceStatement() {
  ...
  if (!check(TokenType::LEFT_BRACE)) {
    lexer.getDiagnosticReporter().addError(
        currentToken, "Expect '{' after resource statement");
    synchronize();
    return nullptr;
  }
  ...
  if (!check(TokenType::RIGHT_BRACE)) {
    lexer.getDiagnosticReporter().addError(
        currentToken, "Expect '}' after resource specifications");
  } else {
    advance(); // Consume the right brace
  }
  ...
  if (!check(TokenType::LEFT_BRACE)) {
    ...
    return std::make_unique<ResourceStmtNode>(
        std::move(keyword), operation, std::move(chronons), std::move(aethel),
        std::move(emptyBody));
  }
...
std::unique_ptr<StmtNode> Parser::parseTemporalStatement() {
  // This would implement temporal statements like BRANCH_TIMELINE,
  // MERGE_TIMELINES, etc. For now, we'll return a placeholder
  return nullptr;
}
...
std::unique_ptr<ExprNode> Parser::parseAssignment() {
  ...
  if (match(TokenType::EQUAL)) {
    ...
    if (!value) {
      ...
      lexer.getDiagnosticReporter().addError(
          currentToken,
          "Invalid or missing expression on right side of assignment");
    ...
    lexer.getDiagnosticReporter().addError(
        currentToken,
        "Invalid assignment target. Left side must be a variable.");
  }
...
std::unique_ptr<ExprNode> Parser::parseLogicalOr() {
  ...
  while (match(TokenType::OR)) {
    ...
    expr = std::make_unique<BinaryExprNode>(std::move(expr), std::move(op),
                                            std::move(right));
  }
...
std::unique_ptr<ExprNode> Parser::parsePrimary() {
  if (match(TokenType::TRUE) || match(TokenType::FALSE) ||
      match(TokenType::INTEGER) || match(TokenType::FLOAT) ||
      match(TokenType::STRING) || match(TokenType::CHRONON_LITERAL) ||
