...
void Parser::consume(TokenType type, const std::string& message) {
    ...
    if (type == TokenType::SEMICOLON) {
        ErrorHandler::getInstance().reportWarning(
            m_current.location, "Missing semicolon; automatic semicolon insertion applied.");
        return;
    }
...
std::unique_ptr<ProgramNode> Parser::parse() {
    ...
    try {
        ...
        ErrorHandler::getInstance().reportError(
            m_current.location, std::string("Unrecoverable parser error: ") + e.what());
    }
...
std::unique_ptr<StmtNode> Parser::parseExpressionStatement() {
    // Special case for handling keywords as identifiers at the start of an expression statement
    if (check(TokenType::IDENTIFIER)) {
        if (m_current.lexeme == "DEFINE_TYPE") {
            advance();  // Consume "DEFINE_TYPE"
            auto typeDef = parseTypeDefinition();
    ...
    // Consume semicolon - this will now issue a warning if missing but won't throw
...
std::unique_ptr<ExprNode> Parser::parsePrimary() {
    ...
    if (match(TokenType::INTEGER_LITERAL)) {
        return std::make_unique<LiteralExprNode>(
            m_previous.location, static_cast<int64_t>(std::stoll(m_previous.lexeme)));
    }
    if (match(TokenType::FLOAT_LITERAL)) {
        return std::make_unique<LiteralExprNode>(m_previous.location, std::stod(m_previous.lexeme));
    }

    // Check for string literals
    if (match(TokenType::STRING_LITERAL)) {
        return std::make_unique<LiteralExprNode>(m_previous.location, m_previous.lexeme);
    }

    // Handle identifiers
    if (check(TokenType::IDENTIFIER)) {
        // Check if the current token is 'DEFINE_TYPE', which should be handled at the statement
        // level
        if (m_current.lexeme == "DEFINE_TYPE") {
            ErrorHandler::getInstance().reportError(
                m_current.location,
        ...
        // Check for special case of "var" which should be a declaration, not an expression
        if (m_current.lexeme == "var") {
            ErrorHandler::getInstance().reportError(m_current.location,
                                                    "Unexpected 'var' in expression context. 'var' "
        ...
        advance();
        return std::make_unique<VariableExprNode>(m_previous.location, m_previous.lexeme);
    }
    ...
    // Error case
    ErrorHandler::getInstance().reportError(m_current.location,
                                            "Expect expression. Got '" + m_current.lexeme + "'.");
    throw std::runtime_error("Parse error");
}
...
LootTable Parser::parseLootTable() {
    ...
    while (!check(TokenType::RIGHT_BRACE) && !check(TokenType::EOF_TOKEN)) {
        ...
        if (!check(TokenType::STRING_LITERAL)) {
            ErrorHandler::getInstance().reportError(m_current.location,
                                                    "Expected loot ID as string literal.");
            break;
        }
        ...
        if (!check(TokenType::FLOAT_LITERAL) && !check(TokenType::INTEGER_LITERAL)) {
            ErrorHandler::getInstance().reportError(m_current.location,
                                                    "Expected probability as number.");
            break;
        }
...
std::unique_ptr<TemporalProgramNode> Parser::parseTemporalProgram() {
    ...
    while (!check(TokenType::RIGHT_BRACE) && !check(TokenType::EOF_TOKEN)) {
        ...
        if (match(TokenType::NAME)) {
            ...
            auto section = parseMetadataSection(TemporalSectionType::RESOURCES_REQUIRED);
            sections[TemporalSectionType::RESOURCES_REQUIRED] = std::move(section);
        } else if (match(TokenType::TIMELINE_BEHAVIOR)) {
            auto section = parseMetadataSection(TemporalSectionType::TIMELINE_BEHAVIOR);
            sections[TemporalSectionType::TIMELINE_BEHAVIOR] = std::move(section);
        } else if (match(TokenType::STABILITY_TARGET)) {
            auto section = parseMetadataSection(TemporalSectionType::STABILITY_TARGET);
            ...
            auto section = parseMetadataSection(TemporalSectionType::PARADOX_HANDLING);
    ...
    consume(TokenType::RIGHT_BRACE, "Expected '}' to close temporal_program block.");
    ...
    if (!hasInitialization) {
        ErrorHandler::getInstance().reportWarning(
            location, "Missing 'initialization_phase' section in temporal_program block.");
    }
    if (!hasExecution) {
        ErrorHandler::getInstance().reportError(
            location, "Missing required 'execution_weave' section in temporal_program block.");
...
std::unique_ptr<TemporalSectionNode> Parser::parseMetadataSection(TemporalSectionType sectionType) {
    ...
    if (check(TokenType::STRING_LITERAL)) {
        ...
        if (sectionType == TemporalSectionType::VERSION &&
            ...
            if (m_previous.type == TokenType::INTEGER_LITERAL) {
                ...
                content = std::make_unique<LiteralExprNode>(location,
                                                            LiteralExprNode::LiteralValue(value));
            } else {
            ...
            ErrorHandler::getInstance().reportError(
                m_current.location,
                "Expected string, identifier, or numeric literal in metadata section.");
            throw std::runtime_error("Parse error");
        }
    }
    ...
    return std::make_unique<TemporalSectionNode>(location, sectionType, std::move(content));
}
...
std::unique_ptr<TemporalSectionNode> Parser::parseVariablesSection() {
    ...
    while (!check(TokenType::RIGHT_BRACE) && !check(TokenType::EOF_TOKEN)) {
        ...
        if (match(TokenType::CONF)) {
            if (match(TokenType::VAR)) {
                declarations.push_back(parseModifiedVarDeclaration(VariableModifier::CONF));
            } else {
            ...
            if (match(TokenType::VAR)) {
                declarations.push_back(parseModifiedVarDeclaration(VariableModifier::REB));
            } else {
    ...
    // Create and return the TemporalSectionNode
    return std::make_unique<TemporalSectionNode>(location, TemporalSectionType::VARIABLES,
                                                 std::move(block));
}
...
std::unique_ptr<TemporalSectionNode> Parser::parseInitializationSection() {
    ...
    // Consume closing brace
    consume(TokenType::RIGHT_BRACE, "Expected '}' to close initialization_phase section.");
...
std::unique_ptr<TemporalSectionNode> Parser::parseExecutionWeaveSection() {
    ...
    // Consume closing brace
    consume(TokenType::RIGHT_BRACE, "Expected '}' to close execution_weave section.");

    // Create a block statement for the statements
    auto block = std::make_unique<BlockStmtNode>(std::move(statements));

    // Create and return the TemporalSectionNode
    return std::make_unique<TemporalSectionNode>(location, TemporalSectionType::EXECUTION_WEAVE,
                                                 std::move(block));
}
...
std::unique_ptr<TemporalSectionNode> Parser::parseCleanupSection() {
    ...
    // Consume closing brace
    consume(TokenType::RIGHT_BRACE, "Expected '}' to close cleanup_phase section.");

    // Create a block statement for the statements
    auto block = std::make_unique<BlockStmtNode>(std::move(statements));

    // Create and return the TemporalSectionNode
    return std::make_unique<TemporalSectionNode>(location, TemporalSectionType::CLEANUP_PHASE,
                                                 std::move(block));
}
...
std::unique_ptr<VariableDeclStmtNode> Parser::parseVarDeclaration() {
    ...
    if (match(TokenType::COLON)) {
        ...
        if (sharedType) {
            type = std::make_unique<TypeNode>(sharedType->getLocation(), sharedType->getName());
        }
    }
    ...
    return std::make_unique<VariableDeclStmtNode>(
        ...
        VariableModifier::CONF,  // Use CONF as the default modifier since NONE doesn't exist
        std::vector<VariableFlag>(), std::move(initializer));
}

std::unique_ptr<VariableDeclStmtNode> Parser::parseModifiedVarDeclaration(
    ...
    return std::make_unique<VariableDeclStmtNode>(location, name, std::move(type), modifier,
                                                  std::move(flags), std::move(initializer));
}
...
std::unique_ptr<DeclareConstantNode> Parser::parseDeclareConstant() {
    ...
    return std::make_unique<DeclareConstantNode>(location, name, std::move(type), std::move(flags),
...
std::vector<VariableFlag> Parser::parseVariableFlags() {
    ...
    while (match(TokenType::DOUBLE_COLON)) {
        // Expect a flag token
        if (check(TokenType::STATIC) || check(TokenType::VOLATILE) || check(TokenType::ANCHOR) ||
            check(TokenType::WEAVER) || check(TokenType::FLUX) || check(TokenType::ECHO)) {
            advance();
            flags.push_back(tokenToVariableFlag(m_previous));
        } else {
            // Unexpected token after ::
            ErrorHandler::getInstance().reportError(m_current.location,
                                                    "Expected variable flag after '::'.");
            throw std::runtime_error("Parse error");
        }
    }
...
std::unique_ptr<ForChrononStmtNode> Parser::parseForChrononLoop() {
    ...
    if (m_previous.lexeme != "to") {
        ErrorHandler::getInstance().reportError(m_previous.location,
                                                "Expected 'to' after range start expression.");
        throw std::runtime_error("Parse error");
    }
    ...
    consume(TokenType::RIGHT_PAREN, "Expected ')' after FOR_CHRONON loop parameters.");
    ...
    return std::make_unique<ForChrononStmtNode>(location, iterator, std::move(rangeStart),
                                                std::move(rangeEnd), std::move(stepExpr),
                                                std::move(body));
}
...
std::unique_ptr<WhileEventStmtNode> Parser::parseWhileEventLoop() {
    ...
    return std::make_unique<WhileEventStmtNode>(location, std::move(condition), std::move(body));
}
...
std::unique_ptr<RewindFlowStmtNode> Parser::parseRewindFlowLoop() {
    ...
    return std::make_unique<RewindFlowStmtNode>(location, std::move(condition),
                                                std::move(maxRewinds), std::move(body));
}
...
std::unique_ptr<AttemptWeaveStmtNode> Parser::parseAttemptWeave() {
    ...
    return std::make_unique<AttemptWeaveStmtNode>(location, std::move(tryBlock),
                                                  std::move(handlers), std::move(finallyBlock));
}
...
std::unique_ptr<DissonanceHandlerNode> Parser::parseDissonanceHandler() {
    ...
    if (check(TokenType::IDENTIFIER) || check(TokenType::INT) || check(TokenType::FLOAT) ||
        check(TokenType::BOOLEAN) || check(TokenType::STRING) || check(TokenType::CHRONON) ||
        check(TokenType::EVENT) || check(TokenType::ENTITY) || check(TokenType::PATTERN) ||
        check(TokenType::WEAVE) || check(TokenType::THREAD) || check(TokenType::RESOURCE)) {
        // This is a dissonance type
        auto sharedType = parseType();
        if (sharedType) {
            dissonanceType =
                std::make_unique<TypeNode>(sharedType->getLocation(), sharedType->getName());
        }
    ...
    // Expect closing parenthesis
    consume(TokenType::RIGHT_PAREN, "Expected ')' after ON_DISSONANCE parameters.");

    // Parse handler body
    consume(TokenType::LEFT_BRACE, "Expected '{' after ON_DISSONANCE parameters.");
    auto body = parseBlock();

    // Create and return the DissonanceHandlerNode
    return std::make_unique<DissonanceHandlerNode>(location, std::move(dissonanceType), errorVar,
                                                   std::move(body));
}
...
std::unique_ptr<ExpendResourcesStmtNode> Parser::parseExpendResourcesStatement() {
    // Get the location of the statement
    SourceLocation location = m_previous.location;
    ...
    if (match(TokenType::AETHEL)) {
        ...
        // Unexpected token
        ErrorHandler::getInstance().reportError(m_current.location,
                                                "Expected 'AETHEL' or 'CHRONONS' after '('.");
        throw std::runtime_error("Parse error");
    }
    ...
    std::unique_ptr<ResourceInsufficiencyHandlerNode> insufficientHandlerBlock = nullptr;
    ...
    return std::make_unique<ExpendResourcesStmtNode>(
        location, std::move(aethelCostExpr), std::move(chrononCostExpr), std::move(mainBlock),
        std::move(insufficientHandlerBlock));
}

std::unique_ptr<ResourceInsufficiencyHandlerNode> Parser::parseResourceInsufficiencyHandler() {
    // Get the location of the handler
    SourceLocation location = m_previous.location;

    // Expect opening brace
    consume(TokenType::LEFT_BRACE, "Expected '{' after ON_INSUFFICIENT_RESOURCES.");
...
std::unique_ptr<TypeDefStmtNode> Parser::parseTypeDefinition() {
    ...
    if (match(TokenType::STRUCT)) {
        ...
        return parseAliasDefinition(location, typeName,
                                    typeParameters);  // This already consumes a semicolon
    } else {
        // Default to struct if no specific type is provided
        ErrorHandler::getInstance().reportError(
            m_current.location,
            "Expected a type kind (STRUCT, ENUM, UNION, or ALIAS) after type name.");
        throw std::runtime_error("Parse error");
    }
...
std::unique_ptr<TypeDefStmtNode> Parser::parseStructDefinition(
    ...
    while (!check(TokenType::RIGHT_BRACE) && !check(TokenType::EOF_TOKEN)) {
        ...
        // Create the field
        fields.emplace_back(fieldName, fieldType->getName(), std::move(defaultValue));

        // Expect a semicolon or comma after each field
        if (!match(TokenType::SEMICOLON) && !match(TokenType::COMMA)) {
            ErrorHandler::getInstance().reportError(m_current.location,
                                                    "Expect ';' or ',' after field definition.");
            throw std::runtime_error("Parse error");
        }
    }
    ...
    return std::make_unique<TypeDefStmtNode>(location, name, fields, typeParameters);
}

std::unique_ptr<TypeDefStmtNode> Parser::parseEnumDefinition(
    const SourceLocation& location, const std::string& name,
    const std::vector<std::string>& typeParameters) {
    // Parse the enum body
    ...
    return std::make_unique<TypeDefStmtNode>(location, name, variants, typeParameters);
}

std::unique_ptr<TypeDefStmtNode> Parser::parseUnionDefinition(
    const SourceLocation& location, const std::string& name,
    const std::vector<std::string>& typeParameters) {
    ...
    return std::make_unique<TypeDefStmtNode>(location, name, unionTypes, typeParameters);
}

std::unique_ptr<TypeDefStmtNode> Parser::parseAliasDefinition(
    const SourceLocation& location, const std::string& name,
    const std::vector<std::string>& typeParameters) {
    ...
    return std::make_unique<TypeDefStmtNode>(
        location, name, std::shared_ptr<TypeNode>(std::move(aliasedType)), typeParameters);
}
...
std::unique_ptr<ExprNode> Parser::parseObjectLiteral() {
    ...
    ErrorHandler::getInstance().reportError(m_previous.location,
                                            "Object literals are not fully supported yet.");
    throw std::runtime_error("Parse error");
}
...
std::unique_ptr<ExprNode> Parser::parseArrayLiteral() {
    ...
    ErrorHandler::getInstance().reportError(m_previous.location,
                                            "Array literals are not fully supported yet.");
    throw std::runtime_error("Parse error");
}
...
std::unique_ptr<TemporalLoopStmtNode> Parser::parseTemporalLoop() {
    ...
    if (match(TokenType::DURATION)) {
        consume(TokenType::LEFT_PAREN, "Expected '(' after DURATION in TEMPORAL_LOOP.");
        durationExpr = parseExpression();
        consume(TokenType::RIGHT_PAREN, "Expected ')' after duration expression in TEMPORAL_LOOP.");
    }

    // Parse condition expression
    consume(TokenType::LEFT_PAREN, "Expected '(' after TEMPORAL_LOOP.");
    std::unique_ptr<ExprNode> conditionExpr = parseExpression();
    consume(TokenType::RIGHT_PAREN, "Expected ')' after condition in TEMPORAL_LOOP.");

    // Parse variance expression (optional)
    std::unique_ptr<ExprNode> varianceExpr = nullptr;
    if (match(TokenType::VARIANCE)) {
        consume(TokenType::LEFT_PAREN, "Expected '(' after VARIANCE in TEMPORAL_LOOP.");
        varianceExpr = parseExpression();
        consume(TokenType::RIGHT_PAREN, "Expected ')' after variance expression in TEMPORAL_LOOP.");
    }
    ...
    if (match(TokenType::LEFT_BRACE)) {
        ...
        // If it's not a block, we need to create a block containing the single statement
        auto stmt = parseStatement();
        std::vector<std::unique_ptr<StmtNode>> statements;
        statements.push_back(std::move(stmt));
    ...
    return std::make_unique<TemporalLoopStmtNode>(startLoc, std::move(durationExpr),
                                                  std::move(conditionExpr), std::move(varianceExpr),
                                                  std::move(blockBody));
}
...
std::unique_ptr<StmtNode> Parser::parseStabilizeTimelineStatement() {
    ...
    consume(TokenType::SEMICOLON, "Expected ';' after STABILIZE_TIMELINE statement.");
    return std::make_unique<StabilizeTimelineStmtNode>(location, std::move(intensity));
}
...
std::unique_ptr<StmtNode> Parser::parsePreventModificationStatement() {
    ...
    consume(TokenType::SEMICOLON, "Expected ';' after PREVENT_MODIFICATION statement.");
    return std::make_unique<PreventModificationStmtNode>(location, std::move(scope));
}
...
std::unique_ptr<ParallelExecutionStmtNode> Parser::parseParallelExecution() {
    ...
    if (match(TokenType::THREADS)) {
        consume(TokenType::LEFT_PAREN, "Expected '(' after THREADS in PARALLEL_EXECUTION.");
    ...
    std::unique_ptr<ExprNode> conditionExpr = parseExpression();
    consume(TokenType::RIGHT_PAREN, "Expected ')' after condition in PARALLEL_EXECUTION.");

    // Parse synchronization points (optional)
    std::vector<std::unique_ptr<ExprNode>> syncPoints;
    while (match(TokenType::SYNC_POINT)) {
        consume(TokenType::LEFT_PAREN, "Expected '(' after SYNC_POINT in PARALLEL_EXECUTION.");
    ...
    return std::make_unique<ParallelExecutionStmtNode>(startLoc, std::move(threadCountExpr),
...
std::unique_ptr<BranchTimelineStmtNode> Parser::parseBranchTimeline() {
    ...
    if (match(TokenType::IDENTIFIER) && previous().lexeme == "merge_strategy") {
        consume(TokenType::LEFT_PAREN, "Expected '(' after merge_strategy in BRANCH_TIMELINE.");
