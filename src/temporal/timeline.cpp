...
std::vector<SyncPoint> Timeline::get_recent_sync_points(size_t count) const {
  ...
  return std::vector<SyncPoint>(sync_points_.begin() + start,
                                sync_points_.end());
}
...
void TimelineManager::add_event(const TimelineEvent &event) {
  ...
  if (event_log_.size() > max_events) {
    event_log_.erase(event_log_.begin(),
                     event_log_.begin() + (event_log_.size() - max_events));
  }
}

std::vector<TimelineEvent>
TimelineManager::get_recent_events(size_t count) const {
  std::shared_lock<std::shared_mutex> lock(mutex_);
  if (event_log_.empty()) {
    return {};
  }
  ...
  return std::vector<TimelineEvent>(event_log_.begin() + start,
                                    event_log_.end());
}
