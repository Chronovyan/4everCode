...
void DiagnosticReporter::addDiagnostic(DiagnosticSeverity severity,
  ...
  if (severity == DiagnosticSeverity::INFO ||
    ...
    // For debug output, also print to stderr for immediate feedback during
    // tests
...
void DiagnosticReporter::addError(const SourceLocation &location,
                                  const std::string &message) {
  addDiagnostic(DiagnosticSeverity::ERROR, location, message);
}

void DiagnosticReporter::addWarning(const SourceLocation &location,
                                    const std::string &message) {
  addDiagnostic(DiagnosticSeverity::WARNING, location, message);
}

void DiagnosticReporter::addInfo(const SourceLocation &location,
                                 const std::string &message) {
  addDiagnostic(DiagnosticSeverity::INFO, location, message);
}

void DiagnosticReporter::addHint(const SourceLocation &location,
                                 const std::string &message) {
  addDiagnostic(DiagnosticSeverity::HINT, location, message);
}

void DiagnosticReporter::addError(const Token &token,
                                  const std::string &message) {
  addDiagnostic(DiagnosticSeverity::ERROR, token, message);
}

void DiagnosticReporter::addWarning(const Token &token,
                                    const std::string &message) {
  addDiagnostic(DiagnosticSeverity::WARNING, token, message);
}

void DiagnosticReporter::addInfo(const Token &token,
                                 const std::string &message) {
  addDiagnostic(DiagnosticSeverity::INFO, token, message);
}

void DiagnosticReporter::addHint(const Token &token,
                                 const std::string &message) {
