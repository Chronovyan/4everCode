...
bool SourceLocation::operator==(const SourceLocation &other) const {
  ...
  return sourceFile == other.sourceFile && line == other.line &&
         column == other.column && position == other.position;
}
...
SourceLocation createBestLocation(std::shared_ptr<const SourceFile> file,
                                  size_t line, size_t column, size_t position) {
  // If we have a file but no position information, try to get the start of the
  // file
...
SourceLocation mergeLocations(const SourceLocation &start,
