...
Value Environment::get(const std::string &name) const {
  ...
  throw ChronovyanRuntimeError("Undefined variable '" + name + "'",
                               SourceLocation());
}
...
void Environment::assign(const std::string &name, Value value) {
  ...
  if (it != m_values.end()) {
    ...
    if (it->second.hasFlag(VariableFlag::STATIC)) {
      throw ChronovyanRuntimeError(
          "Cannot reassign static variable '" + name + "'", SourceLocation());
    }
  ...
  // Not found in any enclosing scope
  throw ChronovyanRuntimeError(
      "Cannot assign to undefined variable '" + name + "'", SourceLocation());
}
...
Environment::getEnvironmentWhere(const std::string &name) const {
...
Environment::getReference(const std::string &name) {
  // Look up variable in current scope
  auto it = m_values.find(name);
  if (it != m_values.end()) {
...
bool Environment::hasEnoughAethel(double amount) {
  return aethelLevel >= amount;
}

bool Environment::hasEnoughChronon(double amount) {
  return chrononLevel >= amount;
}
...
void Environment::setResourceThreshold(double threshold) {
  resourceThreshold = threshold;
}

void Environment::logResourceIntensiveOperation(const Location &location,
                                                double cost) {
  std::cout << "WARNING: Resource-intensive operation at "
            << location.getFilename() << ":" << location.line << ":"
            << location.column << " (cost: " << cost
            << " - threshold: " << resourceThreshold << ")" << std::endl;
}

bool Environment::checkResourceAvailability(double aethelCost,
                                            double chrononCost,
...
bool Environment::hasResourceInsufficiencyHandler() const {
  ...
  // For now, we'll implement a simple check to see if a specific handler
  // function exists
