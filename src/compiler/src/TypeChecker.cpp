...
TypeChecker::TypeChecker(DiagnosticReporter& diagnostics)
    ...
    , chrononType(DataType::CHRONON)
    , aethelType(DataType::AETHEL) {}
...
Type TypeChecker::checkBinaryOperation(TokenType op, const Type& leftType,
    ...
    if (op == TokenType::PLUS || op == TokenType::MINUS || op == TokenType::STAR ||
        ...
        if (op == TokenType::PLUS && (leftType.getBaseType() == DataType::STRING ||
    ...
    if (op == TokenType::EQUAL_EQUAL || op == TokenType::BANG_EQUAL || op == TokenType::LESS ||
        op == TokenType::LESS_EQUAL || op == TokenType::GREATER || op == TokenType::GREATER_EQUAL) {
...
Type TypeChecker::checkUnaryOperation(TokenType op, const Type& operandType) const {
    // Handle negation
...
bool TypeChecker::checkAssignment(const Type& variableType, const Type& valueType) const {
...
void TypeChecker::reportTypeError(const std::string& message, const Token& token) const {
