...
BytecodeGenerator::generateBytecode(const IRProgram &irProgram) {
  ...
  for (const auto &irFunction : irProgram.getFunctions()) {
    auto bytecodeFunction =
        std::make_shared<bytecode::BytecodeFunction>(irFunction.getName());
    ...
    // instructions We'll need to update the IRFunction class to provide access
    // to instructions For now, we'll skip this part
...
void BytecodeGenerator::generateInstructionForFunction(
    std::shared_ptr<bytecode::BytecodeFunction> function,
  ...
  case IROpCode::PUSH: {
    if (!operands.empty()) {
      ...
      bytecode::BytecodeConstant constant =
          convertIRValueToConstant(operands[0]);
...
bytecode::BytecodeConstant
BytecodeGenerator::convertIRValueToConstant(const IRValue &value) {
...
size_t
BytecodeGenerator::addConstant(const bytecode::BytecodeConstant &constant) {
  // Check if the constant already exists in the pool
  for (size_t i = 0; i < constantPool.size(); ++i) {
    // This is a simplified comparison; a real implementation would compare
    // based on type
    if (constant.getType() == constantPool[i].getType()) {
      ...
      default:
        ...
        if (constant.getType() ==
            bytecode::BytecodeConstant::Type::NULL_VALUE) {
          return i;
        }
        break;
      }
