...
AdvancedResourceVisualizer::AdvancedResourceVisualizer(
...
std::string AdvancedResourceVisualizer::generateResourceVisualization(
  ...
  case VisualizationOutputFormat::JSON: {
    ...
    ss << "  \"aethel_data\": " << generateJsonData(aethel_values, config)
       << ",\n";
    ss << "  \"temporal_debt_data\": "
       << generateJsonData(temporal_debt_values, config) << ",\n";
    ss << "  \"paradox_risk_data\": "
       << generateJsonData(paradox_risk_values, config) << "\n";
    ss << "}\n";
    break;
  }
...
std::string AdvancedResourceVisualizer::generateCombinedResourceVisualization(
  ...
  case VisualizationOutputFormat::ASCII: {
    ...
    double max_chronon =
        *std::max_element(chronon_values.begin(), chronon_values.end());
    double max_aethel =
        *std::max_element(aethel_values.begin(), aethel_values.end());
    double max_debt = *std::max_element(temporal_debt_values.begin(),
                                        temporal_debt_values.end());
    double max_risk = *std::max_element(paradox_risk_values.begin(),
                                        paradox_risk_values.end());
    ...
    std::vector<size_t> norm_debt =
        normalizeValues(temporal_debt_values, height);
    std::vector<size_t> norm_risk =
        normalizeValues(paradox_risk_values, height);
    ...
    for (size_t row = 0; row < height; ++row) {
      ...
      for (size_t x = 0; x < std::min(width - 4, norm_chronon.size()); ++x) {
        ...
        bool has_chronon =
            (x < norm_chronon.size() && norm_chronon[x] == y_pos);
      ...
      // Pad to full width
      line += std::string(
                  width - line.length() + reset_for_padding.length() - 1, ' ') +
              "║";
      grid[row] = line;
    }
...
bool AdvancedResourceVisualizer::exportVisualization(
    const std::string &visualization, const std::string &filename,
...
std::string AdvancedResourceVisualizer::generateAsciiChart(
    const std::vector<double> &values, ChartType chart_type,
    const ChartConfiguration &config) const {
  switch (chart_type) {
  case ChartType::LINE:
...
std::string AdvancedResourceVisualizer::generateSvgChart(
    const std::vector<double> &values, ChartType chart_type,
    const ChartConfiguration &config) const {
  ...
  ss << "<svg width=\"" << config.width * 10 << "\" height=\""
     << config.height * 10 << "\" xmlns=\"http://www.w3.org/2000/svg\">\n";

  ss << "  <rect width=\"100%\" height=\"100%\" fill=\"white\"/>\n";
  ss << "  <text x=\"50%\" y=\"20\" text-anchor=\"middle\" "
        "font-family=\"Arial\" "
        "font-size=\"16\">"
     << config.title << "</text>\n";

  // Placeholder message
  ss << "  <text x=\"50%\" y=\"50%\" text-anchor=\"middle\" "
        "font-family=\"Arial\" "
        "font-size=\"14\">"
...
std::string AdvancedResourceVisualizer::generateHtmlChart(
    const std::vector<double> &values, ChartType chart_type,
    const ChartConfiguration &config) const {
...
std::string AdvancedResourceVisualizer::generateJsonData(
...
std::vector<size_t>
AdvancedResourceVisualizer::normalizeValues(const std::vector<double> &values,
                                            size_t height) const {
  if (values.empty()) {
    return {};
  }
...
std::string
AdvancedResourceVisualizer::getColorForValue(double value,
                                             double max_value) const {
  // Choose color based on value intensity (for heatmaps and other
  // visualizations)
  if (max_value == 0) {
...
std::string AdvancedResourceVisualizer::generateAsciiLineChart(
  ...
  for (size_t x = 0; x < std::min(width, normalized.size()); ++x) {
    ...
    std::string color = getColorForValue(
        values[x], *std::max_element(values.begin(), values.end()));
  ...
  if (width > 10) {
    ...
    for (size_t i = 0; i < std::min(width, values.size());
         i += label_interval) {
...
std::string AdvancedResourceVisualizer::generateAsciiHeatmap(
  ...
  // Add legend
  ss << "\nLegend: ";
  ss << getColorForValue(min_value, max_value) << "." << config.reset_color
     << " Low  ";
  ...
  ss << getColorForValue(max_value, max_value) << "#" << config.reset_color
     << " High";
...
std::string AdvancedResourceVisualizer::generateAsciiScatterPlot(
  ...
  for (size_t i = 0; i < x_values.size(); ++i) {
    ...
    size_t x = static_cast<size_t>((x_values[i] - x_min) / (x_max - x_min) *
                                   (width - 1));
    size_t y = height - 1 -
               static_cast<size_t>((y_values[i] - y_min) / (y_max - y_min) *
                                   (height - 1));
  ...
  // Add axis labels
  ss << "  X: " << x_min << " to " << x_max << "  Y: " << y_min << " to "
     << y_max << "\n";
...
std::string AdvancedResourceVisualizer::generateAsciiChartFrame(
    const std::vector<std::string> &chart_rows,
    const ChartConfiguration &config) const {
  std::stringstream ss;
  ...
  if (!config.title.empty()) {
    ss << "│ " << config.title
       << std::string(config.width - config.title.length(), ' ') << " │\n";
    ss << "├─" << std::string(config.width, '-') << "─┤\n";
  }
