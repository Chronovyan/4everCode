...
Value::Value(const std::vector<Value>& value) : m_value(std::make_shared<ValueArray>(value)) {}
...
// Array methods
bool Value::isArray() const { return std::holds_alternative<std::shared_ptr<ValueArray>>(m_value); }
...
// Map methods
bool Value::isMap() const { return std::holds_alternative<std::shared_ptr<ValueMap>>(m_value); }
