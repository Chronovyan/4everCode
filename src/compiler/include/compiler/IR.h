...
class IRValue {
public:
    using ValueVariant = std::variant<std::monostate,  // VOID
                                      bool,            // BOOLEAN
                                      int64_t,         // INTEGER
                                      double,          // FLOAT
    ...
    explicit IRValue(const std::string& val, IRValueType strType = IRValueType::STRING)
...
class IRInstruction {
public:
    IRInstruction(IROpCode opcode, const SourceLocation& location = SourceLocation())
    ...
    IRInstruction(IROpCode opcode, const IRValue& operand1, const IRValue& operand2,
                  const SourceLocation& location = SourceLocation())
        : opcode(opcode), operands({operand1, operand2}), location(location) {}

    IRInstruction(IROpCode opcode, const IRValue& operand1, const IRValue& operand2,
                  const IRValue& operand3, const SourceLocation& location = SourceLocation())
        : opcode(opcode), operands({operand1, operand2, operand3}), location(location) {}
...
public:
    ...
    void addInstruction(const IRInstruction& instruction) { instructions.push_back(instruction); }
...
public:
    ...
    const std::vector<IRBasicBlock>& getBasicBlocks() const { return basicBlocks; }

    void addBasicBlock(const IRBasicBlock& block) { basicBlocks.push_back(block); }

    void addBasicBlock(IRBasicBlock&& block) { basicBlocks.push_back(std::move(block)); }
...
public:
    ...
    void addFunction(const IRFunction& function) { functions.push_back(function); }

    void addFunction(IRFunction&& function) { functions.push_back(std::move(function)); }
