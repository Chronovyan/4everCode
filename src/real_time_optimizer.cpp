...
RealTimeOptimizer::RealTimeOptimizer(double initial_threshold,
                                     double min_threshold, double max_threshold,
                                     size_t window_size)
    : current_threshold(initial_threshold), min_threshold(min_threshold),
      max_threshold(max_threshold), window_size(window_size) {
...
void RealTimeOptimizer::update_thresholds() {
  ...
  current_threshold = std::clamp(current_threshold + threshold_change,
...
void RealTimeOptimizer::calculate_trend_indicators() {
  ...
  for (auto it = metrics_history.rbegin() + 1; it != metrics_history.rend();
       ++it) {
