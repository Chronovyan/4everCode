...
// LiteralExprNode
void LiteralExprNode::accept(ASTVisitor& visitor) const { visitor.visitLiteralExpr(*this); }

// VariableExprNode

void VariableExprNode::accept(ASTVisitor& visitor) const { visitor.visitVariableExpr(*this); }

// UnaryExprNode

UnaryExprNode::UnaryExprNode(Token op, std::unique_ptr<ExprNode> right)
    : ExprNode(op.location), m_operator(std::move(op)), m_right(std::move(right)) {}

void UnaryExprNode::accept(ASTVisitor& visitor) const { visitor.visitUnaryExpr(*this); }

// BinaryExprNode

BinaryExprNode::BinaryExprNode(std::unique_ptr<ExprNode> left, Token op,
                               std::unique_ptr<ExprNode> right)
    : ExprNode(op.location)
    , m_left(std::move(left))
    , m_operator(std::move(op))
    , m_right(std::move(right)) {}

void BinaryExprNode::accept(ASTVisitor& visitor) const { visitor.visitBinaryExpr(*this); }
...
void GroupingExprNode::accept(ASTVisitor& visitor) const { visitor.visitGroupingExpr(*this); }

// AssignExprNode

void AssignExprNode::accept(ASTVisitor& visitor) const { visitor.visitAssignExpr(*this); }
...
void CallExprNode::accept(ASTVisitor& visitor) const { visitor.visitCallExpr(*this); }
...
void ExprStmtNode::accept(ASTVisitor& visitor) const { visitor.visitExprStmt(*this); }

// BlockStmtNode

BlockStmtNode::BlockStmtNode(std::vector<std::unique_ptr<StmtNode>> statements)
    : StmtNode(statements.empty() ? SourceLocation() : statements[0]->getLocation())
    , m_statements(std::move(statements)) {}

void BlockStmtNode::accept(ASTVisitor& visitor) const { visitor.visitBlockStmt(*this); }
...
void IfStmtNode::accept(ASTVisitor& visitor) const { visitor.visitIfStmt(*this); }
...
void TemporalOpStmtNode::accept(ASTVisitor& visitor) const { visitor.visitTemporalOpStmt(*this); }
...
void ProgramNode::accept(ASTVisitor& visitor) const { visitor.visitProgram(*this); }

// TypeDefStmtNode implementation
TypeDefStmtNode::TypeDefStmtNode(const SourceLocation& location, const std::string& name,
...
TypeDefStmtNode::TypeDefStmtNode(const SourceLocation& location, const std::string& name,
    ...
    , m_typeParameters(typeParameters)
    , m_kind(TypeDefKind::ALIAS) {}

void TypeDefStmtNode::accept(ASTVisitor& visitor) const { visitor.visitTypeDefStmt(*this); }

const std::string& TypeDefStmtNode::getName() const { return m_name; }

const std::vector<StructField>& TypeDefStmtNode::getFields() const { return m_fields; }

const std::vector<EnumVariant>& TypeDefStmtNode::getVariants() const { return m_variants; }

const std::vector<std::shared_ptr<TypeNode>>& TypeDefStmtNode::getUnionTypes() const {
    return m_unionTypes;
}

std::shared_ptr<TypeNode> TypeDefStmtNode::getAliasedType() const { return m_aliasedType; }
