...
const DebuggerState& DejaVuDebugger::getCurrentState() const { return *currentState; }

const StateHistory& DejaVuDebugger::getStateHistory() const { return *stateHistory; }

std::string DejaVuDebugger::getVariableValue(const std::string& variableName) const {
...
std::string DejaVuDebugger::getVariableValueAt(const std::string& variableName,
    ...
    if (historyIndex < 0 || historyIndex >= static_cast<int>(stateHistory->getStateCount())) {
        return "";
    }
...
int DejaVuDebugger::getCurrentTimelineId() const { return currentState->getTimelineId(); }

std::map<std::string, double> DejaVuDebugger::getCurrentResourceUsage() const {
    return currentState->getResourceUsage();
}

int DejaVuDebugger::registerBreakpointCallback(std::function<void(const DebuggerState&)> callback) {
...
std::string DejaVuDebugger::visualizeCurrentState(const std::string& format) const {
    return stateVisualizer->visualizeState(*currentState, format);
}

std::string DejaVuDebugger::visualizeStateHistory(const std::string& format) const {
    return stateVisualizer->visualizeHistory(*stateHistory, format);
}

std::string DejaVuDebugger::visualizeTemporalFlow(const std::string& format) const {
    return stateVisualizer->visualizeTemporalFlow(*stateHistory, format);
}

bool DejaVuDebugger::checkBreakpoints() { return breakpointMgr->isBreakpointHit(*currentState); }
...
bool DejaVuDebugger::executeInstruction() {
    ...
    if (functionIt == functions.end()) {
        diagnostics.addError(SourceLocation(), "Function not found: " + currentFunction);
        stop();
        return false;
    }
    ...
    if (ip >= instructions.size()) {
        ...
        if (!currentState->getCallStack().empty()) {
            ...
            // Note: In a real implementation, we would need to get the caller's instruction pointer
            // and restore it here. For simplicity, we just increment it.
            currentState->setInstructionPointer(currentState->getInstructionPointer() + 1);
        } else {
    ...
    switch (instruction.getOpcode()) {
        ...
        case bytecode::OpCode::STORE_VAR: {
            ...
            // In a real implementation, we would pop a value from the stack and store it in the
            // variable

            // For demonstration purposes, assume the instruction has a string operand that is the
            // variable name
            if (instruction.getOperands().empty() ||
                instruction.getOperands()[0].getType() != bytecode::OperandType::STRING) {
        ...
        case bytecode::OpCode::CALL: {
            // Call a function
            // In a real implementation, we would set up the call frame, push arguments, etc.
        ...
        case bytecode::OpCode::JUMP: {
            ...
            // For demonstration purposes, assume the instruction has an integer operand that is the
            // jump offset
            if (instruction.getOperands().empty() ||
                instruction.getOperands()[0].getType() != bytecode::OperandType::INTEGER) {
        ...
        case bytecode::OpCode::JUMP_IF_TRUE: {
            ...
            if (instruction.getOperands().empty() ||
                instruction.getOperands()[0].getType() != bytecode::OperandType::INTEGER) {
                diagnostics.addError(SourceLocation(), "Invalid JUMP_IF_TRUE instruction");
                stop();
                return false;
            }
        ...
        case bytecode::OpCode::TIMELINE_BRANCH_BEGIN: {
            ...
            if (instruction.getOperands().empty() ||
                instruction.getOperands()[0].getType() != bytecode::OperandType::INTEGER) {
                diagnostics.addError(SourceLocation(), "Invalid TIMELINE_BRANCH_BEGIN instruction");
                stop();
                return false;
            }
        ...
        case bytecode::OpCode::PARALLEL_BEGIN: {
            ...
            if (instruction.getOperands().empty() ||
                instruction.getOperands()[0].getType() != bytecode::OperandType::INTEGER) {
                diagnostics.addError(SourceLocation(), "Invalid PARALLEL_BEGIN instruction");
                stop();
                return false;
            }
        ...
        case bytecode::OpCode::RESOURCE_BEGIN: {
            ...
            // For demonstration purposes, assume the instruction has a float operand that is the
            // resource amount
            if (instruction.getOperands().empty() ||
                instruction.getOperands()[0].getType() != bytecode::OperandType::FLOAT) {
                diagnostics.addError(SourceLocation(), "Invalid RESOURCE_BEGIN instruction");
                stop();
                return false;
            }
        ...
        default:
            ...
            diagnostics.addError(SourceLocation(),
                                 "Unknown opcode: " +
                                     std::to_string(static_cast<int>(instruction.getOpcode())));
            stop();
            return false;
    }
...
void DejaVuDebugger::updateStateHistory() { stateHistory->addState(*currentState); }
