...
NativeFunction::NativeFunction(FunctionType func, int arity)
    : m_func(func), m_arity(arity), m_minArity(arity), m_maxArity(arity), m_variableArity(false) {}
...
Value NativeFunction::call(Interpreter& interpreter, const std::vector<Value>& args) const {
    ...
    if (m_variableArity) {
        if (m_minArity >= 0 && static_cast<int>(args.size()) < m_minArity) {
            // Too few arguments
            std::stringstream error;
            error << "Expected at least " << m_minArity << " arguments but got " << args.size();
            ErrorHandler::getInstance().reportError(SourceLocation(), error.str());
            return Value();  // Return nil on error
        }
        ...
        if (m_maxArity >= 0 && static_cast<int>(args.size()) > m_maxArity) {
            // Too many arguments
            std::stringstream error;
            error << "Expected at most " << m_maxArity << " arguments but got " << args.size();
            ErrorHandler::getInstance().reportError(SourceLocation(), error.str());
            return Value();  // Return nil on error
        }
    }
