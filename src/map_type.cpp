...
// Set key-value pair
void TemporalMap::set(const std::string &key, const Value &value,
...
bool TemporalMap::remove(const std::string &key) {
  ...
  if (m_runtime) {
    int currentCycle = static_cast<int>(m_runtime->getCycleNumber());
    auto &history = m_keyHistory[key];
    history.values[currentCycle] =
        Value(); // nil value indicates key was removed
    history.latest_cycle = currentCycle;
  }
...
Value TemporalMap::reduce(
    std::function<Value(const Value &, const std::string &, const Value &)>
        func,
...
std::shared_ptr<TemporalMap> TemporalMap::merge(const TemporalMap &other,
...
Value TemporalMap::getAtCycle(const std::string &key, int cycle) const {
  ...
  throw std::out_of_range("Key not found at cycle " + std::to_string(cycle) +
                          ": " + key);
}
...
void TemporalMap::trackResourceUsage(const std::string &operation) const {
  if (m_runtime) {
    ...
    // Unfortunately, consumeChronons is not const, so we need to use a
    // const_cast This is a safe use of const_cast since we know the object is
    // non-const in reality
