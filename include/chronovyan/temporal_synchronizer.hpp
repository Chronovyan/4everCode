...
public:
    ...
    enum class RecoveryStrategy { Automatic, Manual, Custom };
    ...
    bool is_paused() const { return is_synchronization_paused.load(std::memory_order_relaxed); }
...
private:
    ...
    std::unique_ptr<RealTimeOptimizer> real_time_optimizer;
    std::unique_ptr<PatternRecognizer> pattern_recognizer;
    std::unique_ptr<ErrorPredictor> error_predictor;
    ...
    std::vector<PatternCluster> k_means_clustering(const std::vector<std::vector<double>>& patterns,
                                                   size_t k) const;
