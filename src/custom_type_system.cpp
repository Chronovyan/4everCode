...
TypeExpression
TypeExpressionParser::parseTypeExpression(const std::string &expr, size_t &pos,
                                          const SourceLocation &location) {
  ...
  if (start == pos) {
    ErrorHandler::getInstance().reportError(
        location, "Expected type name in type expression");
    return TypeExpression("Any"); // Default to Any type
  }
...
TypeExpressionParser::parseTypeArguments(const std::string &expr, size_t &pos,
                                         const SourceLocation &location) {
  ...
  if (pos >= expr.size() || expr[pos] != '>') {
    ErrorHandler::getInstance().reportError(
        location, "Expected '>' to close type arguments");
  } else {
    pos++; // Skip '>'
  }
...
void TypeExpressionParser::skipWhitespace(const std::string &expr,
                                          size_t &pos) {
...
CustomTypeDefinition::CustomTypeDefinition(
    const std::string &name, CustomTypeKind kind,
    const std::map<std::string, Value> &properties,
    const std::vector<std::string> &typeParameters)
    : m_name(name), m_kind(kind), m_properties(properties),
...
void CustomTypeDefinition::setProperty(const std::string &name,
                                       const Value &value) {
  m_properties[name] = value;
}

bool CustomTypeDefinition::isGeneric() const {
  return !m_typeParameters.empty();
}

const std::vector<std::string> &
CustomTypeDefinition::getTypeParameters() const {
  return m_typeParameters;
}

const std::vector<TypeParameter> &
CustomTypeDefinition::getTypeParametersWithConstraints() const {
...
CustomTypeInstance::CustomTypeInstance(
    std::shared_ptr<CustomTypeDefinition> typeDefinition,
    const std::map<std::string, Value> &values)
    : m_typeDefinition(typeDefinition), m_values(values) {}

std::shared_ptr<CustomTypeDefinition>
CustomTypeInstance::getTypeDefinition() const {
...
void CustomTypeInstance::setField(const std::string &name, const Value &value) {
  ...
  if (m_typeDefinition->hasProperty(name)) {
    ...
    std::stringstream ss;
    ss << "Field '" << name << "' does not exist in type '"
       << m_typeDefinition->getName() << "'";
    ErrorHandler::getInstance().reportError(SourceLocation(), ss.str());
  }
}

const std::map<std::string, Value> &CustomTypeInstance::getAllFields() const {
  return m_values;
}
...
void CustomTypeSystem::initializeTypeCheckers() {
  ...
  m_typeCheckers[CustomTypeKind::STRUCT] =
      [this](const Value &value, const CustomTypeDefinition &typeDef) -> bool {
    ...
    if (!typeDef.hasProperty("fields") ||
        !typeDef.getProperty("fields").isMap()) {
      // Missing or invalid fields property
      return false;
    }

    // Check each property defined in the type
    for (const auto &[propName, propValue] :
         typeDef.getProperty("fields").asMap()) {
      ...
      if (propValue.isMap() &&
          propValue.asMap().find("type") != propValue.asMap().end() &&
          propValue.asMap().at("type").isString()) {
        const std::string &fieldTypeName =
            propValue.asMap().at("type").asString();
  ...
  m_typeCheckers[CustomTypeKind::ENUM] =
    ...
    if (!typeDef.hasProperty("variants") ||
        !typeDef.getProperty("variants").isMap()) {
  ...
  m_typeCheckers[CustomTypeKind::UNION] =
    ...
    const auto &possibleTypes =
        typeDef.hasProperty("types")
  ...
  m_typeCheckers[CustomTypeKind::ALIAS] =
      [this](const Value &value, const CustomTypeDefinition &typeDef) -> bool {
    ...
    const std::string &aliasedTypeName =
  ...
  m_typeCheckers[CustomTypeKind::MODULE_TYPE] =
      [](const Value &value, const CustomTypeDefinition &typeDef) -> bool {
...
void CustomTypeSystem::initializeBuiltInTypes() {
  ...
  auto arrayType = std::make_shared<CustomTypeDefinition>(
      "Array", CustomTypeKind::STRUCT, arrayProps, arrayParams);
  ...
  auto mapType = std::make_shared<CustomTypeDefinition>(
      "Map", CustomTypeKind::STRUCT, mapProps, mapParams);
  ...
  auto objectType = std::make_shared<CustomTypeDefinition>(
      "Object", CustomTypeKind::STRUCT, objProps);
  ...
  auto anyProps = std::map<std::string, Value>{
      {"primitive", Value(false)},
      {"memberTypes",
       Value(typesList)} // Use memberTypes instead of types for validation
  };
  auto anyType = std::make_shared<CustomTypeDefinition>(
      "Any", CustomTypeKind::UNION, anyProps);
  registerType(anyType);
}

bool CustomTypeSystem::registerType(
    std::shared_ptr<CustomTypeDefinition> typeDef) {
  ...
  case CustomTypeKind::STRUCT:
    if (!typeDef->hasProperty("fields")) {
      isValid = false;
      validationError =
          "Struct type '" + name + "' must have a 'fields' property";
    }
    break;
  ...
  case CustomTypeKind::ENUM:
    if (!typeDef->hasProperty("variants")) {
      isValid = false;
      validationError =
          "Enum type '" + name + "' must have a 'variants' property";
    }
    break;

  case CustomTypeKind::UNION:
    if (!typeDef->hasProperty("memberTypes") &&
        !typeDef->hasProperty("types")) {
      isValid = false;
      validationError =
          "Union type '" + name + "' must have a 'memberTypes' property";
    }
    break;
  ...
  case CustomTypeKind::ALIAS:
    if (!typeDef->hasProperty("aliasedType") &&
      ...
      isValid = false;
      validationError =
          "Alias type '" + name + "' must have an 'aliasedType' property";
    }
    break;
...
std::shared_ptr<CustomTypeDefinition>
CustomTypeSystem::getTypeDefinition(const std::string &name) const {
...
std::shared_ptr<CustomTypeInstance>
CustomTypeSystem::createInstance(const std::string &typeName,
                                 const std::map<std::string, Value> &values) {
...
bool CustomTypeSystem::checkType(const Value &value,
    ...
    if (!result) {
      ...
      case CustomTypeKind::ENUM:
        ...
        if (value.isString() || value.isInteger()) {
          ...
          ss << " - Expected a string or integer value representing an enum "
                "variant";
        }
        break;
...
const std::map<std::string, std::shared_ptr<CustomTypeDefinition>> &
CustomTypeSystem::getAllTypes() const {
  return m_types;
}
...
std::shared_ptr<CustomTypeDefinition> CustomTypeSystem::defineBuiltInType(
  ...
  auto typeDef = std::make_shared<CustomTypeDefinition>(
      name,
      CustomTypeKind::STRUCT, // Use STRUCT as the default kind for built-in
                              // types
      properties);
...
TypeExpression
CustomTypeSystem::parseAndValidateType(const std::string &expression,
                                       const SourceLocation &location) {
  ...
  if (!isTypeRegistered(typeExpr.baseName) && typeExpr.baseName != "nil" &&
      typeExpr.baseName != "bool" && typeExpr.baseName != "int" &&
      typeExpr.baseName != "float" && typeExpr.baseName != "string" &&
      typeExpr.baseName != "Array" && typeExpr.baseName != "Map" &&
      typeExpr.baseName != "Function" && typeExpr.baseName != "Object" &&
      typeExpr.baseName != "Any") {
    std::stringstream ss;
  ...
  if (!typeExpr.typeArguments.empty()) {
    ...
    if (!typeDef) {
      ...
      // Check if the number of type arguments matches the number of type
      // parameters
...
std::string CustomTypeSystem::createGenericTypeInstance(
    const std::string &baseType, const std::vector<std::string> &typeArgs,
    const SourceLocation &location) {
  // Check if the base type exists and is generic
  auto typeDef = getTypeDefinition(baseType);
  if (!typeDef) {
    std::stringstream ss;
    ss << "Cannot create generic instance: Base type '" << baseType
       << "' is not defined";
    ErrorHandler::getInstance().reportError(location, ss.str());
  ...
  if (!typeDef->isGeneric()) {
    std::stringstream ss;
    ss << "Cannot create generic instance: Type '" << baseType
  ...
  // Copy properties from the base type, substituting type parameters with
  // arguments
...
bool CustomTypeSystem::isSubtypeOf(const std::string &subType,
                                   const std::string &superType) const {
  ...
  if (!subTypeDef) {
    ...
    if (subType == "nil" || subType == "bool" || subType == "int" ||
        subType == "float" || subType == "string") {
  ...
  if (subTypeDef->getKind() == CustomTypeKind::ALIAS) {
    ...
    std::string aliasedType =
        subTypeDef->hasProperty("aliasedType")
