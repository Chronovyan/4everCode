...
public:
    void flagComponentForMaintenance(const std::string& componentName) {
        std::cout << "[SYSTEM MONITOR] Component flagged for maintenance: " << componentName
                  << std::endl;
    }
};
}  // namespace monitoring
...
public:
    ...
    SurgeHandler(sensors::WhisperingStoneSensor& whisperingStone)
        : m_whisperingStone(whisperingStone), m_systemMonitor(), m_userInterface() {
    ...
    void handleTemporalSurge(float magnitude, float duration) {
        Logger::info("SurgeHandler: Processing temporal surge with magnitude " +
                     std::to_string(magnitude) + " and duration " + std::to_string(duration));

        // Simulate some processing time
        std::this_thread::sleep_for(std::chrono::milliseconds(static_cast<int>(duration * 100)));

        // Enhanced sensor recovery for high-magnitude surges
        if (magnitude > 3.7f) {
            Logger::warning("SurgeHandler: High-magnitude surge detected (" +
                            std::to_string(magnitude) + "), performing enhanced recovery");
            ...
            for (int attempt = 1; attempt <= maxRetries && !resetSuccess; attempt++) {
                ...
                if (!resetSuccess && attempt < maxRetries) {
                    ...
                    Logger::warning("SurgeHandler: Reset attempt " + std::to_string(attempt) +
            ...
            if (!resetSuccess) {
                Logger::error("SurgeHandler: Failed to reset WhisperingStoneSensor after " +
                              std::to_string(maxRetries) + " attempts. Flagging for maintenance.");
                ...
                Logger::info("SurgeHandler: WhisperingStoneSensor successfully reset after surge");
            }
        } else {
            // Normal reset for low-magnitude surges
            Logger::info("SurgeHandler: Low-magnitude surge, performing standard reset");
...
void testSurgeHandler() {
    ...
    auto whisperingStoneSensor = std::make_unique<sensors::WhisperingStoneSensorImpl>();
    ...
    sensors::PressureReading baselineReading = whisperingStoneSensor->getPressureReading();
    std::cout << "Baseline reading before surge: " << baselineReading.getValue() << std::endl;
    ...
    sensors::PressureReading afterLowSurgeReading = whisperingStoneSensor->getPressureReading();
    std::cout << "Reading after low-magnitude surge: " << afterLowSurgeReading.getValue()
              << " (valid: " << (afterLowSurgeReading.isValid() ? "yes" : "no") << ")" << std::endl;
    ...
    sensors::PressureReading afterHighSurgeReading = whisperingStoneSensor->getPressureReading();
    std::cout << "Reading after high-magnitude surge: " << afterHighSurgeReading.getValue()
    ...
    for (int i = 0; i < 3; i++) {
        ...
        sensors::PressureReading reading = whisperingStoneSensor->getPressureReading();
        std::cout << "Reading after surge " << (i + 1) << ": " << reading.getValue()
                  << " (valid: " << (reading.isValid() ? "yes" : "no") << ")" << std::endl;
    }
