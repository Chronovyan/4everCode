...
DiagnosticReporter& AstBuilder::getDiagnosticReporter() { return m_diagnostics; }

std::unique_ptr<ExpressionNode> AstBuilder::createLiteral(const std::string& value) {
    ...
    if (value == "true" || value == "false") {
        token =
            Token(value == "true" ? TokenType::TRUE : TokenType::FALSE, value, SourceLocation());
        ...
        try {
            ...
            m_diagnostics.addError(SourceLocation(), "Invalid float literal: " + value);
            return nullptr;
        }
        ...
        try {
            ...
            m_diagnostics.addError(SourceLocation(), "Invalid integer literal: " + value);
            return nullptr;
        }
    }
}

std::unique_ptr<ExpressionNode> AstBuilder::createVariable(const std::string& name) {
...
std::unique_ptr<ExpressionNode> AstBuilder::createBinaryExpr(
    std::unique_ptr<ExpressionNode> left, const std::string& op,
    ...
    if (op == "+")
        ...
        m_diagnostics.addError(SourceLocation(), "Unsupported binary operator: " + op);
        return nullptr;
    }

    Token opToken(opType, op, SourceLocation());
    return std::make_unique<BinaryExprNode>(std::move(left), opToken, std::move(right));
}

std::unique_ptr<ExpressionNode> AstBuilder::createCall(
...
std::unique_ptr<StatementNode> AstBuilder::createVariableDecl(
    const std::string& name, const std::string& type, std::unique_ptr<ExpressionNode> initializer) {
    ...
    return std::make_unique<VarDeclNode>(nameToken, std::move(typeNode), std::move(initializer));
}

std::unique_ptr<StatementNode> AstBuilder::createBlock(
    std::vector<std::unique_ptr<StatementNode>> statements) {
    return std::make_unique<BlockStmtNode>(std::move(statements), SourceLocation());
}

std::unique_ptr<StatementNode> AstBuilder::createIf(std::unique_ptr<ExpressionNode> condition,
...
std::unique_ptr<StatementNode> AstBuilder::createFor(std::unique_ptr<StatementNode> initializer,
    ...
    return std::make_unique<ForStmtNode>(std::move(initializer), std::move(condition),
                                         std::move(increment), std::move(body), SourceLocation());
}

std::unique_ptr<StatementNode> AstBuilder::createWhile(std::unique_ptr<ExpressionNode> condition,
                                                       std::unique_ptr<StatementNode> body) {
    return std::make_unique<WhileStmtNode>(std::move(condition), std::move(body), SourceLocation());
}

std::unique_ptr<StatementNode> AstBuilder::createFunctionDecl(
    const std::string& name, const std::vector<std::string>& params, const std::string& returnType,
    std::unique_ptr<StatementNode> body) {
    ...
    for (const auto& param : params) {
        paramTokens.push_back(Token(TokenType::IDENTIFIER, param, SourceLocation()));
    }
    ...
    return std::make_unique<FunctionDeclNode>(nameToken, paramTokens, std::move(returnTypeNode),
                                              std::move(body));
}
