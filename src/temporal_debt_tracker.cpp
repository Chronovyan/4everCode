...
// Constructor
TemporalDebtTracker::TemporalDebtTracker(
    std::shared_ptr<TemporalRuntime> runtime)
    : m_runtime(runtime), m_current_cycle(0),
      m_debt_limit(1000.0), // Default debt limit
      m_low_alert_threshold(0.3), m_medium_alert_threshold(0.5),
      m_high_alert_threshold(0.7), m_critical_alert_threshold(0.9) {
...
void TemporalDebtTracker::initializeStrategies() {
  m_strategies[RepaymentStrategyType::OLDEST_FIRST] =
      std::make_shared<OldestFirstStrategy>();
  m_strategies[RepaymentStrategyType::HIGHEST_INTEREST] =
      std::make_shared<HighestInterestStrategy>();
  m_strategies[RepaymentStrategyType::CRITICAL_FIRST] =
      std::make_shared<CriticalFirstStrategy>();
  m_strategies[RepaymentStrategyType::BALANCED] =
  ...
  m_strategies[RepaymentStrategyType::SNOWBALL] =
      std::make_shared<SnowballStrategy>();
  m_strategies[RepaymentStrategyType::AVALANCHE] =
      std::make_shared<AvalancheStrategy>();
...
std::shared_ptr<RepaymentStrategy>
TemporalDebtTracker::getStrategy(RepaymentStrategyType strategy_type) const {
...
RepaymentResult
TemporalDebtTracker::applyRepaymentStrategy(RepaymentStrategyType strategy_type,
...
// Borrow chronons
bool TemporalDebtTracker::borrowChronons(double amount,
                                         const std::string &operation_id,
                                         bool critical) {
  // Check if exceeding limit
  if (wouldExceedLimit(amount)) {
    // Create a debt alert for the failed borrowing attempt
    DebtAlert alert(DebtAlertLevel::CRITICAL,
                    "Cannot borrow chronons: would exceed limit", operation_id,
                    calculateDebtRatio(), "chronon");
  ...
  double interest_rate =
      calculateInterestRate(amount, critical, RebelOperationType::NONE, 1);
  int due_cycle = calculateDueCycle(amount, critical, RebelOperationType::NONE);

  // Create debt record
  DebtRecord record(operation_id, amount, 0.0, interest_rate, due_cycle,
                    critical);
...
// Borrow aethel
bool TemporalDebtTracker::borrowAethel(double amount,
                                       const std::string &operation_id,
                                       bool critical) {
  // Check if exceeding limit
  if (wouldExceedLimit(amount)) {
    // Create a debt alert for the failed borrowing attempt
    DebtAlert alert(DebtAlertLevel::CRITICAL,
                    "Cannot borrow aethel: would exceed limit", operation_id,
                    calculateDebtRatio(), "aethel");
  ...
  // Create debt record
  DebtRecord record(operation_id, 0.0, amount, interest_rate, due_cycle,
                    critical);
...
bool TemporalDebtTracker::accrueRebelDebt(RebelOperationType operation_type,
  ...
  double aethel_amount = custom_aethel_amount >= 0.0
                             ? custom_aethel_amount
                             : getDefaultAethelCost(operation_type);

  // Check if exceeding limit
  if (wouldExceedLimit(aethel_amount)) {
    // Create a debt alert for the failed borrowing attempt
    DebtAlert alert(DebtAlertLevel::CRITICAL,
                    "Cannot accrue rebel debt: would exceed limit",
  ...
  if (it != m_debts.end()) {
    ...
    it->interest_rate = calculateInterestRate(it->aethel_debt, is_critical,
  ...
  // Calculate interest rate and due cycle
  double interest_rate =
      calculateInterestRate(aethel_amount, is_critical, operation_type, 1);
  int due_cycle = calculateDueCycle(aethel_amount, is_critical, operation_type);

  // Create new debt record
  DebtRecord record(operation_id, 0.0, aethel_amount, interest_rate, due_cycle,
...
bool TemporalDebtTracker::accrueRebelDebtWithChronons(
    RebelOperationType operation_type, const std::string &operation_id,
    double custom_chronon_amount, double custom_aethel_amount,
    bool is_critical) {
  // Determine chronon and aethel amounts
  double chronon_amount = custom_chronon_amount >= 0.0
                              ? custom_chronon_amount
                              : getDefaultChronCost(operation_type);
  double aethel_amount = custom_aethel_amount >= 0.0
                             ? custom_aethel_amount
                             : getDefaultAethelCost(operation_type);
  ...
  if (it != m_debts.end()) {
    ...
    // Recalculate interest rate
    it->interest_rate =
        calculateInterestRate(it->chronon_debt + it->aethel_debt, is_critical,
                              operation_type, operation_count);
  ...
  // Calculate interest rate and due cycle
  double interest_rate = calculateInterestRate(chronon_amount + aethel_amount,
                                               is_critical, operation_type, 1);
  int due_cycle = calculateDueCycle(chronon_amount + aethel_amount, is_critical,
                                    operation_type);

  // Create new debt record
  DebtRecord record(operation_id, chronon_amount, aethel_amount, interest_rate,
                    due_cycle, is_critical, operation_type, 1);
...
// Repay chronons
double TemporalDebtTracker::repayChronons(double amount,
                                          const std::string &operation_id) {
  ...
  if (!operation_id.empty()) {
    ...
    RepaymentResult result =
        getStrategy(RepaymentStrategyType::HIGHEST_INTEREST)
            ->apply(m_debts, amount, 0.0);
...
// Repay aethel
double TemporalDebtTracker::repayAethel(double amount,
                                        const std::string &operation_id) {
  double total_repaid = 0.0;
...
// Find existing debt
std::vector<DebtRecord>::iterator
TemporalDebtTracker::findExistingDebt(const std::string &operation_id,
                                      RebelOperationType rebel_type) {
  return std::find_if(m_debts.begin(), m_debts.end(),
                      [&](const DebtRecord &debt) {
                        return debt.operation_id == operation_id &&
                               (rebel_type == RebelOperationType::NONE ||
                                debt.rebel_type == rebel_type);
                      });
}
...
double TemporalDebtTracker::calculateInterestRate(double amount, bool critical,
  ...
  double compounding_factor =
      calculateCompoundingFactor(rebel_type, operation_count);

  // Combine factors
  return base_rate + amount_factor + critical_factor + rebel_factor +
         compounding_factor;
}

// Calculate due cycle
int TemporalDebtTracker::calculateDueCycle(
    double amount, bool critical, RebelOperationType rebel_type) const {
  ...
  // Combine offsets
  return std::max(1, base_due_cycle + amount_offset + critical_offset +
                         rebel_offset);
}

// Get interest rate modifier
double TemporalDebtTracker::getInterestRateModifier(
...
int TemporalDebtTracker::getDueCycleOffset(
    RebelOperationType operation_type) const {
  double base_cost = 0;
  double factor = 1.0;
...
double TemporalDebtTracker::getDefaultAethelCost(
    RebelOperationType operation_type) const {
  switch (operation_type) {
  case RebelOperationType::NONE:
    // No specific factor for generic operations
...
double TemporalDebtTracker::getDefaultChronCost(
    RebelOperationType operation_type) const {
  switch (operation_type) {
  case RebelOperationType::NONE:
    return 10; // Base cost for any temporal operation
...
void TemporalDebtTracker::setAlertThresholds(double low_threshold,
...
RepaymentResult
TemporalDebtTracker::applyMaximumRepayment(double max_chronon_percent,
...
std::string TemporalDebtTracker::getStrategyDescription(
...
int TemporalDebtTracker::advanceCycle() {
  ...
  for (auto it = m_debts.begin(); it != m_debts.end();) {
    ...
    if (it->due_cycle <= m_current_cycle) {
      ...
      DebtAlert alert(DebtAlertLevel::CRITICAL,
                      "Debt has defaulted: " + it->operation_id,
      ...
      if (debt_ratio >= m_critical_alert_threshold) {
        ...
        DebtAlert alert(DebtAlertLevel::HIGH, "High debt level reached",
                        it->operation_id, debt_ratio, "ratio");
        ...
        DebtAlert alert(DebtAlertLevel::LOW, "Low debt level reached",
                        it->operation_id, debt_ratio, "ratio");
...
double TemporalDebtTracker::projectFutureDebt(int cycles) const {
  ...
  for (const auto &debt : m_debts) {
    ...
    double chronon_future =
        debt.chronon_debt * std::pow(1.0 + debt.interest_rate, cycles);
    double aethel_future =
        debt.aethel_debt * std::pow(1.0 + debt.interest_rate, cycles);
...
double TemporalDebtTracker::getRebelOperationRiskLevel(
    RebelOperationType operation_type) const {
  switch (operation_type) {
  case RebelOperationType::NONE:
    // No specific factor for generic operations
...
double
TemporalDebtTracker::calculateCompoundingFactor(RebelOperationType rebel_type,
                                                int operation_count) const {
...
double
TemporalDebtTracker::calculateChronoImpact(RebelOperationType operation_type,
                                           int operation_count) const {
  ...
  double compounding =
      calculateCompoundingFactor(operation_type, operation_count);
...
double
TemporalDebtTracker::calculateAethelImpact(RebelOperationType operation_type,
                                           int operation_count) const {
...
std::string TemporalDebtTracker::getRebelOperationName(
    RebelOperationType operation_type) const {
  switch (operation_type) {
  case RebelOperationType::TIMELINE_ALTERATION:
...
std::string TemporalDebtTracker::getRebelOperationInfo(
    RebelOperationType operation_type) const {
  std::stringstream ss;
  ...
  case RebelOperationType::PARADOX_CREATION:
    ...
    ss << "Very high risk of timeline collapse and severe resource "
          "penalties.\n";
    break;
...
double
TemporalDebtTracker::projectParadoxRisk(RebelOperationType operation_type,
                                        int cycles, int operation_count) const {
...
std::pair<double, double> TemporalDebtTracker::getRebelDebtTotal(
  ...
  for (const auto &debt : m_debts) {
    if (operation_type == RebelOperationType::NONE ||
        debt.rebel_type == operation_type) {
...
std::map<RebelOperationType, int>
TemporalDebtTracker::getOperationCounts() const {
  ...
  try {
    std::cerr << "[getOperationCounts] Attempting "
                 "counts[RebelOperationType::NONE] = 0;"
              << std::endl;
    counts[RebelOperationType::NONE] =
        0; // Explicitly insert a benign key for diagnostics
    std::cerr << "[getOperationCounts] counts[RebelOperationType::NONE] = 0; "
                 "successful."
              << std::endl;

    std::cerr << "[getOperationCounts] Attempting to iterate m_debts. Size: "
              << m_debts.size() << std::endl;
    for (const auto &debt_record :
         m_debts) { // m_debts is std::vector<DebtRecord>
      std::cerr
          << "[getOperationCounts] Processing debt_record. Rebel Type (int): "
          << static_cast<int>(debt_record.rebel_type)
          << ", Operation ID: " << debt_record.operation_id << std::endl;
      counts[debt_record.rebel_type]++;
      std::cerr
          << "[getOperationCounts] Incremented count for Rebel Type (int): "
          << static_cast<int>(debt_record.rebel_type) << std::endl;
    }
    std::cerr << "[getOperationCounts] Finished iterating m_debts."
              << std::endl;
  } catch (const std::exception &e) {
    std::cerr << "[getOperationCounts] EXCEPTION (std::exception): " << e.what()
              << std::endl;
    throw; // Rethrow
  } catch (...) {
  ...
  std::cerr << "[getOperationCounts] Returning counts. Map size: "
            << counts.size() << std::endl;
  return counts;
}

// Get the count of operations of a specific type
int TemporalDebtTracker::getRebelOperationCount(
    RebelOperationType operation_type, const std::string &operation_id) const {
  if (operation_id.empty()) {
    ...
    int count = 0;
    for (const auto &debt : m_debts) {
      if (debt.rebel_type == operation_type &&
          debt.operation_id == operation_id) {
        count++;
      }
    }
    return count;
...
TemporalDebtTracker::calculateMinimumSafeRepayment() const {
  return {0.0, 0.0};
}

std::map<std::string, std::pair<double, double>>
TemporalDebtTracker::calculateOptimalRepayment(double available_chronons,
...
double TemporalDebtTracker::getParadoxRisk() const {
  ...
  // A more sophisticated implementation might consider debt levels, critical
  // debts, stability, etc. For now, returning a fixed value or a simple
  // calculation based on existing members. double risk = (getOverallDebtLevel()
  // / m_debt_limit) * 0.5; // Example: half of debt ratio return std::min(1.0,
  // std::max(0.0, risk));
  return 0.0;
}
