...
namespace {
std::string getStringStat(const std::map<std::string, std::string>& stats, const std::string& key,
                          const std::string& defaultValue = "N/A") {
    auto it = stats.find(key);
    if (it != stats.end()) {
        return it->second;
    }
    std::cerr << "[Stat Helper] Key '" << key << "' not found in snapshot stats." << std::endl;
    return defaultValue;
}

double getDoubleStat(const std::map<std::string, std::string>& stats, const std::string& key,
    ...
    std::cerr << "[Stat Helper] Key '" << key << "' not found or conversion failed for double."
              << std::endl;
    return defaultValue;
}

long long getLongStat(const std::map<std::string, std::string>& stats, const std::string& key,
    ...
    std::cerr << "[Stat Helper] Key '" << key << "' not found or conversion failed for long long."
              << std::endl;
    return defaultValue;
}
}  // anonymous namespace
...
ResourceSnapshot ResourceSnapshot::takeSnapshot(
    ...
    std::cerr << "[Snapshot] Entered ResourceSnapshot::takeSnapshot. Context: " << context
              << std::endl;
    ...
    if (runtime) {
        std::cerr << "[Snapshot] Runtime pointer is valid. Accessing members..." << std::endl;
        try {
            std::cerr << "[Snapshot] Attempting runtime->getCycleNumber()..." << std::endl;
            long long cycle_num = runtime->getCycleNumber();
            std::cerr << "[Snapshot] runtime->getCycleNumber() successful. Value: " << cycle_num
                      << std::endl;
            std::cerr << "[Snapshot] Attempting to assign CURRENT_CYCLE..." << std::endl;
            ...
            std::cerr << "[Snapshot] Attempting runtime->getChrononsLevel()..." << std::endl;
            ...
            std::cerr << "[Snapshot] Attempting to assign CHRONONS_LEVEL..." << std::endl;
            snapshot.m_resource_stats["CHRONONS_LEVEL"] = std::to_string(chronons_level);
            std::cerr << "[Snapshot] Assigned CHRONONS_LEVEL. Value: "
                      << snapshot.m_resource_stats["CHRONONS_LEVEL"] << std::endl;

            std::cerr << "[Snapshot] Attempting runtime->getAethelLevel()..." << std::endl;
            double aethel_level = runtime->getAethelLevel();
            std::cerr << "[Snapshot] runtime->getAethelLevel() successful. Value: " << aethel_level
                      << std::endl;
            std::cerr << "[Snapshot] Attempting to assign AETHEL_LEVEL..." << std::endl;
            ...
            std::cerr << "[Snapshot] EXCEPTION while accessing runtime members: " << e.what()
                      << std::endl;
    ...
    if (debtTracker) {
        std::cerr << "[Snapshot] DebtTracker pointer is valid. Accessing members..." << std::endl;
        try {
            std::cerr << "[Snapshot] Attempting debtTracker->getOverallDebtLevel()..." << std::endl;
            double overall_debt = debtTracker->getOverallDebtLevel();
            std::cerr << "[Snapshot] debtTracker->getOverallDebtLevel() successful. Value: "
                      << overall_debt << std::endl;
            std::cerr << "[Snapshot] Attempting to assign OVERALL_DEBT_LEVEL..." << std::endl;
            snapshot.m_resource_stats["OVERALL_DEBT_LEVEL"] = std::to_string(overall_debt);
            std::cerr << "[Snapshot] Assigned OVERALL_DEBT_LEVEL. Value: "
                      << snapshot.m_resource_stats["OVERALL_DEBT_LEVEL"] << std::endl;

            std::cerr << "[Snapshot] Attempting debtTracker->getParadoxRisk()..." << std::endl;
            double paradox_risk = debtTracker->getParadoxRisk();
            std::cerr << "[Snapshot] debtTracker->getParadoxRisk() successful. Value: "
                      << paradox_risk << std::endl;
            std::cerr << "[Snapshot] Attempting to assign PARADOX_RISK..." << std::endl;
            ...
            std::cerr << "[Snapshot] Attempting debtTracker->getOperationCounts()..." << std::endl;
            auto op_counts = debtTracker->getOperationCounts();
            std::cerr << "[Snapshot] debtTracker->getOperationCounts() successful. Map size: "
                      << op_counts.size() << std::endl;
            std::cerr << "[Snapshot] Attempting to process operation counts..." << std::endl;
    ...
    if (optimizer) {
        std::cerr << "[Snapshot] Optimizer pointer is valid. Accessing members..." << std::endl;
        try {
            std::cerr << "[Snapshot] Attempting optimizer->getOverallEfficiency()..." << std::endl;
            double efficiency = optimizer->getOverallEfficiency();
            std::cerr << "[Snapshot] optimizer->getOverallEfficiency() successful. Value: "
                      << efficiency << std::endl;
            std::cerr << "[Snapshot] Attempting to assign OVERALL_EFFICIENCY..." << std::endl;
            snapshot.m_resource_stats["OVERALL_EFFICIENCY"] = std::to_string(efficiency);
            std::cerr << "[Snapshot] Assigned OVERALL_EFFICIENCY. Value: "
                      << snapshot.m_resource_stats["OVERALL_EFFICIENCY"] << std::endl;

            std::cerr << "[Snapshot] Attempting optimizer->getLastOptimizationSuggestion()..."
                      << std::endl;
            std::string suggestion = optimizer->getLastOptimizationSuggestion();
            std::cerr << "[Snapshot] optimizer->getLastOptimizationSuggestion() successful. Value: "
                      << suggestion << std::endl;
            std::cerr << "[Snapshot] Attempting to assign LAST_OPTIMIZATION_SUGGESTION..."
                      << std::endl;
            ...
            std::cerr << "[Snapshot] Assigned LAST_OPTIMIZATION_SUGGESTION. Value: "
                      << snapshot.m_resource_stats["LAST_OPTIMIZATION_SUGGESTION"] << std::endl;
        } catch (const std::exception& e) {
            std::cerr << "[Snapshot] EXCEPTION while accessing optimizer members: " << e.what()
                      << std::endl;
            snapshot.m_resource_stats["OVERALL_EFFICIENCY"] = "ERROR_OPTIMIZER";
            snapshot.m_resource_stats["LAST_OPTIMIZATION_SUGGESTION"] = "ERROR_OPTIMIZER";
        }
    } else {
        std::cerr << "[Snapshot] Optimizer pointer is NULL." << std::endl;
        snapshot.m_resource_stats["OVERALL_EFFICIENCY"] = "N/A (optimizer null)";
        snapshot.m_resource_stats["LAST_OPTIMIZATION_SUGGESTION"] = "N/A (optimizer null)";
    }
    std::cerr << "[Snapshot] Finished processing optimizer data." << std::endl;

    std::cerr << "[Snapshot] Exiting ResourceSnapshot::takeSnapshot." << std::endl;
    return snapshot;
}
...
{
    if (!m_runtime) {
        throw std::invalid_argument("TemporalRuntime cannot be null for ResourceVisualization");
    }
    if (!m_debt_tracker) {
        throw std::invalid_argument("TemporalDebtTracker cannot be null for ResourceVisualization");
    }
    if (!m_optimizer) {
        ...
        std::cerr << "Warning: ResourceOptimizer is null in ResourceVisualization constructor."
                  << std::endl;
    }
    if (!m_graph_visualizer) {
        ...
        // This matches the behavior in ResourceExportTest::SetUp
        // throw std::invalid_argument("GraphicalResourceVisualizer cannot be null for
        // ResourceVisualization");
        std::cerr
            << "Warning: GraphicalResourceVisualizer is null in ResourceVisualization constructor."
            << std::endl;
    }
    std::cerr << "ResourceVisualization constructed with "
              ...
              << (m_graph_visualizer ? "valid graph_visualizer." : "null graph_visualizer.")
              << std::endl;
}
...
ResourceVisualization::ResourceVisualization(std::shared_ptr<TemporalRuntime> runtime,
    ...
    // Create a ResourceTracker instance but store it separately to manage lifetime
...
ResourceVisualization::ResourceVisualization(ResourceTracker* tracker)
    ...
    ,  // Initialize m_resource_tracker
    m_runtime(nullptr)
    ,  // Default initialize other shared_ptrs
    m_debt_tracker(nullptr)
    , m_optimizer(nullptr)
    , m_graph_visualizer(nullptr) {
    if (!m_resource_tracker) {
        // Optionally, handle or log if tracker is null, but constructor signature allows it.
        // For now, we'll allow it to proceed. Some visualization methods might behave differently.
        std::cerr << "Warning: ResourceVisualization constructed with a null ResourceTracker."
                  << std::endl;
    }
    // If m_resource_tracker is expected to be owned, this constructor might need adjustment
    // or the class needs to handle non-owned m_resource_tracker appropriately.
    // For now, this fulfills the constructor signature.
}
...
ResourceVisualization::ResourceVisualization(std::shared_ptr<ResourceTracker> resourceTracker,
    ...
    , m_debt_tracker(debtTracker)
    , m_optimizer(nullptr)
    ,  // Default initialize optimizer
    m_graph_visualizer(nullptr)
    ,                                          // Default initialize graph_visualizer
    m_owned_resource_tracker(resourceTracker)  // Store shared_ptr for ownership
{
    if (!m_resource_tracker) {
        // This indicates the shared_ptr<ResourceTracker> was null
        std::cerr
            << "Warning: ResourceVisualization constructed with a null ResourceTracker shared_ptr."
            << std::endl;
    }
    if (!m_runtime) {
        std::cerr
            << "Warning: ResourceVisualization constructed with a null TemporalRuntime shared_ptr."
            << std::endl;
        // Depending on requirements, might throw std::invalid_argument here
    }
    if (!m_debt_tracker) {
        std::cerr << "Warning: ResourceVisualization constructed with a null TemporalDebtTracker "
                     "shared_ptr."
                  << std::endl;
        // Depending on requirements, might throw std::invalid_argument here
    }
    // Note: m_optimizer can be set later if needed, or a constructor providing it can be used.
}
...
std::string ResourceVisualization::generateTextReport() const {
    ...
    report << "Context: "
           << getStringStat(snapshot->m_resource_stats, "CONTEXT", snapshot->m_context)
           << std::endl;
    report << "Timestamp: " << getStringStat(snapshot->m_resource_stats, "TIMESTAMP", "N/A")
           << std::endl;
    report << "Cycle: " << getLongStat(snapshot->m_resource_stats, "CURRENT_CYCLE") << std::endl;
    report << "Aethel Usage: " << getDoubleStat(snapshot->m_resource_stats, "AETHEL_LEVEL")
           << std::endl;
    report << "Chronon Usage: " << getDoubleStat(snapshot->m_resource_stats, "CHRONONS_LEVEL")
           << std::endl;
    report << "Overall Efficiency: "
           << getDoubleStat(snapshot->m_resource_stats, "OVERALL_EFFICIENCY") << std::endl;
    report << "Overall Debt Level: "
           << getDoubleStat(snapshot->m_resource_stats, "OVERALL_DEBT_LEVEL") << std::endl;
    report << "Paradox Risk: " << getDoubleStat(snapshot->m_resource_stats, "PARADOX_RISK")
           << std::endl;
    report << "Last Suggestion: "
           << getStringStat(snapshot->m_resource_stats, "LAST_OPTIMIZATION_SUGGESTION")
           << std::endl;
    ...
    for (const auto& pair : snapshot->m_resource_stats) {
        if (pair.first.rfind(op_prefix, 0) == 0) {
            ...
            report << "  " << op_name << ": "
                   << getStringStat(snapshot->m_resource_stats, pair.first, "0") << std::endl;
            found_ops = true;
        }
    }
    if (!found_ops) {
...
bool ResourceVisualization::generateResourceGraph(const std::string& outputFile) const {
...
bool ResourceVisualization::generateTimelineVisualization(const std::string& outputFile) const {
    // Placeholder implementation - would require a visualization library
    // For now, just return false to indicate the feature is not available
    return false;
}

bool ResourceVisualization::exportRebelOperationImpactReport(RebelOperationType type,
    ...
    try {
        ...
        file << visualizeRebelOperationImpact(type, projection_cycles, operations_per_cycle,
                                              format);
        return true;
    } catch (...) {
        return false;
...
std::string ResourceVisualization::generateDetailedReport(VisualizationFormat format,
    ...
    if (format == VisualizationFormat::TEXT) {
        ...
        ss << "  Chronons: " << getDoubleStat(snapshot->m_resource_stats, "CHRONONS_LEVEL", 0.0)
           << std::endl;
        ss << "  Aethel: " << getDoubleStat(snapshot->m_resource_stats, "AETHEL_LEVEL", 0.0)
           << std::endl;
        ss << std::endl;

        ss << "Temporal Debt Information" << std::endl;
        ss << "  Total Chronon Debt: "
           << getDoubleStat(snapshot->m_resource_stats, "CHRONON_DEBT", 0.0) << std::endl;
        ss << "  Total Aethel Debt: "
           << getDoubleStat(snapshot->m_resource_stats, "AETHEL_DEBT", 0.0) << std::endl;
        ss << "  Overall Debt Level: "
           << getDoubleStat(snapshot->m_resource_stats, "OVERALL_DEBT_LEVEL", 0.0) << std::endl;
        ss << "  Paradox Risk: " << getDoubleStat(snapshot->m_resource_stats, "PARADOX_RISK", 0.0)
           << std::endl;
        ss << std::endl;

        ss << "Efficiency Metrics" << std::endl;
        ss << "  Overall Efficiency: "
           << getDoubleStat(snapshot->m_resource_stats, "OVERALL_EFFICIENCY", 1.0) << std::endl;
        ss << "  Last Suggestion: "
           << getStringStat(snapshot->m_resource_stats, "LAST_OPTIMIZATION_SUGGESTION",
        ...
        ss << "<ul>\n";
        ss << "  <li>Chronons: " << getDoubleStat(snapshot->m_resource_stats, "CHRONONS_LEVEL", 0.0)
           << "</li>\n";
        ss << "  <li>Aethel: " << getDoubleStat(snapshot->m_resource_stats, "AETHEL_LEVEL", 0.0)
           << "</li>\n";
        ss << "</ul>\n";
        ...
        ss << "  <li>Total Chronon Debt: "
           << getDoubleStat(snapshot->m_resource_stats, "CHRONON_DEBT", 0.0) << "</li>\n";
        ss << "  <li>Total Aethel Debt: "
           << getDoubleStat(snapshot->m_resource_stats, "AETHEL_DEBT", 0.0) << "</li>\n";
        ss << "  <li>Overall Debt Level: "
           << getDoubleStat(snapshot->m_resource_stats, "OVERALL_DEBT_LEVEL", 0.0) << "</li>\n";
        ss << "  <li>Paradox Risk: "
           << getDoubleStat(snapshot->m_resource_stats, "PARADOX_RISK", 0.0) << "</li>\n";
        ss << "</ul>\n";

        ss << "<h3>Efficiency Metrics</h3>\n";
        ss << "<ul>\n";
        ss << "  <li>Overall Efficiency: "
           << getDoubleStat(snapshot->m_resource_stats, "OVERALL_EFFICIENCY", 1.0) << "</li>\n";
        ss << "</ul>\n";
    }
    ...
    if (!history.empty()) {
        if (format == VisualizationFormat::TEXT) {
            ss << std::endl << "Historical Data (" << history.size() << " entries):" << std::endl;
            for (size_t i = 0; i < history.size(); ++i) {
                ...
                ss << "  Chronons: "
                   ...
                   << std::endl;
                ss << "  Aethel: "
                   << getDoubleStat(history[i]->m_resource_stats, "AETHEL_LEVEL", 0.0) << std::endl;
                ss << std::endl;
            }
        } else if (format == VisualizationFormat::JSON) {
            ss << ",\n    \"history\": [\n";
            ...
            for (size_t i = 0; i < history.size(); ++i) {
                ...
                ss << "  <li>Chronons: "
                   ...
                   << "</li>\n";
                ss << "  <li>Aethel: "
                   << getDoubleStat(history[i]->m_resource_stats, "AETHEL_LEVEL", 0.0) << "</li>\n";
                ss << "</ul>\n";
                ss << "</div>\n";
            }
            ss << "</div>\n";
...
std::string ResourceVisualization::visualizeDebtImpact(int projection_cycles,
    ...
    double currentDebt = getDoubleStat(snapshot->m_resource_stats, "OVERALL_DEBT_LEVEL", 0.0);
    double currentStability = getDoubleStat(snapshot->m_resource_stats, "PARADOX_RISK", 0.0);
    double chrononsDebt = getDoubleStat(snapshot->m_resource_stats, "CHRONONS_DEBT", 0.0);
    double aethelDebt = getDoubleStat(snapshot->m_resource_stats, "AETHEL_DEBT", 0.0);
    double rebelDebt = getDoubleStat(snapshot->m_resource_stats, "REBEL_OPS_DEBT", 0.0);
    ...
    double aggressiveRepaymentDebt = currentDebt * (1.0 - 0.03 * projection_cycles);

    // Calculate stability projections
    double noRepaymentStability = currentStability * (1.0 - 0.02 * projection_cycles);
    double minimalRepaymentStability = currentStability * (1.0 - 0.01 * projection_cycles);
    double aggressiveRepaymentStability = currentStability * (1.0 + 0.01 * projection_cycles);
    ...
    if (currentDebt > 75.0) {
        recommendation =
            "CRITICAL: Aggressive debt repayment strongly recommended to avoid paradox collapse.";
    } else if (currentDebt > 50.0) {
        recommendation =
            "WARNING: Debt levels are concerning. Implement regular repayment strategy.";
    } else if (currentDebt > 25.0) {
        recommendation = "CAUTION: Monitor debt levels closely and begin minimal repayment.";
    } else {
        recommendation = "STABLE: Current debt levels are manageable. Continue routine operations.";
    }
    ...
    switch (format) {
        case VisualizationFormat::TEXT:
            ...
            report << "Current Stability Metric: " << currentStability << std::endl << std::endl;

            report << "DEBT PROJECTION OVER " << projection_cycles << " CYCLES" << std::endl;
            report << "-----------------------------" << std::endl;
            report << "No Repayment Strategy: " << noRepaymentDebt << std::endl;
            report << "Minimal Repayment Strategy: " << minimalRepaymentDebt << std::endl;
            report << "Aggressive Repayment Strategy: " << aggressiveRepaymentDebt << std::endl
                   << std::endl;

            report << "STABILITY PROJECTION" << std::endl;
            report << "-------------------" << std::endl;
            report << "Current Stability: " << currentStability << std::endl;
            report << "Projected Stability (No Repayment): " << noRepaymentStability << std::endl;
            report << "Projected Stability (Minimal Repayment): " << minimalRepaymentStability
                   << std::endl;
            report << "Projected Stability (Aggressive Repayment): " << aggressiveRepaymentStability
                   << std::endl
                   << std::endl;
            ...
            report << "Resource Impact (Aggressive Repayment): " << aggressiveRepaymentImpact
                   << " units/cycle" << std::endl;
            report << "Operation Impact: Extended operation times, reduced efficiency" << std::endl
                   << std::endl;
        ...
        case VisualizationFormat::JSON:
            ...
            report << "      \"aggressive_repayment_impact\": " << aggressiveRepaymentImpact
                   << ",\n";
            report
                << "      \"operation_impact\": \"Extended operation times, reduced efficiency\"\n";
            report << "    },\n";
        ...
        case VisualizationFormat::HTML:
            ...
            report << "    <p>Current Stability Metric: " << currentStability << "</p>\n";
            report << "  </div>\n";

            report << "  <div class='section'>\n";
            report << "    <h2>DEBT PROJECTION OVER " << projection_cycles << " CYCLES</h2>\n";
            report << "    <p>No Repayment Strategy: " << noRepaymentDebt << "</p>\n";
            report << "    <p>Minimal Repayment Strategy: " << minimalRepaymentDebt << "</p>\n";
            report << "    <p>Aggressive Repayment Strategy: " << aggressiveRepaymentDebt
                   << "</p>\n";
            report << "  </div>\n";
            ...
            report << "    <p>Projected Stability (No Repayment): " << noRepaymentStability
                   << "</p>\n";
            ...
            report << "    <p>Resource Impact (Minimal Repayment): " << minimalRepaymentImpact
                   << " units/cycle</p>\n";
            report << "    <p>Resource Impact (Aggressive Repayment): " << aggressiveRepaymentImpact
                   << " units/cycle</p>\n";
            report << "    <p>Operation Impact: Extended operation times, reduced efficiency</p>\n";
            report << "  </div>\n";
...
void ResourceVisualization::setVisualizationTheme(const VisualizationTheme& theme) {
    // Simple implementation - in a real scenario, we would store the theme
}

// Implement generateASCIIChart
std::string ResourceVisualization::generateASCIIChart(const std::vector<double>& data, int width,
    ...
    double max_value = data.empty() ? 1.0 : *std::max_element(data.begin(), data.end());
    double min_value = data.empty() ? 0.0 : *std::min_element(data.begin(), data.end());
    ...
    for (int row = 0; row < height; ++row) {
        ...
        double row_value = max_value - (row * (max_value - min_value) / (height - 1));
        chart << std::setw(8) << std::fixed << std::setprecision(2) << row_value << " |";

        // Data points for this row
        for (size_t col = 0; col < data.size() && col < static_cast<size_t>(width); ++col) {
            double normalized_value = (data[col] - min_value) / (max_value - min_value);
...
std::string ResourceVisualization::generateASCIIGauge(double value, int width,
...
std::string ResourceVisualization::generateThemedBar(double value, int width) const {
...
VisualizationTheme VisualizationTheme::createTheme(ThemeType type) {
    ...
    switch (type) {
        ...
        case ThemeType::CUSTOM:
            // For custom theme, we just return the default and expect the caller to modify it
            break;
...
bool ResourceVisualization::startRealTimeMonitoring(
    ...
    m_monitor_thread = std::thread(&ResourceVisualization::monitorResources, this);
...
std::string ResourceVisualization::generateDashboardHeader(VisualizationFormat format) {
    ...
    switch (format) {
        ...
        default:
            ...
            header << "=======================================\n\n";
            header << "Update frequency: " << m_dashboard_config.update_frequency_ms << "ms\n";
...
std::string ResourceVisualization::generateDashboardFooter(VisualizationFormat format) {
...
std::string ResourceVisualization::generateResourceSection(VisualizationFormat format) {
...
std::string ResourceVisualization::generateDebtSection(VisualizationFormat format) {
    std::ostringstream section;
...
std::string ResourceVisualization::generateEfficiencySection(VisualizationFormat format) {
    std::ostringstream section;
...
std::string ResourceVisualization::generateTrendsSection(VisualizationFormat format) {
    std::ostringstream section;
...
bool ResourceVisualization::shouldUpdateDashboard() const {
    ...
    auto elapsed =
        std::chrono::duration_cast<std::chrono::milliseconds>(now - m_last_update_time).count();
...
std::string ResourceVisualization::visualizeTrends(int /*history_length*/,
                                                   VisualizationFormat /*format*/) {
    // In a previous step, we attempted to change takeSnapshot() calls here.
    // For now, ensuring a stub exists. Actual logic would call takeSnapshot("VisualizeTrends").
...
std::string ResourceVisualization::visualizeDebtStatus(VisualizationFormat /*format*/) {
    // In a previous step, we attempted to change takeSnapshot() calls here.
    // For now, ensuring a stub exists. Actual logic would call takeSnapshot("VisualizeDebtStatus").
...
std::string ResourceVisualization::visualizeOptimizationOpportunities(
    ...
    std::string suggestion =
        getStringStat(snapshot->m_resource_stats, "LAST_OPTIMIZATION_SUGGESTION", "None");
    return "[Stub] Optimization opportunities: " + suggestion;
}

std::string ResourceVisualization::visualizeRebelOperationImpact(RebelOperationType /*type*/,
                                                                 int /*projection_cycles*/,
                                                                 int /*operations_per_cycle*/,
                                                                 VisualizationFormat /*format*/) {
...
std::shared_ptr<ResourceSnapshot> ResourceVisualization::takeSnapshot(
    ...
    auto snapshot = std::make_shared<ResourceSnapshot>(ResourceSnapshot::takeSnapshot(
