...
// Constructor
TemporalDebtAlertSystem::TemporalDebtAlertSystem(std::shared_ptr<TemporalDebtTracker> debt_tracker,
...
bool TemporalDebtAlertSystem::startContinuousMonitoring() {
    ...
    monitoring_thread_ = std::thread(&TemporalDebtAlertSystem::monitoringThreadFunction, this);
...
bool TemporalDebtAlertSystem::checkDebtLevels() {
    ...
    if (!alerts_triggered && isDebtApproachingCritical()) {
        ...
        DebtAlert approaching_alert(DebtAlertLevel::MEDIUM,
...
void TemporalDebtAlertSystem::sendNotification(const DebtAlert& alert, AlertNotificationType type) {
    switch (type) {
        case AlertNotificationType::CONSOLE:
            // Output to console
            std::cout << "[DEBT ALERT] " << getAlertLevelString(alert.level) << ": "
                      << alert.message << std::endl;
            std::cout << "  Debt Ratio: " << std::fixed << std::setprecision(2) << alert.debt_ratio
                      << std::endl
                      << std::endl;
            break;
        ...
        case AlertNotificationType::EMAIL:
            ...
            // This is a placeholder - actual implementation would depend on the email system
            std::cerr << "Email notification not implemented" << std::endl;
            break;

        case AlertNotificationType::UI_POPUP:
            // UI popup would go here
            // This is a placeholder - actual implementation would depend on the UI system
            std::cerr << "UI popup notification not implemented" << std::endl;
            break;

        case AlertNotificationType::SOUND:
            // Sound alert would go here
            // This is a placeholder - actual implementation would depend on the sound system
...
void TemporalDebtAlertSystem::logAlertToFile(const DebtAlert& alert) const {
    ...
    // Add debt ratio
    logfile << " - Debt Ratio: " << std::fixed << std::setprecision(2) << alert.debt_ratio
            << " - Type: " << alert.debt_type << std::endl;

    logfile.close();
}

// Get a string representation of an alert level
std::string TemporalDebtAlertSystem::getAlertLevelString(DebtAlertLevel level) const {
    switch (level) {
...
// Set configuration
void TemporalDebtAlertSystem::setConfiguration(const AlertConfiguration& config) {
...
// Get configuration
AlertConfiguration TemporalDebtAlertSystem::getConfiguration() const { return config_; }

// Register a custom callback for specific alert levels
void TemporalDebtAlertSystem::registerAlertCallback(std::function<void(const DebtAlert&)> callback,
...
std::vector<AlertHistory> TemporalDebtAlertSystem::getAlertHistory(DebtAlertLevel level) const {
...
std::string TemporalDebtAlertSystem::generateAlertReport(bool include_history) const {
    ...
    report << "Generated: " << std::put_time(std::localtime(&now_time), "%Y-%m-%d %H:%M:%S")
           << std::endl
           << std::endl;
    ...
    report << "Current Debt Ratio: " << std::fixed << std::setprecision(2) << debt_ratio
           << std::endl;
    report << "Temporal Paradox Risk: " << std::fixed << std::setprecision(2) << paradox_risk << "%"
           << std::endl;
    report << "Projected Debt (in " << config_.projection_cycles << " cycles): " << std::fixed
           << std::setprecision(2) << projected_debt << std::endl
           << std::endl;
    ...
    if (include_history && !alert_history_.empty()) {
        ...
        for (size_t i = 0; i < alert_history_.size(); ++i) {
            ...
            report << "  [" << i << "] " << getTimestampString(history.timestamp) << " - "
                   << getAlertLevelString(history.alert.level) << ": " << history.alert.message;
...
double TemporalDebtAlertSystem::projectFutureDebt(int cycles) const {
    ...
    for (size_t i = 1; i < history_size; ++i) {
        ...
        double prev_amount = debt_records[i - 1].chronon_debt + debt_records[i - 1].aethel_debt;
        double curr_amount = debt_records[i].chronon_debt + debt_records[i].aethel_debt;
    ...
    double avg_growth_rate = (growth_samples > 0) ? (total_growth_rate / growth_samples) : 1.05;
...
std::string TemporalDebtAlertSystem::getDebtManagementRecommendations() const {
    ...
    if (current_ratio >= config_.critical_threshold || paradox_risk >= 0.8) {
        // Critical situation
        recommendations << "- CRITICAL: Immediate action required to prevent temporal paradox."
                        << std::endl;
        recommendations << "- Stop all rebel operations immediately." << std::endl;
        recommendations << "- Implement emergency debt repayment plan." << std::endl;
        recommendations << "- Consider requesting temporal bailout if available." << std::endl;
        recommendations << "- System stability at high risk - prepare contingency measures."
                        << std::endl;
    } else if (current_ratio >= config_.high_threshold || paradox_risk >= 0.5) {
        // High alert situation
        recommendations << "- URGENT: Implement immediate debt reduction measures." << std::endl;
        recommendations << "- Temporarily halt all non-essential temporal operations." << std::endl;
        recommendations << "- Consider accelerated repayment schedules." << std::endl;
        recommendations << "- Review all ongoing operations for optimization opportunities."
                        << std::endl;
    } else if (current_ratio >= config_.medium_threshold || paradox_risk >= 0.3) {
        // Medium alert situation
        recommendations << "- Implement a structured repayment plan to reduce debt." << std::endl;
        recommendations << "- Limit new temporal operations that would increase debt." << std::endl;
        recommendations << "- Review operation efficiency to minimize resource usage." << std::endl;
    } else if (current_ratio >= config_.low_threshold || paradox_risk >= 0.1) {
        // Low alert situation
        recommendations << "- Monitor debt levels closely." << std::endl;
        recommendations << "- Consider implementing proactive repayment strategies." << std::endl;
        recommendations << "- Review upcoming operations for potential debt impact." << std::endl;
    } else {
        // Normal situation
        recommendations << "- Debt levels are currently within acceptable parameters." << std::endl;
        recommendations << "- Continue regular monitoring and maintenance." << std::endl;
    }

    // Add recommendations based on projected debt
    if (projected_ratio > current_ratio + 0.1) {
        // Significant projected increase
        recommendations << "- WARNING: Debt is projected to increase significantly." << std::endl;
        recommendations << "- Preemptive action recommended to prevent future crisis." << std::endl;
    }
...
std::string TemporalDebtAlertSystem::visualizeDebtTrends() const {
    ...
    for (int y = 0; y < height; ++y) {
        ...
        for (int x = 0; x < width && x < static_cast<int>(scaled_values.size()); ++x) {
...
double TemporalDebtAlertSystem::calculateParadoxRisk() const {
    ...
    double debt_ratio = debt_tracker_->calculateDebtRatio();
    double base_risk =
        debt_ratio * debt_ratio;  // Quadratic scaling for more sensitivity at higher ratios
