...
TemporalDebtBenchmark::TemporalDebtBenchmark(
    std::shared_ptr<ResourceManager> resourceManager)
    : m_resourceManager(resourceManager),
      m_debtTracker(std::make_shared<TemporalDebtTracker>(
          resourceManager->getTemporalRuntime())) {}
...
void TemporalDebtBenchmark::addDebt(const TemporalDebt &debt) {
  m_debtTracker->addDebtForTesting(
      debt.id, debt.chronon_component, debt.aethel_component,
      debt.interest_rate, debt.age_in_cycles, debt.is_critical, debt.type, 1);
}
...
void TemporalDebtBenchmark::setAvailableResources(double chronons,
                                                  double aethel) {
  m_resourceManager->setChronons(chronons);
  m_resourceManager->setAethel(aethel);
}

TemporalDebtBenchmark::BenchmarkResult
TemporalDebtBenchmark::runStrategyBenchmark(RepaymentStrategyType strategy) {
  ...
  RepaymentResult repaymentResult = m_debtTracker->applyRepaymentStrategy(
      strategy, initialChronons, initialAethel);
  ...
  result.resourcesUsed =
      repaymentResult.chronons_repaid + repaymentResult.aethel_repaid;

  if (result.resourcesUsed > 0) {
    result.stabilityPerResource =
        result.stabilityImprovement / result.resourcesUsed;
  } else {
  ...
  for (const auto &debt : m_debtTracker->getAllDebts()) {
    if (debt.chronon_debt < initialChronons ||
        debt.aethel_debt < initialAethel) {
      partiallyPaid++;
    }
  }
...
std::vector<TemporalDebtBenchmark::BenchmarkResult>
TemporalDebtBenchmark::runAllStrategies() {
...
double
TemporalDebtBenchmark::compareStrategies(RepaymentStrategyType strategy1,
  ...
  double score1 = (0.5 * result1.stabilityImprovement) +
                  (0.3 * result1.stabilityPerResource) +
                  (0.2 * result1.debtsFullyPaid);

  double score2 = (0.5 * result2.stabilityImprovement) +
                  (0.3 * result2.stabilityPerResource) +
...
RepaymentStrategyType TemporalDebtBenchmark::findOptimalStrategy() {
  ...
  RepaymentStrategyType optimalStrategy =
      RepaymentStrategyType::BALANCED; // Default
  double highestScore = -1.0;

  for (const auto &result : results) {
    double score = (0.5 * result.stabilityImprovement) +
                   (0.3 * result.stabilityPerResource) +
...
std::string
TemporalDebtBenchmark::formatBenchmarkResult(const BenchmarkResult &result) {
  std::stringstream ss;
  ...
  ss << "Stability: " << std::fixed << std::setprecision(2)
     << result.initialStability << " → " << result.finalStability << " (+"
     << result.stabilityImprovement << ")\n";
  ss << "Resources Used: " << result.resourcesUsed << "\n";
  ss << "Efficiency: " << result.stabilityPerResource
     << " stability per resource\n";
  ss << "Execution Time: " << result.executionTime.count() << " μs\n";
  ss << "Debts: " << result.debtsFullyPaid << " fully paid, "
     << result.debtsPartiallyPaid << " partially paid, " << result.debtsUnpaid
     << " unpaid\n";
...
std::string TemporalDebtBenchmark::generateBenchmarkReport(
  ...
  report << "-----------------------------------------------\n";
  report << std::left << std::setw(25) << "Strategy" << std::right
         << std::setw(15) << "Stability +" << std::setw(15) << "Efficiency"
         << std::setw(10) << "Paid Off" << "\n";
  report << "-----------------------------------------------\n";
  ...
  for (const auto &result : sortedResults) {
    ...
    report << std::left << std::setw(25) << strategyName << std::right
           << std::fixed << std::setprecision(2) << std::setw(15)
           << result.stabilityImprovement << std::setw(15)
           << result.stabilityPerResource << std::setw(10)
