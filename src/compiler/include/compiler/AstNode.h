...
public:
  ...
  const std::vector<std::unique_ptr<DeclNode>> &getDeclarations() const {
    return declarations;
  }
...
public:
  BinaryExprNode(std::unique_ptr<ExprNode> left, Token op,
                 std::unique_ptr<ExprNode> right)
...
public:
  ExprStmtNode(std::unique_ptr<ExprNode> expression)
...
public:
  IfStmtNode(Token ifToken, std::unique_ptr<ExprNode> condition,
      ...
      : ifToken(std::move(ifToken)), condition(std::move(condition)),
...
public:
  WhileStmtNode(Token whileToken, std::unique_ptr<ExprNode> condition,
      ...
      : whileToken(std::move(whileToken)), condition(std::move(condition)),
        body(std::move(body)) {}

  SourceLocation getLocation() const override {
    return whileToken.getLocation();
  }
  void accept(AstVisitor &visitor) override;
...
public:
  VarDeclNode(Token name, std::unique_ptr<TypeNode> type,
...
public:
  ...
  ResourceStmtNode(Token keyword, ResourceOperation operation,
                   std::unique_ptr<ExprNode> chronons,
                   std::unique_ptr<ExprNode> aethel,
                   std::unique_ptr<StmtNode> body)
      : keyword(std::move(keyword)), operation(operation),
        chronons(std::move(chronons)), aethel(std::move(aethel)),
        body(std::move(body)) {}
...
/**
 * StmtDeclNode is a wrapper for statements that need to be treated as
 * declarations. This is used for handling statement expressions at the top
 * level of a program.
 */
...
public:
  ParallelExecutionStmtNode(Token keyword,
                            std::unique_ptr<ExprNode> threadCount,
                            std::unique_ptr<StmtNode> body)
      : keyword(std::move(keyword)), threadCount(std::move(threadCount)),
        body(std::move(body)) {}
...
public:
  BranchTimelineStmtNode(Token keyword, std::unique_ptr<ExprNode> branchCount,
                         std::unique_ptr<StmtNode> body)
      : keyword(std::move(keyword)), branchCount(std::move(branchCount)),
        body(std::move(body)) {}
