...
std::string StateVisualizer::visualizeState(const DebuggerState& state, const std::string& format) {
...
std::string StateVisualizer::visualizeHistory(const StateHistory& history,
    ...
    ss << "State History (" << history.getStateCount() << " states):" << std::endl;
...
std::string StateVisualizer::visualizeTemporalFlow(const StateHistory& history,
    ...
    for (const auto& [timelineId, states] : timelineStates) {
        ss << "Timeline " << timelineId << " (" << states.size() << " states):" << std::endl;

        for (size_t stateIndex : states) {
            const DebuggerState& state = history.getState(stateIndex);

            ss << "  State " << stateIndex << ": ";
            ss << state.getSourceLocation().file << ":" << state.getSourceLocation().line;
...
std::string StateVisualizer::formatState(const DebuggerState& state, const std::string& format) {
    std::stringstream ss;

    if (format == "json") {
        // JSON format
        ss << "{" << std::endl;
        ss << "  \"sourceFile\": \"" << state.getSourceLocation().file << "\"," << std::endl;
        ss << "  \"sourceLine\": " << state.getSourceLocation().line << "," << std::endl;
        ss << "  \"instructionPointer\": " << state.getInstructionPointer() << "," << std::endl;
        ...
        ss << "  \"isPotentialParadox\": " << (state.isPotentialParadox() ? "true" : "false")
           << std::endl;

        ss << "}";
    } else {
        // Default text format
        ss << "Source: " << state.getSourceLocation().file << ":" << state.getSourceLocation().line
           << std::endl;
        ...
        const auto& resources = state.getResourceUsage();
        for (const auto& [name, value] : resources) {
            ss << "  " << name << " = " << std::fixed << std::setprecision(2) << value << std::endl;
        }
