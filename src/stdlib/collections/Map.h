...
public:
  ...
  create(std::initializer_list<std::pair<const KeyType, ValueType>> init) {
    auto map = create();
  ...
  bool has(const KeyType &key) const override {
  ...
  ValueType reduce(const FunctionPtr &reducer,
                   ValueType initial = nullptr) const {
    if (!reducer)
      return NIL;
    ...
    for (const auto &[key, value] : map_) {
      accumulator =
          reducer->call({accumulator, value, String::create(key),
