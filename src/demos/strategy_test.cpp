...
void printDebts(const std::vector<DebtRecord>& debts) {
    ...
    std::cout << "-------------------------------------------------" << std::endl;
    std::cout << std::left << std::setw(15) << "Operation ID" << std::setw(15) << "Chronons"
              << std::setw(15) << "Aethel" << std::setw(15) << "Interest"
              << "Critical" << std::endl;
    std::cout << "-------------------------------------------------" << std::endl;

    for (const auto& debt : debts) {
        std::cout << std::left << std::setw(15) << debt.operation_id << std::fixed
                  << std::setprecision(2) << std::setw(15) << debt.chronon_debt << std::setw(15)
                  << debt.aethel_debt << std::setw(15) << debt.interest_rate * 100 << "% "
...
int main() {
    ...
    std::vector<DebtRecord> debts = {
        ...
        {"op3", 50.0, 200.0, 0.03, 7, false, OperationType::CAUSALITY_PRESERVATION},
        {"op4", 150.0, 100.0, 0.06, 2, true, OperationType::TIMELINE_MANIPULATION},
        {"op5", 80.0, 70.0, 0.04, 4, false, OperationType::TIMELINE_MANIPULATION}};

    // Add the debts to the tracker
    for (const auto& debt : debts) {
        tracker.addDebtForTesting(debt.operation_id, debt.chronon_debt, debt.aethel_debt,
    ...
    for (const auto& strategy_type : strategies) {
        ...
        // Apply the strategy
        auto result =
            tracker.applyRepaymentStrategy(strategy_type, available_chronons, available_aethel);
