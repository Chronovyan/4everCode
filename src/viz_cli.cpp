...
void printUsage() {
    ...
    std::cout << "  --report <type>      Generate a report of the specified type" << std::endl;
    std::cout << "  --format <format>    Specify the output format" << std::endl;
    std::cout << "  --output <file>      Save the output to the specified file" << std::endl;
    std::cout << "  --rebel-op <type>    Specify the rebel operation type for impact reports"
              << std::endl;
    std::cout << "  --projection <int>   Specify the number of cycles to project for debt impact"
              << std::endl;
    std::cout << "  --operations <int>   Specify the operations per cycle for rebel impact reports"
              << std::endl;

#ifndef CHRONOVYAN_NO_GRAPHICAL_VISUALIZATION
    // Only show graphical options if graphical visualization is enabled
    std::cout << "  --chart <type>       Generate a chart of the specified type" << std::endl;
    std::cout << "  --graph-format <fmt> Specify the graph output format (PNG, SVG, etc.)"
              << std::endl;
    std::cout << "  --graph-output <file> Save the graph to the specified file" << std::endl;
    std::cout << "  --resource <type>    Specify the resource type for resource charts"
              << std::endl;
    std::cout << "  --history <int>      Specify the history length for time series charts"
              << std::endl;
    std::cout << "  --title <string>     Specify the chart title" << std::endl;
    std::cout << "  --width <int>        Specify the chart width in pixels" << std::endl;
    std::cout << "  --height <int>       Specify the chart height in pixels" << std::endl;
#else
    std::cout << std::endl;
    std::cout << "Note: Graphical visualization is disabled in this build." << std::endl;
    std::cout << "Install Matplot++ library and rebuild to enable graphical features." << std::endl;
#endif

    std::cout << std::endl;
    std::cout << "Report Types:" << std::endl;
    std::cout << "  summary             Basic summary of current resource usage" << std::endl;
    std::cout << "  detailed            Detailed report with all metrics and historical data"
              << std::endl;
    std::cout << "  efficiency          Focus on efficiency metrics and optimization opportunities"
              << std::endl;
    std::cout << "  debt-analysis       Detailed analysis of temporal debt" << std::endl;
    std::cout << "  debt-impact         Debt impact over time with repayment strategies"
              << std::endl;
    std::cout << "  rebel-impact        Analysis of Rebel operation impact on debt and stability"
              << std::endl;
    std::cout << "  historical-trends   Analysis of resource usage trends over time" << std::endl;
    std::cout << "  complete            Complete report with all available information"
              << std::endl;

    std::cout << std::endl;
    std::cout << "Output Formats:" << std::endl;
    std::cout << "  text                Simple text output" << std::endl;
    std::cout << "  json                JSON formatted output for web interfaces" << std::endl;
    std::cout << "  csv                 CSV formatted output for spreadsheets" << std::endl;
    std::cout << "  compact             Compact single-line representation" << std::endl;
...
GraphOutputFormat getGraphOutputFormatFromString(const std::string& format_str) {
    if (format_str == "svg")
...
int main(int argc, char* argv[]) {
    ...
    auto visualization = std::make_shared<ResourceVisualization>(runtime, optimizer, debt_tracker);

#ifndef CHRONOVYAN_NO_GRAPHICAL_VISUALIZATION
    // Create the graphical resource visualizer
    auto graphical_visualization = std::make_shared<GraphicalResourceVisualizer>(visualization);
#endif
    ...
    GraphOutputFormat graph_format = getGraphOutputFormatFromString(graph_format_str);
#endif
    ...
    if (graphical) {
        ...
        std::cout << "Graphical visualization is not fully implemented yet." << std::endl;
        return 1;
    } else {
#endif
        ...
        if (report_type == ReportType::REBEL_IMPACT) {
            if (filename.empty()) {
                // Output to console
                output = visualization->visualizeRebelOperationImpact(rebel_op, projection_cycles,
                                                                      operations_per_cycle, format);
                std::cout << output << std::endl;
            } else {
                // Export to file
                export_result = visualization->exportRebelOperationImpactReport(
                    rebel_op, format, projection_cycles, operations_per_cycle, filename);
                if (export_result) {
            ...
            if (filename.empty()) {
                ...
                // Even without graphical visualization, we can still generate text reports
...
#else
            // Export to file
            export_result = visualization->exportReport(report_type, format, filename);
            if (export_result) {
