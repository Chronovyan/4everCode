...
ConstantFoldingPass::ConstantFoldingPass(DiagnosticReporter* diagnostics)
    : diagnostics(diagnostics), expressionReplacements(), statementReplacements() {}

std::string ConstantFoldingPass::getName() const { return "ConstantFolding"; }

std::string ConstantFoldingPass::getDescription() const {
    return "Evaluates constant expressions at compile time and replaces them with their computed "
           "values.";
}
...
StmtNode* ConstantFoldingPass::visitIfStmtNode(IfStmtNode& node) {
    ...
    LiteralExprNode* conditionLiteral = dynamic_cast<LiteralExprNode*>(condition);
    ...
    if (conditionLiteral) {
        ...
        if (conditionValue) {
            ...
            if (thenBlock) {
                ...
                statements.push_back(
                    std::make_unique<ExprStmtNode>(std::unique_ptr<ExprNode>(thenBranch)));
        ...
        // If condition is always false and there's an else branch, replace with the else branch
            ...
            if (elseBlock) {
                ...
                std::vector<std::unique_ptr<StmtNode>> statements;
                statements.push_back(
                    std::make_unique<ExprStmtNode>(std::unique_ptr<ExprNode>(elseBranch)));
        ...
        // If condition is always false and there's no else branch, replace with an empty block
        else {
...
StmtNode* ConstantFoldingPass::visitWhileStmtNode(WhileStmtNode& node) {
    ...
    if (conditionLiteral) {
        ...
        // If condition is always true, we should add a warning about an infinite loop
        // but we still need to visit the body for constant folding
        else if (diagnostics) {
            diagnostics->addWarning(node.getLocation(),
...
StmtNode* ConstantFoldingPass::visitParallelExecutionStmtNode(ParallelExecutionStmtNode& node) {
    // Visit all branches
...
StmtNode* ConstantFoldingPass::visitBranchTimelineStmtNode(BranchTimelineStmtNode& node) {
    // Visit the condition
...
bool ConstantFoldingPass::tryFoldBinaryExpr(BinaryExprNode* node) {
    // Check if both operands are literals
    LiteralExprNode* leftLiteral = dynamic_cast<LiteralExprNode*>(&node->getLeft());
    LiteralExprNode* rightLiteral = dynamic_cast<LiteralExprNode*>(&node->getRight());
    ...
    if ((std::holds_alternative<int64_t>(leftValue) || std::holds_alternative<double>(leftValue)) &&
...
bool ConstantFoldingPass::tryFoldUnaryExpr(UnaryExprNode* node) {
    // Check if the operand is a literal
    LiteralExprNode* operandLiteral = dynamic_cast<LiteralExprNode*>(&node->getRight());
...
std::variant<int64_t, double, std::string, bool> ConstantFoldingPass::evaluateLiteral(
    const LiteralExprNode* node) {
    return node->getValue();
}

void ConstantFoldingPass::replaceLiteral(AstNode* parent, ExprNode** expr,
                                         std::variant<int64_t, double, std::string, bool> value) {
    // This would replace the expression with a new literal node
    // For now, this is just a placeholder
}

void ConstantFoldingPass::replaceExpressionNode(ExprNode* oldNode,
...
void ConstantFoldingPass::replaceStatementNode(StmtNode* oldNode,
