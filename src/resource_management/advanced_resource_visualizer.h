...
public:
     ...
     * @param tracker Reference to the resource tracker containing data to visualize
     */
    ...
    /**
     * @brief Generate a combined visualization showing relationship between resources
     * @param format The desired output format
     * @param config Configuration options for the chart
     * @return String containing the visualization in the requested format
     */
    ...
    bool exportVisualization(
        ...
        VisualizationOutputFormat format = VisualizationOutputFormat::ASCII) const;

private:
    const ResourceTracker& tracker_;

    // Helper methods for generating different output formats
    std::string generateAsciiChart(const std::vector<double>& values, ChartType chart_type,
                                   const ChartConfiguration& config) const;

    std::string generateSvgChart(const std::vector<double>& values, ChartType chart_type,
                                 const ChartConfiguration& config) const;

    std::string generateHtmlChart(const std::vector<double>& values, ChartType chart_type,
                                  const ChartConfiguration& config) const;
    ...
    std::string generateAsciiChartFrame(const std::vector<std::string>& chart_rows,
                                        const ChartConfiguration& config) const;
