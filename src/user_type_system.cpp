...
FieldDefinition::FieldDefinition(const std::string &name,
                                 const std::string &type,
                                 VariableModifier modifier)
    : m_name(name), m_type(type), m_modifier(modifier),
      m_hasDefaultValue(false), m_required(true) {}

FieldDefinition::FieldDefinition(const std::string &name,
                                 const std::string &type,
                                 const Value &defaultValue,
                                 VariableModifier modifier, bool required)
    : m_name(name), m_type(type), m_defaultValue(defaultValue),
      m_modifier(modifier), m_hasDefaultValue(true), m_required(required) {}
...
MethodDefinition::MethodDefinition(
    ...
    std::function<Value(const std::vector<Value> &,
        ...
        implementation,
    bool isStatic)
    : m_name(name), m_paramTypes(paramTypes), m_returnType(returnType),
      m_implementation(implementation), m_isStatic(isStatic) {}

const std::string &MethodDefinition::getName() const { return m_name; }

const std::vector<std::string> &MethodDefinition::getParamTypes() const {
  return m_paramTypes;
}

const std::string &MethodDefinition::getReturnType() const {
  return m_returnType;
}
...
TypeBuilder::TypeBuilder(UserTypeSystem &typeSystem, const std::string &name)
    : m_typeSystem(&typeSystem), m_name(name), m_isUnion(false),
      m_temporalEnabled(false), m_predictiveEnabled(false) {}
...
TypeBuilder &TypeBuilder::addField(const std::string &fieldName,
...
std::shared_ptr<UserDefinedType> TypeBuilder::registerStructType() {
  ...
  auto type =
      std::make_shared<UserDefinedType>(m_name, UserDefinedType::Kind::STRUCT);
...
std::shared_ptr<UserDefinedType> TypeBuilder::registerEnumType() {
  ...
  auto type =
      std::make_shared<UserDefinedType>(m_name, UserDefinedType::Kind::ENUM);
...
std::shared_ptr<UserDefinedType> TypeBuilder::registerUnionType() {
  ...
  auto type =
      std::make_shared<UserDefinedType>(m_name, UserDefinedType::Kind::UNION);
...
std::shared_ptr<UserDefinedType> TypeBuilder::registerAliasType() {
  ...
  auto type =
      std::make_shared<UserDefinedType>(m_name, UserDefinedType::Kind::ALIAS);
...
UserTypeSystem::UserTypeSystem()
    : m_typeSystem(std::make_shared<CustomTypeSystem>()), m_runtime(nullptr),
      m_instanceCount(0) {}
...
TypeBuilder UserTypeSystem::createTypeBuilder(const std::string &name,
                                              UserTypeKind kind) {
...
UserTypeSystem::createInstance(const std::string &typeName,
...
UserTypeSystem::getType(const std::string &name) const {
...
std::shared_ptr<UserDefinedType>
UserTypeSystem::registerType(const std::string &name,
...
Value UserTypeSystem::getInstanceField(const Value &instance,
...
bool UserTypeSystem::setInstanceFieldAt(const Value &instance,
...
bool UserTypeSystem::setInstanceField(const Value &instance,
                                      const std::string &fieldName,
                                      const Value &value) {
  // Implementation
  return false;
...
Value UserTypeSystem::getInstanceAtCycle(const Value &instance,
...
UserDefinedType::UserDefinedType(const std::string &name, Kind kind)
    : m_name(name), m_kind(kind), m_temporalSupport(false),
      m_predictiveSupport(false) {}
...
void UserDefinedType::addField(const StructField &field) {
  m_fields.push_back(field);
}

void UserDefinedType::addVariant(const EnumVariant &variant) {
...
void UserDefinedType::setAliasTarget(const std::string &target) {
  m_aliasTarget = target;
}
...
bool UserDefinedType::hasPredictiveSupport() const {
  return m_predictiveSupport;
}

const std::vector<StructField> &UserDefinedType::getFields() const {
  return m_fields;
}

const std::vector<EnumVariant> &UserDefinedType::getVariants() const {
  return m_variants;
}
...
const std::string &UserDefinedType::getAliasTarget() const {
  return m_aliasTarget;
}

// InstanceData Implementation
InstanceData::InstanceData(std::shared_ptr<UserDefinedType> type)
    : m_type(type) {}

std::shared_ptr<UserDefinedType> InstanceData::getType() const {
  return m_type;
}
