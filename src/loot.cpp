...
// Loot implementation
Loot::Loot(const std::string &id, const std::string &name,
           const std::string &description, LootRarity rarity, LootEffect effect,
           double effectStrength)
    : m_id(id), m_name(name), m_description(description), m_rarity(rarity),
      m_effect(effect), m_effectStrength(effectStrength) {}
...
void Loot::setRecipe(const LootRecipe &recipe) {
  m_recipe = std::make_unique<LootRecipe>(recipe);
}
...
bool Loot::applyEffect(TemporalRuntime &runtime) const {
  ...
  case LootEffect::PARADOX_REDUCTION:
    // Reduce paradox level based on effect strength (1.0 = complete reduction)
    {
      ...
      int reduction =
          std::max(1, static_cast<int>(currentParadox * m_effectStrength));
      runtime.setParadoxLevel(std::max(0, currentParadox - reduction));
      std::cout << "Paradox level reduced by " << reduction << " points!"
                << std::endl;
      effectApplied = true;
    }
    break;
  ...
  case LootEffect::STABILITY_BOOST:
    // Increase timeline stability (implementation depends on runtime)
    {
      ...
      std::cout << "Timeline stability increased by " << std::fixed
                << std::setprecision(1) << (m_effectStrength * 100) << "%!"
                << std::endl;
      effectApplied = true;
    }
    break;
...
std::string Loot::getVisualRepresentation() const {
  ...
  if (m_effect != LootEffect::NONE) {
    ss << "Effect: " << effectToString(m_effect) << " (" << std::fixed
       << std::setprecision(0) << (m_effectStrength * 100) << "%)" << std::endl;
  }
...
LootSet::LootSet(const std::string &id, const std::string &name,
...
bool LootSet::isCompleted(
    const std::set<std::string> &discoveredLootIds) const {
...
bool LootSet::applySetBonus(TemporalRuntime &runtime) const {
  ...
  // For now, a simple implementation that applies each loot's effect with a 50%
  // strength bonus
  ...
  for (const auto &[id, loot] : m_lootItems) {
    ...
    if (loot->getEffect() != LootEffect::NONE &&
      ...
      auto boostedLoot = std::make_shared<Loot>(
          ...
          loot->getDescription(), loot->getRarity(), loot->getEffect(),
          boostedStrength);
...
bool LootCrafter::canCraft(const std::string &lootId,
...
bool LootCrafter::craftLoot(const std::string &lootId,
  ...
  if (!hasIngredients(*recipe, runtime)) {
    std::cout << "Missing ingredients for crafting " << loot->getName() << "."
              << std::endl;
    return false;
  }

  if (!hasResources(*recipe, runtime)) {
    std::cout << "Insufficient resources for crafting " << loot->getName()
              << "." << std::endl;
    return false;
  }
  ...
  if (!consumeResources(*recipe, runtime)) {
    ...
    // we'd need to restore them, but for simplicity we'll skip that in this
    // implementation
    return false;
  }
...
std::vector<std::shared_ptr<Loot>>
LootCrafter::getCraftableLoot(const TemporalRuntime &runtime) {
...
bool LootCrafter::hasIngredients(const LootRecipe &recipe,
                                 const TemporalRuntime &runtime) {
...
bool LootCrafter::hasResources(const LootRecipe &recipe,
                               const TemporalRuntime &runtime) {
  // Check if we have enough resources
  if (recipe.chrononCost > 0.0 &&
      runtime.getChrononsLevel() < recipe.chrononCost) {
    return false;
  }
  ...
  // Paradox cost is a bit different - it checks if adding the paradox would be
  // allowed For now, we'll always allow it

  return true;
}

bool LootCrafter::consumeIngredients(const LootRecipe &recipe,
                                     TemporalRuntime &runtime) {
...
bool LootCrafter::consumeResources(const LootRecipe &recipe,
                                   TemporalRuntime &runtime) {
  try {
    // Consume chronons if needed
    if (recipe.chrononCost > 0.0) {
      runtime.consumeChronons(recipe.chrononCost);
      std::cout << "Consumed " << recipe.chrononCost
                << " chronons for crafting." << std::endl;
    }

    // Consume aethel if needed
    if (recipe.aethelCost > 0.0) {
      runtime.consumeAethel(recipe.aethelCost);
      std::cout << "Consumed " << recipe.aethelCost << " aethel for crafting."
                << std::endl;
    }
    ...
    if (recipe.paradoxCost > 0) {
      ...
      std::cout << "Paradox increased by " << recipe.paradoxCost
                << " for crafting." << std::endl;
    }
...
std::shared_ptr<Loot>
LootTable::rollForLootWithRarityBias(double rarityModifier) const {
  if (m_lootEntries.empty()) {
    return nullptr;
  }
  ...
  if (rarityModifier > 0.0) {
    ...
    std::vector<std::pair<std::shared_ptr<Loot>, double>> sortedEntries =
        m_lootEntries;
    std::sort(sortedEntries.begin(), sortedEntries.end(),
              [](const auto &a, const auto &b) {
                return static_cast<int>(a.first->getRarity()) >
                       static_cast<int>(b.first->getRarity());
              });
...
const std::map<std::string, std::shared_ptr<Loot>> &
LootRegistry::getAllLoot() const {
  return m_lootRegistry;
}

std::vector<std::shared_ptr<Loot>>
LootRegistry::getLootByRarity(LootRarity rarity) const {
  std::vector<std::shared_ptr<Loot>> result;
...
std::shared_ptr<LootSet>
LootRegistry::getLootSetById(const std::string &id) const {
...
const std::map<std::string, std::shared_ptr<LootSet>> &
LootRegistry::getAllLootSets() const {
...
void LootJournal::recordDiscovery(const std::string &lootId,
                                  const TemporalRuntime &runtime,
                                  bool wasCrafted) {
  ...
  LootDiscovery discovery(lootId, runtime.getParadoxLevel(),
                          runtime.getChrononsLevel(), runtime.getAethelLevel(),
                          wasCrafted);
...
int LootJournal::getTotalDiscoveryCount() const {
  return m_discoveredLootIds.size();
}
...
void LootJournal::registerAchievement(const Achievement &achievement) {
  ...
  std::cout << "LootJournal: Registered achievement: " << achievement.name
            << std::endl;
}
...
void LootJournal::updateAchievements(const TemporalRuntime &runtime) {
  for (auto &achievement : m_achievements) {
    if (!achievement.isEarned &&
        checkAchievementCondition(achievement, runtime)) {
...
const std::vector<Achievement> &LootJournal::getAchievements() const {
...
std::string LootJournal::generateSummaryReport() const {
  ...
  for (int i = 0; i <= static_cast<int>(LootRarity::MYTHIC); i++) {
    ...
    ss << "  " << std::left << std::setw(10) << Loot::rarityToString(rarity)
       << ": " << count << " / " << lootOfRarity.size() << std::endl;
  }
  ...
  for (const auto &achievement : earned) {
    ...
    auto time =
        std::chrono::system_clock::to_time_t(achievement.earnedTimestamp);
...
bool LootJournal::checkAchievementCondition(Achievement &achievement,
  ...
  case AchievementType::RARITY_COLLECTION: {
    ...
    for (const auto &loot : lootOfRarity) {
      if (m_discoveredLootIds.find(loot->getId()) ==
          m_discoveredLootIds.end()) {
        return false;
      }
    }
