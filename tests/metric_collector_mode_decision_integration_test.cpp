...
protected:
    void SetUp() override {
        ...
        collector = std::make_unique<MetricCollector>(cpu_source.get(), memory_source.get(),
                                                      gpu_source.get());
...
TEST_F(MetricCollectorModeDecisionIntegrationTest, HandlesIntermittentMetricAvailability) {
    ...
    EXPECT_TRUE(decision3.reason.find("recovered") !=
                std::string::npos);  // Changed from hysteresis to recovered
}
...
TEST_F(MetricCollectorModeDecisionIntegrationTest, HandlesNaNMetrics) {
    ...
    EXPECT_CALL(*cpu_source, getValue()).WillOnce(Return(nan_value));
    EXPECT_CALL(*memory_source, getValue()).WillOnce(Return(60.0));
    EXPECT_CALL(*gpu_source, getValue()).WillOnce(Return(75.0));
    ...
    std::cout << "Debug - Decision: mode=" << static_cast<int>(decision.mode) << ", reason='"
              << decision.reason << "'"
...
TEST_F(MetricCollectorModeDecisionIntegrationTest, HandlesRapidMetricChanges) {
    ...
    EXPECT_TRUE(decision2.reason.find("high_load") !=
                std::string::npos);  // Changed from hysteresis to high_load
}
