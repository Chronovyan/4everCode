...
public:
  ChronovyanFunction(const FunctionDeclStmtNode *declaration)
...
Interpreter::Interpreter()
    : m_globals(std::make_shared<Environment>()), m_environment(m_globals),
      ...
      m_lootSystem(std::make_shared<LootSystem>()), m_isReturning(false),
      m_breakChronon_flag(false), m_continueWeave_flag(false),
      m_reverseFlow_flag(false), m_haltRewind_flag(false),
      m_preventModification_flag(false), m_readOnlyMode(false),
      m_stabilizationFactor(0.0), m_flowDirection(1),
      m_inParallelExecution(false), m_currentThreadId(0), m_totalThreads(1),
      m_aethel(1000.0), m_chronons(1000.0) {
  initializeGlobals();
...
Value Interpreter::interpret(const ProgramNode &program) {
  try {
    ...
    ErrorHandler::getInstance().reportError(
        SourceLocation(), "Runtime error: " + std::string(e.what()));
    return Value(); // Return nil
  }
}
...
std::shared_ptr<ResourceOptimizer> Interpreter::getResourceOptimizer() const {
  return m_optimizer;
}

std::shared_ptr<TemporalDebtTracker>
Interpreter::getTemporalDebtTracker() const {
  return m_debt_tracker;
}

std::shared_ptr<CustomTypeSystem> Interpreter::getTypeSystem() const {
  return m_typeSystem;
}

std::shared_ptr<UserTypeSystem> Interpreter::getUserTypeSystem() const {
  return m_userTypeSystem;
}

std::shared_ptr<Environment> Interpreter::getGlobals() const {
  return m_globals;
}

std::shared_ptr<Environment> Interpreter::getEnvironment() const {
  return m_environment;
}

Value Interpreter::getLastValue() const { return m_lastValue; }

std::shared_ptr<ResourceVisualization>
Interpreter::getResourceVisualization() const {
...
void Interpreter::visitBinaryExpr(const BinaryExprNode &expr) {
  ...
  case TokenType::LESS_EQUAL:
    if (left.isNumeric() && right.isNumeric()) {
      ...
      throw std::runtime_error(
          "Less than or equal operator requires numeric operands");
    }
    break;
  ...
  case TokenType::GREATER:
    if (left.isNumeric() && right.isNumeric()) {
      ...
      throw std::runtime_error(
          "Greater than operator requires numeric operands");
    }
    break;
...
void Interpreter::visitCallExpr(const CallExprNode &expr) {
  ...
  if (callee.isNativeFunction()) {
    ...
    ErrorHandler::getInstance().reportError(
        expr.getLocation(), "Can only call native functions at this time");
  }
}
...
void Interpreter::visitTemporalOpStmt(const TemporalOpStmtNode &stmt) {
  ...
  case TemporalOpType::TEMPORAL_ECHO_LOOP:
    ...
    break;
  default:
    throw ChronovyanRuntimeError("Unknown temporal operation",
                                 stmt.getLocation());
  }
}
...
void Interpreter::visitFunctionDeclStmt(const FunctionDeclStmtNode &stmt) {
  ...
  m_globals->define(stmt.getName(),
                    Value(std::make_shared<ChronovyanFunction>(&stmt)));
}
...
void Interpreter::visitTemporalSection(const TemporalSectionNode &section) {
  ...
  std::cout << "Temporal section encountered (stub implementation)"
            << std::endl;
}
...
void Interpreter::executeTemporalLoop(const TemporalSectionNode &section) {
  ...
  if (section.getType() != TemporalSectionType::EXECUTION_WEAVE) {
    ErrorHandler::getInstance().reportError(
        section.getLocation(), "Attempted to execute a temporal loop on a "
                               "section that is not an EXECUTION_WEAVE.");
    return;
  }

  const auto *bodyContent =
      dynamic_cast<const BlockStmtNode *>(&section.getContent());
  if (!bodyContent) {
    ErrorHandler::getInstance().reportError(
        section.getLocation(), "Content of EXECUTION_WEAVE section is not a "
                               "valid block statement for temporal loop.");
    return;
  }
  ...
  while (continueLoop && iteration < maxIterations) {
    ...
    if (iteration >= maxIterations) {
      ErrorHandler::getInstance().reportWarning(
          section.getLocation(), "Temporal loop hit maximum iteration limit "
...
void Interpreter::registerEventHandler(const TemporalSectionNode &section) {
  ...
  if (section.getType() == TemporalSectionType::NAME) {
    const auto *nameContentNode =
        dynamic_cast<const LiteralExprNode *>(&section.getContent());
    if (nameContentNode) {
      ...
      if (std::holds_alternative<std::string>(literalVal)) {
        ...
        ErrorHandler::getInstance().reportError(
            section.getLocation(), "Event name content in temporal NAME "
                                   "section is not a string literal.");
        return;
      }
    } else {
      ErrorHandler::getInstance().reportError(
          section.getLocation(), "Content of temporal NAME section is not a "
                                 "literal expression for event name.");
      return;
    }
  } else {
    ...
    if (m_debugLogging) {
      std::cout << "Attempted to register event handler for a non-NAME "
                   "section. Type: "
  ...
  if (m_debugLogging) {
    std::cout << "Registered event handler for '" << eventName << "'"
              << std::endl;
  }
}
...
Value Interpreter::lookUpVariable(const std::string &name,
  ...
  try {
    ...
    ErrorHandler::getInstance().reportError(location, "Undefined variable '" +
                                                          name + "'");
    throw;
  }
}
...
void Interpreter::executeForChronon(
    [[maybe_unused]] const TemporalOpStmtNode &stmt) {
  // Placeholder implementation
}

void Interpreter::executeWhileEvent(
    [[maybe_unused]] const TemporalOpStmtNode &stmt) {
  // Placeholder implementation
}

void Interpreter::executeRewindFlow(const TemporalOpStmtNode &stmt) {
  // Simplified implementation until TemporalOpStmtNode attributes are fully
  // implemented Track resource usage for the REWIND_FLOW operation
  double cost = calculateTemporalResourceCost(TemporalOperation::REWIND_FLOW);
  consumeResources(cost);

  // Log the operation if in debug mode
  if (m_debugLogging) {
    std::cout << "Executing REWIND_FLOW operation with cost: " << cost
              << std::endl;
  }
  ...
  // (Body execution would go here once the TemporalOpStmtNode is updated to
  // support it)
}

void Interpreter::executeBranchTimeline(
    [[maybe_unused]] const TemporalOpStmtNode &stmt) {
  // Placeholder implementation
}

void Interpreter::executeMergeTimelines(
    [[maybe_unused]] const TemporalOpStmtNode &stmt) {
  // Placeholder implementation
}

void Interpreter::executeTemporalEchoLoop(
    [[maybe_unused]] const TemporalOpStmtNode &stmt) {
  // Placeholder implementation
}
...
void Interpreter::defineNativeFunctions() {
                    ...
                    Value(std::make_shared<NativeFunction>(
                        []([[maybe_unused]] Interpreter &interpreter,
                          ...
                          if (args.empty()) {
                            ...
                            std::cout << "***** OUTPUT: " << args[0].toString()
                                      << " *****" << std::endl;
                          }
                          return Value(); // Return nil
                        },
      ...
      Value(std::make_shared<NativeFunction>(
          []([[maybe_unused]] Interpreter &interpreter,
            ...
            if (args.empty() || !args[0].isNumeric()) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(), "floor() requires a numeric argument");
              return Value(); // nil
            }
  ...
  m_globals->define("GET_CHRONONS_LEVEL",
                    Value(std::make_shared<NativeFunction>(
                        [](Interpreter &interpreter,
                           const std::vector<Value> & /*args*/) -> Value {
                          return Value(static_cast<int64_t>(
                              interpreter.getRuntime()->getChrononsLevel()));
                        },
                        0 // No arguments
                        )));

  // GET_AETHEL_LEVEL function
  m_globals->define("GET_AETHEL_LEVEL",
                    Value(std::make_shared<NativeFunction>(
                        [](Interpreter &interpreter,
                           const std::vector<Value> & /*args*/) -> Value {
                          return Value(static_cast<int64_t>(
                              interpreter.getRuntime()->getAethelLevel()));
                        },
                        0 // No arguments
                        )));
      ...
      Value(std::make_shared<NativeFunction>(
          [](Interpreter &interpreter,
            ...
            double optimizationFactor =
                interpreter.getResourceOptimizer()->optimizeChronons(
                    operationId);
  ...
  m_globals->define(
      ...
      Value(std::make_shared<NativeFunction>(
          [](Interpreter &interpreter,
             [[maybe_unused]] const std::vector<Value> &args) -> Value {
            auto analysis =
                interpreter.getResourceOptimizer()->analyzeResourceUsage();
  ...
  m_globals->define(
      ...
      Value(std::make_shared<NativeFunction>(
          [](Interpreter &interpreter,
            ...
            if (args.empty() || !args[0].isNumeric()) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(), "PREDICT_RESOURCE_NEEDS() requires a steps "
                                    "parameter (integer)");
              return Value();
            }
            ...
            auto predictions =
                interpreter.getResourceOptimizer()->predictResourceNeeds(steps);
            ...
            for (const auto &[chronons, aethel] : predictions) {
              ...
              resultArray.push_back(
                  Value(std::make_shared<ChronovyanMap>(predictionMap)));
            }
  ...
  m_globals->define(
      ...
      Value(std::make_shared<NativeFunction>(
          [this]([[maybe_unused]] Interpreter &interpreter,
            ...
            m_visualization
                ->takeSnapshot(); // Take a snapshot after advancing the cycle
            return Value();       // Return nil
          },
  ...
  m_globals->define(
      ...
      Value(std::make_shared<NativeFunction>(
          [](Interpreter &interpreter,
            ...
            map->set("total_chronon_debt",
                     Value(debt_tracker->getTotalChrononsDebt()));
            map->set("total_aethel_debt",
                     Value(debt_tracker->getTotalAethelDebt()));
            ...
            map->set("chronon_debt_count",
                     Value(static_cast<int64_t>(chronon_debt_count)));
            map->set("aethel_debt_count",
                     Value(static_cast<int64_t>(aethel_debt_count)));
            map->set("critical_debts",
                     Value(static_cast<int64_t>(critical_debt_count)));
            ...
            map->set("past_due_count",
                     Value(static_cast<int64_t>(
                         debt_tracker->getPastDueDebts().size())));
            ...
            environment->setProperty(
                ...
                Value(std::make_shared<NativeFunction>(
                    []([[maybe_unused]] Interpreter &interpreter,
                      ...
                      std::cout << "Setting time delta to " << args[1].asFloat()
                                << std::endl;
                      return Value(true);
                    },
  ...
  m_globals->define(
      ...
      Value(std::make_shared<NativeFunction>(
          [this]([[maybe_unused]] Interpreter &interpreter,
            ...
            if (args.size() >= 1 &&
                (args[0].isInteger() || args[0].isFloat())) {
  ...
  m_globals->define(
      "VISUALIZE_OPTIMIZATION_OPPORTUNITIES",
      Value(std::make_shared<NativeFunction>(
          [this]([[maybe_unused]] Interpreter &interpreter,
                 const std::vector<Value> &args) -> Value {
            VisualizationFormat format = VisualizationFormat::TEXT;
  ...
  m_globals->define("PRINT_CONSOLE",
                    Value(std::make_shared<NativeFunction>(
                        []([[maybe_unused]] Interpreter &interpreter,
                           const std::vector<Value> &args) -> Value {
                          if (args.empty()) {
                            ...
                            std::cout
                                << "[CONSOLE OUTPUT]: " << args[0].toString()
                                << std::endl;
                          }
                          return Value(); // Return nil
                        },
                    ...
                    Value(std::make_shared<NativeFunction>(
                        []([[maybe_unused]] Interpreter &interpreter,
                          ...
                          if (args.empty()) {
                            ...
                            std::cerr << "[LOG OUTPUT]: " << args[0].toString()
                                      << std::endl;
                          }
                          return Value(); // Return nil
                        },
...
Value Interpreter::handleVariableInteraction(
    [[maybe_unused]] const Value &left, [[maybe_unused]] const Value &right,
    [[maybe_unused]] TokenType operation) {
...
void Interpreter::visitTypeDefStmt(const TypeDefStmtNode &stmt) {
  ...
  // For unions, collect type names
    ...
    for (const auto &type : stmt.getUnionTypes()) {
      ...
      if (memberTypeName.empty()) {
        ErrorHandler::getInstance().reportError(
            stmt.getLocation(), "Member type in union has an empty name");
        continue;
      }
      ...
      if (uniqueTypes.find(memberTypeName) != uniqueTypes.end()) {
        std::stringstream ss;
        ss << "Duplicate member type '" << memberTypeName << "' in union '"
           << typeName << "'";
        ErrorHandler::getInstance().reportWarning(stmt.getLocation(), ss.str());
        continue;
      }
  ...
  if (stmt.isGeneric()) {
    ...
    for (const auto &param : stmt.getTypeParameters()) {
      ...
      if (param.empty()) {
        ErrorHandler::getInstance().reportError(
            stmt.getLocation(), "Type parameter has an empty name");
        continue;
      }
  ...
  if (typeName == "nil" || typeName == "bool" || typeName == "int" ||
      typeName == "float" || typeName == "string" || typeName == "Array" ||
      typeName == "Map" || typeName == "Function" || typeName == "Object" ||
      typeName == "Any") {
    std::stringstream ss;
    ss << "'" << typeName
       << "' is a reserved type name and cannot be redefined";
    ErrorHandler::getInstance().reportError(stmt.getLocation(), ss.str());
    return;
  }
  ...
  auto typeDef = std::make_shared<CustomTypeDefinition>(
      typeName, kind, properties, stmt.getTypeParameters());
                    ...
                    Value(std::make_shared<NativeFunction>(
                        [this]([[maybe_unused]] Interpreter &interpreter,
                          ...
                          if (args.empty()) {
                            ErrorHandler::getInstance().reportError(
                                SourceLocation(),
                                "TYPE_OF() requires a value parameter");
                            return Value("nil");
                          }
  ...
  m_globals->define(
      ...
      Value(std::make_shared<NativeFunction>(
          [](Interpreter &interpreter,
            ...
            if (!args[1].isString()) {
              std::stringstream ss;
              ss << "IS_TYPE() second parameter must be a string type name, "
                    "got "
                 << args[1].getTypeName();
            ...
            if (!typeName.empty() && typeName != "nil" && typeName != "bool" &&
                ...
                typeName != "string" && typeName != "Array" &&
                typeName != "Map" && typeName != "Function" &&
            ...
            return Value(
                interpreter.getTypeSystem()->checkType(value, typeName));
          },
          2 // Arity
          )));
  ...
  m_globals->define(
      ...
      Value(std::make_shared<NativeFunction>(
          [](Interpreter &interpreter,
            ...
            if (!args[0].isString()) {
              std::stringstream ss;
              ss << "CREATE_STRUCT() first parameter must be a string type "
                    "name, got "
                 << args[0].getTypeName();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
            ...
            if (typeName.empty()) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(),
                  "CREATE_STRUCT() requires a non-empty type name");
              return Value();
            }
            ...
            auto typeDef =
                interpreter.getTypeSystem()->getTypeDefinition(typeName);
            ...
            if (args.size() > 1) {
              if (!args[1].isMap()) {
                std::stringstream ss;
                ss << "CREATE_STRUCT() second parameter must be a map of field "
                      "values, got "
                   << args[1].getTypeName();
                ErrorHandler::getInstance().reportError(SourceLocation(),
                                                        ss.str());
            ...
            for (const auto &[fieldName, fieldProps] : fieldsValue.asMap()) {
              if (!fieldProps.isMap()) {
                std::stringstream ss;
                ss << "Invalid field definition for '" << fieldName
                   << "' in struct '" << typeName
                   << "': field properties not a map";
                ErrorHandler::getInstance().reportWarning(SourceLocation(),
                                                          ss.str());
                continue;
              }
              ...
              if (fieldValues.find(fieldName) != fieldValues.end()) {
                ...
                if (fieldPropsMap.find("type") != fieldPropsMap.end()) {
                  ...
                  if (!interpreter.getTypeSystem()->checkType(
                          fieldValues[fieldName], fieldType)) {
                    std::stringstream ss;
                    ss << "Type mismatch for field '" << fieldName
                       << "' in struct '" << typeName << "': expected "
                       << fieldType << ", got "
  ...
  m_globals->define(
      ...
      Value(std::make_shared<NativeFunction>(
          [](Interpreter &interpreter,
            ...
            if (args.size() < 2) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(), "CREATE_ENUM_VALUE() requires two "
                                    "parameters: a type name and a variant "
                                    "name");
              return Value();
            }
            ...
            if (typeName.empty()) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(),
                  "CREATE_ENUM_VALUE() requires a non-empty type name");
              return Value();
            }
            ...
            if (variantName.empty()) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(),
                  "CREATE_ENUM_VALUE() requires a non-empty variant name");
              return Value();
            }
            ...
            if (variantsValue.asMap().find(variantName) ==
                variantsValue.asMap().end()) {
              std::stringstream ss;
              ss << "Unknown variant '" << variantName << "' in enum type '"
                 << typeName << "'. Available variants: ";
...
void Interpreter::visitBreakChrononStmt(const BreakChrononStmtNode &node) {
  ...
  // std::cout << "BREAK_CHRONON executed at " << node.getLocation().toString()
  // << std::endl;
}
...
void Interpreter::visitContinueWeaveStmt(const ContinueWeaveStmtNode &node) {
  ...
  // For debugging/tracing purposes, you can add a message
  // std::cout << "CONTINUE_WEAVE executed at " << node.getLocation().toString()
  // << std::endl;
}
...
void Interpreter::visitReverseFlowStmt(const ReverseFlowStmtNode &node) {
  ...
  //           << ", new direction: " << (m_flowDirection > 0 ? "forward" :
  //           "backward") << std::endl;
}
...
void Interpreter::visitHaltRewindStmt(const HaltRewindStmtNode &stmt) {
  // Log the operation if in debug mode
  if (m_debugLogging) {
    std::cout << "Executing HALT_REWIND at " << stmt.getLocation().toString()
              << std::endl;
  }
...
void Interpreter::visitStabilizeTimelineStmt(
    const StabilizeTimelineStmtNode &stmt) {
  // Log the operation if in debug mode
  if (m_debugLogging) {
    std::cout << "Executing STABILIZE_TIMELINE at "
              << stmt.getLocation().toString() << std::endl;
  }
  ...
  if (m_debugLogging) {
    std::cout << "Timeline stabilization set to intensity: " << intensity
              << std::endl;
  }
}

void Interpreter::visitPreventModificationStmt(
    const PreventModificationStmtNode &stmt) {
  // Log the operation if in debug mode
  if (m_debugLogging) {
    std::cout << "Executing PREVENT_MODIFICATION at "
              << stmt.getLocation().toString() << std::endl;
  }
  ...
  if (stmt.hasScope()) {
    ...
    if (scopeValue.getType() == Value::Type::STRING) {
      ...
      if (scope != "local" && scope != "global" && scope != "timeline") {
        throw std::runtime_error("Invalid scope for PREVENT_MODIFICATION. Must "
                                 "be 'local', 'global', or "
                                 "'timeline'.");
      }
    } else {
  ...
  if (m_debugLogging) {
    std::cout << "Modification prevention enabled with scope: " << scope
              << std::endl;
  }
}

void Interpreter::trackResourceUsage(double aethel, double chronons,
  ...
  if (m_debugLogging || m_debugMode) { // Use both flags for compatibility
    std::cout << "Resource usage - Operation: " << operation
              << " | Aethel: " << aethel << " | Chronons: " << chronons
              << std::endl;
  ...
  // Update visualization (commented out until visualization is fully
  // implemented) if (m_visualization) {
  //     m_visualization->recordResourceUsage(operation, aethel, chronons);
  // }
}

// Implement resource tracking method
// REMOVED: Duplicate implementation of trackResourceUsage was here (BUILD-005
// fix)

// Implementation of visitor methods for resource management

void Interpreter::visitExpendResourcesStmt(
    const ExpendResourcesStmtNode &node) {
  ...
  // Check if we have enough resources
  bool hasEnoughResources = m_environment->checkResourceAvailability(
      aethelCost, chrononCost, node.getLocation());
  ...
  if (hasEnoughResources) {
    ...
    throw ChronovyanRuntimeError(
        "Insufficient resources: required " + std::to_string(aethelCost) +
            " aethel and " + std::to_string(chrononCost) + " chronon",
        node.getLocation());
  }
}
...
void Interpreter::visitResourceInsufficiencyHandler(
  ...
  // (Handler body execution would go here once the
  // ResourceInsufficiencyHandlerNode is updated to support it)
}
...
void Interpreter::visitForChrononStmt(const ForChrononStmtNode &node) {
  ...
  if (!startValue.isNumeric() || !endValue.isNumeric() ||
    ...
    ErrorHandler::getInstance().reportError(
        node.getLocation(),
        "FOR_CHRONON loop requires numeric range and step values");
    return;
  }
  ...
  double step =
      stepValue.asFloat() * m_flowDirection; // Apply direction to step
...
void Interpreter::visitWhileEventStmt(const WhileEventStmtNode &node) {
  ...
  while (true) {
    ...
    if (!conditionValue.isBoolean()) {
      ErrorHandler::getInstance().reportError(
          node.getLocation(),
          "WHILE_EVENT condition must evaluate to a boolean value");
      return;
    }
...
void Interpreter::visitRewindFlowStmt(const RewindFlowStmtNode &stmt) {
  ...
  double oldStabilizationFactor =
      m_stabilizationFactor; // Track previous stabilization
  ...
  m_preventModification_flag =
      false;                   // Initialize the PREVENT_MODIFICATION flag
  ...
  if (m_debugLogging || m_debugMode) {
    std::cout << "Starting REWIND_FLOW at " << stmt.getLocation().toString()
              << std::endl;
  }
  ...
  if (stmt.hasMaxRewinds()) {
    ...
    if (maxRewindsValue.getType() != Value::Type::NUMBER ||
        !maxRewindsValue.isInteger()) {
    ...
    if (m_debugLogging || m_debugMode) {
      std::cout << "REWIND_FLOW maximum rewinds set to: " << maxRewinds
                << std::endl;
    }
  }
  ...
  double baseCost =
      calculateTemporalResourceCost(TemporalOperation::REWIND_FLOW);
  ...
  while (true) {
    ...
    if (maxRewinds >= 0 && rewindCount >= maxRewinds) {
      if (m_debugLogging || m_debugMode) {
        std::cout << "REWIND_FLOW reached maximum rewinds: " << maxRewinds
                  << std::endl;
      }
      break;
    }
    ...
    if (!conditionValue.asBool()) {
      if (m_debugLogging || m_debugMode) {
        std::cout << "REWIND_FLOW condition evaluated to false, exiting loop"
                  << std::endl;
      }
      break;
    }

    // Calculate resource cost for this iteration, affected by stabilization
    // factor
    double iterationCost = baseCost;

    // Apply stabilization factor - higher stabilization reduces resource cost
    // but increases paradox risk
    if (m_stabilizationFactor > 0.0) {
      ...
      if (!paradoxRiskIncreased && m_stabilizationFactor > 0.3) {
        if (m_debugLogging || m_debugMode) {
          std::cout << "REWIND_FLOW stabilization factor "
                    << m_stabilizationFactor << " increases paradox risk"
                    << std::endl;
        }
    ...
    trackResourceUsage(iterationCost * 0.6, iterationCost * 0.4,
                       "REWIND_FLOW iteration " +
                           std::to_string(rewindCount + 1));
    ...
    if (m_haltRewind_flag) {
      if (m_debugLogging || m_debugMode) {
        std::cout << "REWIND_FLOW halted due to HALT_REWIND control mechanism"
                  << std::endl;
      }
      break;
    }
  ...
  if (m_debugLogging || m_debugMode) {
    std::cout << "REWIND_FLOW completed after " << rewindCount << " iterations"
              << std::endl;
  }
...
double Interpreter::getNumericValue(const Value &value) {
  if (value.isInteger()) {
    ...
    throw ChronovyanRuntimeError(
        "Expected a number, but got " + value.getTypeName(), SourceLocation());
  }
}
  ...
  m_globals->define(
      ...
      Value(std::make_shared<NativeFunction>(
          [this](Interpreter &interpreter,
            ...
            if (!args[0].isString()) {
              std::stringstream ss;
              ss << "DEFINE_STRUCT() first parameter must be a string type "
                    "name, got "
                 << args[0].getTypeName();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
            ...
            if (typeName.empty()) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(),
                  "DEFINE_STRUCT() requires a non-empty type name");
              return Value(false);
            }

            // Create a struct builder
            auto builder = m_userTypeSystem->createTypeBuilder(
                typeName, UserTypeKind::STRUCT);
            ...
            if (args.size() > 1) {
              ...
              for (const auto &[fieldName, fieldProps] : fieldsMap) {
                ...
                // If it's a map, it contains detailed field properties
                  ...
                  // Get required flag (optional, defaults to false if there's a
                  // default value)
                  bool isRequired = false;
                  ...
                  std::stringstream ss;
                  ss << "Field '" << fieldName
                     << "' definition must be a string type or a properties "
                        "map, got "
            ...
            try {
              ...
              std::stringstream ss;
              ss << "Failed to define struct type '" << typeName
                 << "': " << e.what();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
              return Value(false);
      ...
      Value(std::make_shared<NativeFunction>(
          [this](Interpreter &interpreter,
            ...
            if (args.size() < 1) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(),
                  "DEFINE_ENUM() requires at least a type name parameter");
              return Value(false);
            }

            if (!args[0].isString()) {
              std::stringstream ss;
              ss << "DEFINE_ENUM() first parameter must be a string type name, "
                    "got "
                 << args[0].getTypeName();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
            ...
            if (typeName.empty()) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(),
                  "DEFINE_ENUM() requires a non-empty type name");
              return Value(false);
            }

            // Create an enum builder
            auto builder = m_userTypeSystem->createTypeBuilder(
                typeName, UserTypeKind::ENUM);

            // If provided, add enum values from the second argument
            if (args.size() > 1) {
              if (!args[1].isMap()) {
                std::stringstream ss;
                ss << "DEFINE_ENUM() second parameter must be a map of enum "
                      "values, got "
                   << args[1].getTypeName();
                ErrorHandler::getInstance().reportError(SourceLocation(),
                                                        ss.str());
              ...
              for (const auto &[valueName, valueData] : valuesMap) {
                ...
                if (valueData.isInteger()) {
                  ...
                  // The advanced implementation with ExprNode would require
                  // more work
            ...
            try {
              ...
              std::stringstream ss;
              ss << "Failed to define enum type '" << typeName
                 << "': " << e.what();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
              return Value(false);
  ...
  m_globals->define(
      ...
      Value(std::make_shared<NativeFunction>(
          [this](Interpreter &interpreter,
            ...
            if (!args[0].isString()) {
              std::stringstream ss;
              ss << "DEFINE_UNION() first parameter must be a string type "
                    "name, got "
                 << args[0].getTypeName();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
            ...
            if (!args[1].isArray()) {
              std::stringstream ss;
              ss << "DEFINE_UNION() second parameter must be an array of "
                    "member types, got "
                 << args[1].getTypeName();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
            ...
            if (typeName.empty()) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(),
                  "DEFINE_UNION() requires a non-empty type name");
              return Value(false);
            }

            // Create a union builder
            auto builder = m_userTypeSystem->createTypeBuilder(
                typeName, UserTypeKind::UNION);
            ...
            if (membersArray.size() < 2) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(),
                  "DEFINE_UNION() requires at least two member types");
              return Value(false);
            }
            ...
            try {
              ...
              std::stringstream ss;
              ss << "Failed to define union type '" << typeName
                 << "': " << e.what();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
              return Value(false);
      ...
      Value(std::make_shared<NativeFunction>(
          [this](Interpreter &interpreter,
            ...
            if (args.size() < 2) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(),
                  "DEFINE_ALIAS() requires a type name and target type");
              return Value(false);
            }

            if (!args[0].isString() || !args[1].isString()) {
              std::stringstream ss;
              ss << "DEFINE_ALIAS() parameters must be strings, got "
                 << args[0].getTypeName() << " and " << args[1].getTypeName();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
              return Value(false);
            ...
            if (aliasName.empty() || targetType.empty()) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(),
                  "DEFINE_ALIAS() requires non-empty type names");
              return Value(false);
            }

            // Create an alias builder
            auto builder = m_userTypeSystem->createTypeBuilder(
                aliasName, UserTypeKind::ALIAS);
            ...
            try {
              ...
              std::stringstream ss;
              ss << "Failed to define type alias '" << aliasName
                 << "': " << e.what();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
              return Value(false);
  ...
  m_globals->define(
      ...
      Value(std::make_shared<NativeFunction>(
          [this](Interpreter &interpreter,
            ...
            if (!args[0].isString()) {
              std::stringstream ss;
              ss << "CREATE_INSTANCE() first parameter must be a string type "
                    "name, got "
                 << args[0].getTypeName();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
            ...
            if (typeName.empty()) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(),
                  "CREATE_INSTANCE() requires a non-empty type name");
              return Value();
            }
            ...
            if (args.size() > 1) {
              if (!args[1].isMap()) {
                std::stringstream ss;
                ss << "CREATE_INSTANCE() second parameter must be a map of "
                      "initial values, "
                      "got "
                   << args[1].getTypeName();
                ErrorHandler::getInstance().reportError(SourceLocation(),
            ...
            try {
              auto instancePtr =
                  m_userTypeSystem->createInstance(typeName, initialValues);
              if (instancePtr) {
              ...
              std::stringstream ss;
              ss << "Failed to create instance of type '" << typeName
                 << "': " << e.what();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
              return Value();
  ...
  m_globals->define(
      ...
      Value(std::make_shared<NativeFunction>(
          [this](Interpreter &interpreter,
            ...
            if (!args[1].isString()) {
              std::stringstream ss;
              ss << "GET_INSTANCE_FIELD() second parameter must be a string "
                    "field name, got "
                 << args[1].getTypeName();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
            ...
            if (!m_userTypeSystem->isValidInstance(args[0])) {
              std::stringstream ss;
              ss << "GET_INSTANCE_FIELD() first parameter must be a valid type "
                    "instance, got "
                 << args[0].getTypeName();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
            ...
            try {
              ...
              std::stringstream ss;
              ss << "Failed to get field '" << fieldName
                 << "' from instance: " << e.what();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
              return Value();
            }
  ...
  m_globals->define(
      ...
      Value(std::make_shared<NativeFunction>(
          [this](Interpreter &interpreter,
            ...
            if (args.size() < 3) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(), "SET_INSTANCE_FIELD() requires an "
                                    "instance, a field name, and a value");
              return Value(false);
            }

            if (!args[1].isString()) {
              std::stringstream ss;
              ss << "SET_INSTANCE_FIELD() second parameter must be a string "
                    "field name, got "
                 << args[1].getTypeName();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
            ...
            if (!m_userTypeSystem->isValidInstance(args[0])) {
              std::stringstream ss;
              ss << "SET_INSTANCE_FIELD() first parameter must be a valid type "
                    "instance, got "
                 << args[0].getTypeName();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
            ...
            try {
              ...
              if (isTemporal) {
                // Get current cycle
                uint64_t currentCycle = static_cast<uint64_t>(
                    m_runtime->getCycleNumber()); // MODIFIED

                // Update history
                m_userTypeSystem->setInstanceFieldAt(args[0], fieldName,
                                                     newValue, currentCycle);
              } else {
                // Regular update
                m_userTypeSystem->setInstanceField(args[0], fieldName,
                                                   newValue);
              }
              ...
              std::stringstream ss;
              ss << "Failed to set field '" << fieldName
                 << "' in instance: " << e.what();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
              return Value(false);
      ...
      Value(std::make_shared<NativeFunction>(
          [this](Interpreter &interpreter,
            ...
            if (args.empty()) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(),
                  "GET_INSTANCE_HISTORY() requires an instance parameter");
              return Value();
            }
            ...
            if (!m_userTypeSystem->isValidInstance(args[0])) {
              std::stringstream ss;
              ss << "GET_INSTANCE_HISTORY() parameter must be a valid type "
                    "instance, got "
                 << args[0].getTypeName();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
            ...
            if (!m_userTypeSystem->isInstanceTemporal(args[0])) {
              std::stringstream ss;
              ss << "GET_INSTANCE_HISTORY() can only be used with temporal "
                    "instances";
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
              return Value();
  ...
  m_globals->define(
      ...
      Value(std::make_shared<NativeFunction>(
          [this](Interpreter &interpreter,
            ...
            if (args.size() < 2) {
              ErrorHandler::getInstance().reportError(
                  SourceLocation(), "GET_INSTANCE_AT_CYCLE() requires an "
                                    "instance and a cycle parameter");
              return Value();
            }

            if (!args[1].isInteger()) {
              std::stringstream ss;
              ss << "GET_INSTANCE_AT_CYCLE() second parameter must be an "
                    "integer cycle, got "
                 << args[1].getTypeName();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
            ...
            if (cycle < 0) {
              std::stringstream ss;
              ss << "GET_INSTANCE_AT_CYCLE() cycle must be non-negative, got "
                 << cycle;
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
              return Value();
            ...
            if (!m_userTypeSystem->isValidInstance(args[0])) {
              std::stringstream ss;
              ss << "GET_INSTANCE_AT_CYCLE() first parameter must be a valid "
                    "type instance, "
                    "got "
                 << args[0].getTypeName();
              ErrorHandler::getInstance().reportError(SourceLocation(),
            ...
            if (!m_userTypeSystem->isInstanceTemporal(args[0])) {
              std::stringstream ss;
              ss << "GET_INSTANCE_AT_CYCLE() can only be used with temporal "
                    "instances";
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
              return Value();
            ...
            try {
              return m_userTypeSystem->getInstanceAtCycle(
                  args[0], static_cast<uint64_t>(cycle));
            } catch (const std::exception &e) {
              std::stringstream ss;
              ss << "Failed to get instance at cycle " << cycle << ": "
                 << e.what();
              ErrorHandler::getInstance().reportError(SourceLocation(),
                                                      ss.str());
              return Value();
  ...
  while (true) {
    ...
    if (node.hasCondition()) {
      ...
      if (!conditionValue.isBoolean()) {
        ErrorHandler::getInstance().reportError(
            node.getLocation(),
            "TEMPORAL_LOOP condition must evaluate to a boolean value");
        return;
      }
    ...
    // If variance is specified, apply a random fluctuation to the duration
    // increment
...
void Interpreter::consumeResources(double amount) {
  ...
  if (m_aethelAvailable < amount) {
    ...
    if (m_environment->hasResourceInsufficiencyHandler()) {
      ...
      throw std::runtime_error(
          "Insufficient aethel resources for temporal operation");
    }
  }
...
Value Interpreter::add(const Value &left, const Value &right) {
  if (left.getType() == Value::Type::NUMBER &&
      right.getType() == Value::Type::NUMBER) {
...
void Interpreter::visitParallelExecutionStmt(
    const ParallelExecutionStmtNode &node) {
  // Reset control flow flags
  m_breakChronon_flag = false;
  m_continueWeave_flag = false;
  ...
  if (node.hasThreadCount()) {
    ...
    if (!threadCountValue.isNumeric()) {
      ErrorHandler::getInstance().reportError(
          node.getLocation(),
          "PARALLEL_EXECUTION thread count must be a numeric value");
      return;
    }
    ...
    if (numThreads < 1) {
      ErrorHandler::getInstance().reportError(
          node.getLocation(),
          "PARALLEL_EXECUTION thread count must be at least 1");
      numThreads = 1;
  ...
  double cost =
      calculateTemporalResourceCost(TemporalOperation::PARALLEL_EXECUTION) *
      numThreads;
  consumeResources(cost);
  ...
  for (int i = 0; i < numThreads; i++) {
    m_threadEnvironments.push_back(
  ...
  for (int i = 0; i < numThreads; i++) {
    ...
    try {
      ...
      std::string errorMsg =
          "Error in thread " + std::to_string(i) + ": " + e.what();
...
void Interpreter::executeParallelBlock(const BlockStmtNode &block, int threadId,
                                       int totalThreads) {
  ...
  try {
    ...
    m_environment->define("TOTAL_THREADS",
                          Value(static_cast<int64_t>(totalThreads)));
    ...
    ErrorHandler::getInstance().reportError(
        block.getLocation(), "Error in parallel execution thread " +
...
void Interpreter::visitBranchTimelineStmt(const BranchTimelineStmtNode &node) {
  ...
  if (node.hasBranchCount()) {
    ...
    if (!branchCountValue.isNumeric()) {
      ErrorHandler::getInstance().reportError(
          node.getLocation(),
          "BRANCH_TIMELINE branch count must be a numeric value");
      return;
    }
    ...
    if (branchCount < 1) {
      ErrorHandler::getInstance().reportError(
          node.getLocation(),
          "BRANCH_TIMELINE branch count must be at least 1");
      branchCount = 1;
    } else if (branchCount > 10) {
      // Arbitrary limit to prevent excessive resource usage
      ErrorHandler::getInstance().reportError(
          node.getLocation(), "BRANCH_TIMELINE branch count limited to 10");
      branchCount = 10;
    }
  }
  ...
  double cost =
      calculateTemporalResourceCost(TemporalOperation::BRANCH_TIMELINE) *
      branchCount;
  consumeResources(cost);
  ...
  for (int i = 0; i < branchCount; i++) {
    ...
    branchEnv->define("TOTAL_BRANCHES",
                      Value(static_cast<int64_t>(branchCount)));
...
std::shared_ptr<LootSystem> Interpreter::getLootSystem() const {
  return m_lootSystem;
}
