...
The Chronicle Bytecode Format is the target output of the Chronicle Compiler, designed to efficiently represent 4ever programs in a format suitable for execution by the Chronicle Virtual Machine (ChronVM). This bytecode format is optimized for the unique temporal semantics and resource management features of the 4ever language, enabling efficient execution while preserving the language's temporal safety guarantees.
...
- `0x0001`: Contains debug information
...
The Chronicle Bytecode instruction set is designed to efficiently represent 4ever programs. Instructions are categorized by their functionality:
...
- `borrow_aethel <amount_index>`: Borrow Aethel
...
- `jmp <offset>`: Opcode (0x50) followed by a 16-bit signed offset
...
The ChronVM maintains a timeline stack to track the current execution path and timeline branches. Timeline operations manipulate this stack to implement 4ever's temporal semantics.
...
The Chronicle Bytecode Format provides an efficient representation of 4ever programs for execution by the ChronVM. Its design emphasizes the unique features of the 4ever language, particularly its temporal semantics, resource management, and CONF/REB variable classification. By providing a well-defined bytecode format, we enable consistent execution of 4ever programs across different implementations of the ChronVM.



As the 4ever language evolves, the Chronicle Bytecode Format will be extended to support new features while maintaining backward compatibility with existing bytecode files. This evolution will be guided by the principles of efficiency, safety, and expressiveness that are central to the 4ever language design.