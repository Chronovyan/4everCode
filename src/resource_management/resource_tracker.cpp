...
ResourceTracker::ResourceTracker(size_t max_history)
    : max_history_size_(max_history) {
  // Reserve space to avoid frequent reallocations
  history_.reserve(max_history);
}

void ResourceTracker::recordCurrentUsage(double chronon_usage,
                                         double aethel_usage,
...
const std::vector<ResourceDataPoint> &
ResourceTracker::getHistoricalData() const {
  return history_;
}

std::map<std::string, double> ResourceTracker::getResourceStatistics() const {
  return statistics_;
}
