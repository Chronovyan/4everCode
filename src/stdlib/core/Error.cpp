...
std::string formatErrorMessage(const ErrorInfo &info,
...
ChronovyanError::ChronovyanError(ErrorInfo info, std::source_location loc)
    : std::runtime_error(formatErrorMessage(info, loc)),
...
ChronovyanError ChronovyanError::create(ErrorCode code,
...
void ChronovyanError::addNote(const std::string &note) {
  errorInfo_.notes.push_back(note);
}
...
void ErrorHandler::handleWarning(ErrorCode code, const std::string &message,
                                 const std::source_location &loc) {
  std::cerr << "Warning: [" << categoryToString(code.category) << ":"
            << code.code << "] " << message << " (at "
            << (loc.file_name() ? loc.file_name() : "") << ':' << loc.line()
            << ':' << loc.column() << ')' << std::endl;
}

// Set a custom error handler
void ErrorHandler::setHandler(ErrorHandler *handler) {
