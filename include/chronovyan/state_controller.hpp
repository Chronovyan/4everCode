...
public:
    // Constants
    static constexpr std::chrono::milliseconds kModeSwitchCooldown{
        1000};                                        // Minimum time between mode switches
    static constexpr size_t kMaxHistoryEntries = 50;  // Maximum number of history entries to keep
    static constexpr size_t kMaxTransitionEntries =
        50;  // Maximum number of transition entries to keep
    ...
    void setCurrentModeForTesting(PerformanceMode mode) { current_mode_ = mode; }
    ...
    std::vector<std::pair<PerformanceMode, std::string>>& getTransitionHistoryForTesting() {
