...
CompilerDriver::CompilerDriver(DiagnosticReporter &diagnostics)
    : diagnostics(diagnostics), dumpIR(false), dumpAST(false),
      provingGroundsEnabled(false), maxParadoxSeverity(5),
      validationReportingEnabled(false), validationReportFormat("text") {
...
void CompilerDriver::registerOptimizationPass(
    std::unique_ptr<OptimizationPass> pass, OptimizationLevel level) {
  optimizationManager.registerPass(std::move(pass), level);
}

void CompilerDriver::registerTemporalOptimizationPass(
    std::unique_ptr<OptimizationPass> pass, OptimizationLevel level) {
  optimizationManager.registerTemporalPass(std::move(pass), level);
}

void CompilerDriver::setOutputFile(const std::string &file) {
  outputFile = file;
}

std::string CompilerDriver::getOutputFile() const { return outputFile; }

void CompilerDriver::enableProvingGrounds(bool enable) {
  provingGroundsEnabled = enable;
}

bool CompilerDriver::isProvingGroundsEnabled() const {
  return provingGroundsEnabled;
}

void CompilerDriver::setMaxParadoxSeverity(int severity) {
  // Ensure the severity is within valid range (1-10)
  if (severity < 1 || severity > 10) {
    diagnostics.addWarning(SourceLocation(), "Invalid paradox severity: " +
...
void CompilerDriver::enableValidationReporting(bool enable,
...
CompilerDriver::getProvingGroundsFramework() const {
  // TODO: Uncomment when the TemporalTestFramework class is implemented
...
bool CompilerDriver::compileString(const std::string &sourceCode,
...
CompilerDriver::parseFile(const std::string &sourceFile) {
...
std::unique_ptr<AstNode>
CompilerDriver::parseString(const std::string &sourceCode,
                            const std::string &fileName) {
...
bool CompilerDriver::generateBytecode(const IRProgram &irProgram) {
  ...
  // Generate bytecode
  bytecode::BytecodeModule bytecodeModule =
      bytecodeGenerator.generateBytecode(irProgram);

  // Validate bytecode with Temporal Proving Grounds if enabled
  if (provingGroundsEnabled) {
    std::string sourceName =
        !outputFile.empty() ? outputFile : "unnamed_source";
    // TODO: Uncomment when the TemporalTestFramework class is implemented
    /*
    if (!validateBytecodeWithProvingGrounds(bytecodeModule, sourceName)) {
        ...
        diagnostics.addWarning(SourceLocation(),
            "Temporal validation issues detected. See validation report for
    details.");
    }
    */
...
bool CompilerDriver::writeBytecodeToFile(
  ...
  if (file.fail()) {
    diagnostics.addError(SourceLocation(),
                         "Failed to write to output file: " + outputFile);
    file.close();
    return false;
  }
...
bool CompilerDriver::validateBytecodeWithProvingGrounds(
  ...
  if (!provingGroundsFramework) {
      diagnostics.addError(SourceLocation(), "Temporal Proving Grounds framework
  not initialized"); return false;
  }

  // Create a test case from the bytecode module
  auto testCase =
  provingGroundsFramework->createTestCaseFromBytecode(bytecodeModule,
  ...
  validationReportFormat);
      ...
      if (!validationReportFile.empty()) {
          if (!provingGroundsFramework->saveReport(report,
  validationReportFile)) { diagnostics.addWarning(SourceLocation(), "Failed to
  save validation report to: " + validationReportFile);
          }
      } else {
