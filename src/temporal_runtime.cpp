...
void TemporalRuntime::increaseParadoxLevel(int amount) {
    ...
    std::cout << "Paradox level increased by " << amount << ". New level: " << m_paradoxLevel
              << std::endl;
...
void TemporalRuntime::decreaseParadoxLevel(int amount) {
    ...
    std::cout << "Paradox level decreased by " << amount << ". New level: " << m_paradoxLevel
              << std::endl;
}
...
void TemporalRuntime::setParadoxLevel(int level) {
    ...
    if (change > 0) {
        std::cout << "Paradox level increased by " << change << ". New level: " << m_paradoxLevel
                  << std::endl;
    } else if (change < 0) {
        std::cout << "Paradox level decreased by " << -change << ". New level: " << m_paradoxLevel
                  << std::endl;
    }
}
...
void TemporalRuntime::replenishChronons(double amount) {
    m_chrononsLevel += amount;
    std::cout << "Replenished " << amount << " chronons. New level: " << m_chrononsLevel
              << std::endl;
}
...
void TemporalRuntime::createTimelineSnapshot() {
    ...
    try {
        ...
        std::cout << "Warning: " << e.what() << " - Creating snapshot with minimal resources"
                  << std::endl;
    }
...
void TemporalRuntime::mergeTimelines(const std::vector<std::string>& timelineIds) {
    ...
    std::cout << "Merging " << validTimelineIds.size() << " timelines..." << std::endl;
    ...
    try {
        ...
        std::cout << "Warning: " << e.what() << " - Merging with minimal resources" << std::endl;
    }
...
size_t TemporalRuntime::getTimelineSnapshotCount() const { return m_timelineSnapshots.size(); }
...
std::shared_ptr<Loot> TemporalRuntime::getDiscoveredLootById(const std::string& lootId) const {
...
std::vector<std::shared_ptr<LootSet>> TemporalRuntime::getCompletedLootSets() const {
...
bool TemporalRuntime::applyLootSetBonus(const std::string& lootSetId) {
    ...
    if (m_appliedLootSetBonuses.find(lootSetId) != m_appliedLootSetBonuses.end()) {
        std::cout << "Set bonus for " << lootSet->getName() << " has already been applied."
                  << std::endl;
        return false;
    }
...
void TemporalRuntime::checkLootSetCompletion() {
    ...
    for (const auto& [id, lootSet] : registry.getAllLootSets()) {
        ...
        if (lootSet->isCompleted(m_discoveredLootIds) &&
            ...
            std::cout << "\n**LOOT SET COMPLETED**: " << lootSet->getName() << std::endl;
            std::cout << lootSet->getDescription() << std::endl;
            std::cout << "You have collected all " << lootSet->size() << " items in this set!"
                      << std::endl;
            std::cout << "Type 'use set " << id << "' to apply the set bonus." << std::endl;
        }
    }
}
...
long long TemporalRuntime::getCycleNumber() const { return static_cast<long long>(m_currentCycle); }
...
std::vector<double> TemporalRuntime::getResourceHistory() const { return m_resourceHistory; }

std::vector<double> TemporalRuntime::getEfficiencyHistory() const { return m_efficiencyHistory; }
