...
TemporalTestFramework::TemporalTestFramework(DiagnosticReporter& diag) : diagnostics(diag) {
...
void TemporalTestFramework::registerTestCase(std::shared_ptr<TemporalTestCase> testCase) {
    testCases.push_back(testCase);
}

void TemporalTestFramework::registerTestSuite(std::shared_ptr<TemporalTestSuite> testSuite) {
...
TemporalTestResult TemporalTestFramework::runTest(const std::string& testCaseName) {
...
std::vector<TemporalTestResult> TemporalTestFramework::runTestSuite(
    ...
    results.push_back(TemporalTestResult(testSuiteName, false, "Test suite not found"));
    return results;
}

std::shared_ptr<ParadoxDetector> TemporalTestFramework::getParadoxDetector() const {
    return paradoxDetector;
}

std::shared_ptr<TemporalBenchmark> TemporalTestFramework::getTemporalBenchmark() const {
    return benchmark;
}

std::string TemporalTestFramework::generateReport(const std::vector<TemporalTestResult>& results,
    ...
    if (format == "json") {
        ...
        report << "    .test { margin-bottom: 10px; padding: 10px; border: 1px solid #ccc; }\n";
        ...
        report << "    <p>Pass percentage: "
               << (results.empty() ? 0 : (passCount * 100.0 / results.size())) << "%</p>\n";
        report << "  </div>\n";
        ...
        for (const auto& result : results) {
            report << "  <div class=\"test " << (result.isPassed() ? "test-passed" : "test-failed")
                   << "\">\n";
            report << "    <div class=\"test-name\">" << result.getTestName() << "</div>\n";
            report << "    <div class=\"test-message\">" << result.getMessage() << "</div>\n";
            ...
            if (!paradoxes.empty()) {
                ...
                for (const auto& paradox : paradoxes) {
                    ...
                    report << "        <div>Description: " << paradox.description << "</div>\n";
            ...
            if (!benchmarks.empty()) {
                ...
                for (const auto& [opName, benchResult] : benchmarks) {
                    ...
                    report << "        <div>Duration: " << benchResult.duration << " ms</div>\n";
                    report << "        <div>Resources:</div>\n";
                    report << "        <ul>\n";
                    for (const auto& [resourceName, usage] : benchResult.resourceUsage) {
                        report << "          <li>" << resourceName << ": " << usage << "</li>\n";
                    }
                    report << "        </ul>\n";
                    report << "      </div>\n";
                }
...
std::shared_ptr<TemporalTestCase> TemporalTestFramework::createTestCaseFromBytecode(
...
std::shared_ptr<TemporalTestCase> TemporalTestFramework::createTestCaseFromSource(
    ...
    diagnostics.addWarning(
        SourceLocation(),
        "Source file compilation not implemented yet. Using empty bytecode module.");
...
TemporalTestResult TemporalTestFramework::runTestCase(
    ...
    if (!debugger->initialize(testCase->getModule())) {
        result = TemporalTestResult(testCase->getName(), false, "Failed to initialize debugger");
        return result;
    }

    // Start the debugger
    if (!debugger->start()) {
        result = TemporalTestResult(testCase->getName(), false, "Failed to start debugger");
        return result;
    }
    ...
    while (completed) {
        ...
        if (!paradoxes.empty()) {
            for (const auto& paradox : paradoxes) {
                result.addParadox(paradox.description, paradox.severity, paradox.location);
                ...
                if (paradox.severity > testCase->getMaxParadoxSeverity()) {
                    result = TemporalTestResult(
                        ...
                        "Paradox detected with severity " + std::to_string(paradox.severity) +
                            " (max allowed: " + std::to_string(testCase->getMaxParadoxSeverity()) +
                            ")");
                    completed = false;
                    break;
                }
    ...
    if (testCase->isBenchmarkingEnabled()) {
        ...
        result.addBenchmarkResult("execution", duration, debugger->getCurrentResourceUsage());
    }
...
void TemporalTestResult::addParadox(const std::string& paradoxDescription, int severity,
                                    const SourceLocation& sourceLocation) {
    paradoxes.push_back({paradoxDescription, severity, sourceLocation});
}

std::vector<TemporalTestResult::Paradox> TemporalTestResult::getParadoxes() const {
    return paradoxes;
}

void TemporalTestResult::addBenchmarkResult(const std::string& operationName, double duration,
                                            const std::map<std::string, double>& resourceUsage) {
    benchmarkResults[operationName] = {duration, resourceUsage};
}

std::map<std::string, TemporalTestResult::BenchmarkResult> TemporalTestResult::getBenchmarkResults()
    const {
    return benchmarkResults;
}

// TemporalTestCase implementation
TemporalTestCase::TemporalTestCase(const std::string& name,
                                   const bytecode::BytecodeModule& bytecodeModule)
    : name(name), module(bytecodeModule), maxParadoxSeverity(5), benchmarkingEnabled(false) {}

std::string TemporalTestCase::getName() const { return name; }

const bytecode::BytecodeModule& TemporalTestCase::getModule() const { return module; }

void TemporalTestCase::addAssertion(std::shared_ptr<TemporalAssertion> assertion) {
    assertions.push_back(assertion);
}

const std::vector<std::shared_ptr<TemporalAssertion>>& TemporalTestCase::getAssertions() const {
    return assertions;
}

void TemporalTestCase::setMaxParadoxSeverity(int severity) { maxParadoxSeverity = severity; }

int TemporalTestCase::getMaxParadoxSeverity() const { return maxParadoxSeverity; }

void TemporalTestCase::enableBenchmarking(bool enable) { benchmarkingEnabled = enable; }

bool TemporalTestCase::isBenchmarkingEnabled() const { return benchmarkingEnabled; }
...
void TemporalTestSuite::addTestCase(std::shared_ptr<TemporalTestCase> testCase) {
    testCases.push_back(testCase);
}

const std::vector<std::shared_ptr<TemporalTestCase>>& TemporalTestSuite::getTestCases() const {
    return testCases;
}

// TemporalAssertion implementation
TemporalAssertion::TemporalAssertion(const std::string& desc) : description(desc) {}

std::string TemporalAssertion::getDescription() const { return description; }

std::string TemporalAssertion::getFailureMessage() const { return failureMessage; }

// StateAssertion implementation
StateAssertion::StateAssertion(const std::string& description, const std::string& varName,
                               const std::string& expectedVal)
    : TemporalAssertion(description), variableName(varName), expectedValue(expectedVal) {}
...
bool StateAssertion::check(debugger::DejaVuDebugger& debugger) {
    ...
    if (actualValue != expectedValue) {
        failureMessage = "Expected " + variableName + " to be '" + expectedValue + "', but got '" +
                         actualValue + "'";
        return false;
    }
...
ResourceAssertion::ResourceAssertion(const std::string& description, const std::string& resName,
                                     double expectedUsage, double tolerance)
    : TemporalAssertion(description)
...
TimelineAssertion::TimelineAssertion(const std::string& description, int expectedCount)
...
bool TimelineAssertion::check(debugger::DejaVuDebugger& debugger) {
    // In a real implementation, we would get the timeline branches from the debugger
    // For now, just check if the current timeline ID is within the expected range
    int currentTimelineId = debugger.getCurrentTimelineId();

    if (currentTimelineId >= expectedTimelineCount) {
        failureMessage = "Expected timeline count to be " + std::to_string(expectedTimelineCount) +
                         ", but timeline ID " + std::to_string(currentTimelineId) + " was detected";
        return false;
    }
