...
std::string StateVisualizer::generateTextVisualization(const DebuggerState& state) const {
...
std::string StateVisualizer::generateJsonVisualization(const DebuggerState& state) const {
    std::ostringstream oss;
    auto [file, line] = state.getSourceLocation();
    ...
    // Paradox risk
    oss << "  \"potentialParadox\": " << (state.isPotentialParadox() ? "true" : "false") << "\n";
...
std::string StateVisualizer::generateHtmlVisualization(const DebuggerState& state) const {
    ...
    if (callStack.empty()) {
        ...
        for (size_t i = 0; i < callStack.size(); ++i) {
            oss << "    <div class=\"call-stack\">" << i << ": " << callStack[i] << "</div>\n";
        }
    }
    oss << "  </div>\n";
    ...
    if (localVars.empty()) {
        ...
        for (const auto& [name, value] : localVars) {
            oss << "    <div class=\"variable\">" << name << " = " << value << "</div>\n";
        }
    }
    oss << "  </div>\n";
    ...
    if (resources.empty()) {
        ...
        for (const auto& [name, value] : resources) {
            oss << "    <div class=\"resource\">" << name << " = " << value << "</div>\n";
        }
    }
    oss << "  </div>\n";
...
std::string StateVisualizer::generateTextHistoryVisualization(const StateHistory& history) const {
    std::ostringstream oss;
...
std::string StateVisualizer::generateJsonHistoryVisualization(const StateHistory& history) const {
    std::ostringstream oss;
...
std::string StateVisualizer::generateHtmlHistoryVisualization(const StateHistory& history) const {
    std::ostringstream oss;
    ...
    oss << "    body { font-family: Arial, sans-serif; margin: 20px; }\n";
    oss << "    h1 { color: #336699; }\n";
    oss << "    .state { margin-bottom: 20px; border: 1px solid #ccc; padding: 10px; }\n";
    oss << "    .state-header { background-color: #f0f0f0; padding: 5px; margin-bottom: 10px; }\n";
...
std::string StateVisualizer::generateHtmlTemporalFlowVisualization(
    ...
    if (timelineBranching.empty()) {
        ...
        oss << "  <ul>\n";
        for (const auto& [timeline, parent] : timelineBranching) {
            oss << "    <li class=\"timeline\">Timeline " << timeline << " branched from Timeline "
                << parent << "</li>\n";
        }
        oss << "  </ul>\n";
    }
