...
ResourceManager::ResourceManager(std::shared_ptr<TemporalRuntime> runtime,
...
bool ResourceManager::consumeAethel(double amount, const std::string& operation) {
...
bool ResourceManager::consumeChronons(double amount, const std::string& operation) {
    // Apply optimization if possible
    double optimizedAmount = amount;
    if (m_optimizer && !operation.empty()) {
...
bool ResourceManager::consumeResources(double aethelAmount, double chrononAmount,
...
double ResourceManager::getAethelLevel() const { return m_runtime->getAethelLevel(); }

double ResourceManager::getChrononsLevel() const { return m_runtime->getChrononsLevel(); }

double ResourceManager::getMaxAethel() const { return m_runtime->getMaxAethel(); }

double ResourceManager::getMaxChronons() const { return m_runtime->getMaxChronons(); }

void ResourceManager::replenishAethel(double amount) { m_runtime->replenishAethel(amount); }

void ResourceManager::replenishChronons(double amount) { m_runtime->replenishChronons(amount); }

void ResourceManager::trackResourceUsage(double aethelAmount, double chrononAmount,
                                         const std::string& operation) {
...
std::map<std::string, std::pair<double, double>> ResourceManager::getResourceUsageHistory() const {
...
double ResourceManager::calculateTemporalOperationCost(const std::string& operationType,
                                                       int paradoxLevel,
