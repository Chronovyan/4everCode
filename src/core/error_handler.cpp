...
int ErrorHandler::reportError(const SourceLocation& location, const std::string& message,
...
int ErrorHandler::reportWarning(const SourceLocation& location, const std::string& message) {
...
[[noreturn]] void ErrorHandler::reportFatalError(const SourceLocation& location,
    ...
    throw ChronovyanException(formatErrorMessage(message, location, m_runtimeContext, true));
}

int ErrorHandler::reportParadox(const SourceLocation& location, const std::string& message,
                                ParadoxType type, int level) {
    // Create the paradox
...
int ErrorHandler::reportGlitch(const SourceLocation& location, const std::string& message,
                               GlitchType type, double severity) {
    // Create the glitch
...
bool ErrorHandler::resolveParadox(int paradoxId) {
    ...
    std::cout << "Paradox resolved: " << ParadoxEvent::paradoxTypeToString(paradox.type)
              << std::endl;
...
bool ErrorHandler::fixGlitch(int glitchId) {
    ...
    glitch.isFixed = true;
    std::cout << "Glitch fixed: " << GlitchEvent::glitchTypeToString(glitch.type) << std::endl;
...
size_t ErrorHandler::getParadoxCount(bool includeResolved) const {
    if (includeResolved) {
        ...
        return std::count_if(m_paradoxes.begin(), m_paradoxes.end(),
...
size_t ErrorHandler::getGlitchCount(bool includeFixed) const {
    if (includeFixed) {
        ...
        return std::count_if(m_glitches.begin(), m_glitches.end(),
...
bool ErrorHandler::removeErrorCallback(int callbackId) {
    auto it = std::find_if(m_errorCallbacks.begin(), m_errorCallbacks.end(),
...
bool ErrorHandler::removeParadoxCallback(int callbackId) {
    auto it = std::find_if(m_paradoxCallbacks.begin(), m_paradoxCallbacks.end(),
                           [callbackId](const auto& pair) { return pair.first == callbackId; });
...
std::string ErrorHandler::getReport(bool includeWarnings, bool includeResolved) const {
    std::stringstream ss;
    ...
    // Report paradoxes
    ss << "=== Paradoxes (" << getParadoxCount(includeResolved) << " total) ===\n";
...
std::string formatErrorMessage(const std::string& message, const SourceLocation& location,
                               const std::string& context, bool includeLineContent) {
    std::stringstream ss;
...
bool isRecoverableError(ErrorSeverity severity) { return severity != ErrorSeverity::FATAL; }
