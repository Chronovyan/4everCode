...
class Value {
public:
    enum class Type { NULL_VALUE, BOOLEAN, INTEGER, FLOAT, STRING, RESOURCE, TIMELINE };
    ...
    explicit Value(const std::string& value) : type(Type::STRING), stringValue(value) {}
...
public:
    ExecutionContext(const std::string& functionName, size_t returnAddress = 0)
        : functionName(functionName), returnAddress(returnAddress), instructionPointer(0) {}
...
private:
     ...
     */
    bool executeInstruction(const compiler::bytecode::BytecodeInstruction& instruction,
    ...
    bool executeTemporalOperation(compiler::bytecode::OpCode opcode,
                                  const std::vector<uint16_t>& operands, ExecutionContext& context);
     ...
     */
    Value convertConstantToValue(const compiler::bytecode::BytecodeConstant& constant);
     ...
     */
    void updateParadoxLevel(compiler::bytecode::OpCode opcode, double paradoxImpact);
