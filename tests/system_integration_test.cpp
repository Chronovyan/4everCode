...
public:
    MOCK_METHOD(void, notifyModeChange, (PerformanceMode new_mode, const std::string& reason),
                (override));
...
protected:
    ...
    void processMetrics() {
        ...
        std::cout << "Is Conservative: " << (decision.is_conservative ? "true" : "false")
                  << std::endl;
        std::cout << "*******************************" << std::endl;
...
TEST_F(SystemIntegrationTest, SwitchesModeCorrectly_OnDecisionEngineOutput) {
    ...
    // Expect notification
    EXPECT_CALL(*notification_service,
                notifyModeChange(PerformanceMode::Lean, testing::HasSubstr("normal")))
        .Times(1);
...
TEST_F(SystemIntegrationTest, EnforcesCooldown_AfterModeSwitch) {
    ...
    // First switch to Balanced - directly create the decision instead of using
    // setupNormalMetrics/processMetrics
    ...
    EXPECT_CALL(*notification_service,
                notifyModeChange(PerformanceMode::Balanced, testing::HasSubstr("normal")))
        .Times(1);
...
TEST_F(SystemIntegrationTest, HandlesFallbackToLean_WhenDecisionEngineIndicatesCriticalFailure) {
    ...
    EXPECT_CALL(*notification_service, notifyError(testing::HasSubstr("critical"))).Times(1);

    // Expect mode change notification
    EXPECT_CALL(*notification_service,
                notifyModeChange(PerformanceMode::Lean, testing::HasSubstr("critical")))
        .Times(1);
...
TEST_F(SystemIntegrationTest, RecoversFromTemporaryFailure_AndRestoresAppropriateMode) {
    ...
    ON_CALL(*cpu_source, isAvailable()).WillByDefault(Return(false));
    ...
    EXPECT_CALL(
        *notification_service,
        notifyModeChange(PerformanceMode::Balanced, testing::HasSubstr("partial sensor failure")))
        .Times(1);
    ...
    EXPECT_CALL(*notification_service,
                notifyModeChange(PerformanceMode::Balanced, testing::HasSubstr("recovered")))
        .Times(1);
...
TEST_F(SystemIntegrationTest, HandlesStaleMetrics_WithModeSwitching) {
    ...
    auto old_time = std::chrono::system_clock::now() -
                    5s;  // Use 5s instead of 2s to ensure it's well past the threshold
    auto current_time = std::chrono::system_clock::now();
    ...
    EXPECT_CALL(*notification_service,
                notifyModeChange(PerformanceMode::Lean, testing::HasSubstr("stale")))
        .Times(1);
...
TEST_F(SystemIntegrationTest, HandlesPartialSensorFailures) {
    ...
    EXPECT_CALL(*notification_service, notifyError(testing::HasSubstr("partial sensor failure")))
        .Times(1);
...
TEST_F(SystemIntegrationTest, HandlesMetricSourceExceptions) {
    ...
    EXPECT_CALL(*notification_service, notifyError(testing::HasSubstr("exception"))).Times(1);

    EXPECT_CALL(*notification_service,
                notifyModeChange(PerformanceMode::Balanced, testing::HasSubstr("exception")))
        .Times(1);
...
TEST_F(SystemIntegrationTest, HandlesMetricSourceTimeouts) {
    ...
    EXPECT_CALL(*cpu_source, getValue()).WillOnce(Invoke([]() {
    ...
    EXPECT_CALL(*notification_service, notifyError(testing::HasSubstr("timeout"))).Times(1);

    EXPECT_CALL(*notification_service,
                notifyModeChange(PerformanceMode::Balanced, testing::HasSubstr("timeout")))
        .Times(1);
...
TEST_F(SystemIntegrationTest, HandlesMetricSourceRecoveryAfterMultipleFailures) {
    ...
    EXPECT_CALL(*notification_service, notifyModeChange(PerformanceMode::Balanced, testing::_))
...
TEST_F(SystemIntegrationTest, HandlesMetricSourceDegradation) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        EXPECT_CALL(*notification_service,
                    notifyModeChange(PerformanceMode::Balanced, std::string("normal")))
            .Times(1);
    ...
    for (int i = 0; i < 2; i++) {
        ...
        EXPECT_CALL(*notification_service,
                    notifyModeChange(PerformanceMode::Balanced, std::string("degraded")))
            .Times(1);
...
TEST_F(SystemIntegrationTest, HandlesMetricSourceCalibration) {
    ...
    // The calibrating phase - expect a notification with "calibrating"
    {
        ...
        EXPECT_CALL(*notification_service,
                    notifyModeChange(PerformanceMode::Balanced, testing::HasSubstr("calibrating")))
            .WillOnce([](PerformanceMode mode, const std::string& reason) {
    ...
    // The calibrated phase - expect a notification with "calibrated"
    {
        // Setup the expectation for this phase
        EXPECT_CALL(*notification_service,
                    notifyModeChange(PerformanceMode::Balanced, testing::HasSubstr("calibrated")))
            .WillOnce([](PerformanceMode mode, const std::string& reason) {
...
TEST_F(SystemIntegrationTest, HandlesMetricSourceDrift) {
    ...
    // Set global expectation for notifications before the loop - allows any mode change
    // notifications
    EXPECT_CALL(*notification_service, notifyModeChange(_, _)).WillRepeatedly(Return());
...
TEST_F(SystemIntegrationTest, StateController_CorrectlyAppliesModeSwitch_FromDecisionEngine) {
    ...
    EXPECT_CALL(
        *notification_service,
        notifyModeChange(PerformanceMode::HighFidelity,
    ...
    EXPECT_TRUE(last_entry.reason.find("High performance mode activated due to high CPU usage") !=
                std::string::npos);
...
TEST_F(SystemIntegrationTest,
    ...
    EXPECT_CALL(*notification_service, notifyModeChange(PerformanceMode::HighFidelity,
    ...
    // Try lean mode again
    EXPECT_CALL(*notification_service,
                notifyModeChange(PerformanceMode::Lean, testing::HasSubstr("Third attempt")))
        .Times(1);
...
TEST_F(SystemIntegrationTest, StateController_RejectsInvalidTargetMode_FromDecisionEngine) {
    ...
    // Expect error notification
    EXPECT_CALL(*notification_service,
                notifyError(testing::HasSubstr("Invalid mode transition detected")))
        .Times(1);
...
TEST_F(SystemIntegrationTest, StateController_PreventsModeOscillation) {
    std::vector<PerformanceMode> modes = {PerformanceMode::HighFidelity, PerformanceMode::Balanced,
                                          PerformanceMode::Lean};

    std::vector<std::string> reasons = {"High performance mode activated due to high CPU usage",
    ...
    for (size_t i = 0; i < modes.size(); ++i) {
        ...
        std::cout << "Debug - Setting mode to: " << static_cast<int>(modes[i]) << " (iteration "
                  << i << ")" << std::endl;
        ...
        state_controller->addTransitionForTesting(i > 0 ? modes[i - 1] : PerformanceMode::Balanced,
                                                  reasons[i]);
    ...
    std::cout << "Debug - Before oscillation test: isInCooldown="
              << (state_controller->isInCooldown() ? "true" : "false") << ", ForceCooldown="
              << (StateController::getForceCooldownForTesting() ? "true" : "false") << std::endl;
    ...
    for (size_t i = 1; i < change_timestamps.size(); ++i) {
        ...
        std::cout
            << "Debug - Time between changes " << (i - 1) << " and " << i << ": "
...
TEST_F(SystemIntegrationTest, StateController_HandlesConflictingRapidDecisions_FromDecisionEngine) {
    ...
    for (int i = 0; i < 5; ++i) {
        if (i % 2 == 0) {
            ...
            decision_engine->setForceModeForTesting(
                PerformanceMode::HighFidelity, true,
        ...
        if (i == 0) {
            EXPECT_CALL(
                *notification_service,
                notifyModeChange(attempted_modes[0], testing::HasSubstr(attempted_reasons[0])))
                .Times(1);
        } else {
            EXPECT_CALL(*notification_service, notifyModeChange(_, _)).Times(0);
        }
    ...
    EXPECT_TRUE(found_expected_mode)
        << "Expected to find mode " << static_cast<int>(attempted_modes[0])
        << " with reason containing '" << attempted_reasons[0] << "' in the mode history";
    ...
    EXPECT_TRUE(found_expected_transition)
        << "Expected to find a transition with reason containing '" << attempted_reasons[0]
...
TEST_F(SystemIntegrationTest,
    ...
    EXPECT_CALL(*notification_service,
                notifyError(std::string("Critical error detected: all sensors unavailable")))
        .Times(1);

    // Verify mode change notification details
    EXPECT_CALL(*notification_service,
                notifyModeChange(PerformanceMode::Lean,
    ...
    std::cout << "After updateMode - Current mode: "
              << static_cast<int>(state_controller->getCurrentMode())
    ...
    auto error_details = state_controller->getErrorDetails();
    std::cout << "Error details: " << error_details << std::endl;
    EXPECT_TRUE(error_details.find("Critical error detected: all sensors unavailable") !=
                std::string::npos);
    ...
    std::cout << "Last history entry - Mode: " << static_cast<int>(last_entry.mode)
              << ", Reason: " << last_entry.reason << ", Is Error: " << last_entry.is_error_state
    ...
    EXPECT_TRUE(last_entry.reason.find("Critical error detected: all sensors unavailable") !=
                std::string::npos);
...
TEST_F(SystemIntegrationTest, StateController_RecordsCorrectReason_ForModeSwitch) {
    ...
    std::vector<TestCase> test_cases = {
        {"HighFidelity", PerformanceMode::HighFidelity,
         "Performance mode set to High Fidelity - low resource usage"},
        {"Balanced", PerformanceMode::Balanced,
         "Performance mode set to Balanced - moderate resource usage"},
    ...
    for (const auto& test_case : test_cases) {
        ...
        EXPECT_CALL(*notification_service, notifyModeChange(test_case.mode, _)).Times(1);
