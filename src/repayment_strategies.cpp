...
RepaymentResult OldestFirstStrategy::apply(std::vector<DebtRecord>& debts,
    ...
    std::sort(debts.begin(), debts.end(),
              [](const DebtRecord& a, const DebtRecord& b) { return a.timestamp < b.timestamp; });
    ...
    for (auto& debt : debts) {
        ...
        double interest_saved = calculateInterestSaved(debt, chronon_to_repay, aethel_to_repay);
        ...
        result.stability_improvement += interest_saved * 0.05;  // Scale factor for stability
    ...
    debts.erase(std::remove_if(debts.begin(), debts.end(),
                               [](const DebtRecord& debt) {
                                   return debt.chronon_debt <= 0 && debt.aethel_debt <= 0;
                               }),
                debts.end());
...
RepaymentResult HighestInterestStrategy::apply(std::vector<DebtRecord>& debts,
    ...
    std::sort(debts.begin(), debts.end(), [](const DebtRecord& a, const DebtRecord& b) {
...
RepaymentResult CriticalFirstStrategy::apply(std::vector<DebtRecord>& debts,
    ...
    // Sort debts by critical flag (critical first) and then by due cycle (earliest first)
    std::sort(debts.begin(), debts.end(), [](const DebtRecord& a, const DebtRecord& b) {
    ...
    for (auto& debt : debts) {
        ...
        result.stability_improvement +=
            interest_saved * (debt.critical ? 0.1 : 0.05);  // Higher factor for critical debts
...
RepaymentResult BalancedStrategy::apply(std::vector<DebtRecord>& debts, double available_chronons,
                                        double available_aethel) const {
    RepaymentResult result;
    ...
    double total_chronon_debt =
        std::accumulate(debts.begin(), debts.end(), 0.0,
                        [](double sum, const DebtRecord& debt) { return sum + debt.chronon_debt; });

    double total_aethel_debt =
        std::accumulate(debts.begin(), debts.end(), 0.0,
                        [](double sum, const DebtRecord& debt) { return sum + debt.aethel_debt; });

    // Distribute repayment proportionally to debt sizes
    for (auto& debt : debts) {
        // Calculate proportion of total debt
        double chronon_proportion =
            (total_chronon_debt > 0) ? (debt.chronon_debt / total_chronon_debt) : 0.0;
        ...
        double aethel_to_repay = std::min(debt.aethel_debt, available_aethel * aethel_proportion);
...
RepaymentResult MinimumPaymentsStrategy::apply(std::vector<DebtRecord>& debts,
    ...
    double min_aethel_payment = available_aethel / std::max(1.0, static_cast<double>(debts.size()));
    ...
    for (auto& debt : debts) {
        ...
        double chronon_to_repay =
            std::min(remaining_chronons, std::min(min_chronon_payment, debt.chronon_debt));
        double aethel_to_repay =
            std::min(remaining_aethel, std::min(min_aethel_payment, debt.aethel_debt));
...
RepaymentResult SnowballStrategy::apply(std::vector<DebtRecord>& debts, double available_chronons,
                                        double available_aethel) const {
    RepaymentResult result;

    // Sort debts by total size (smallest first)
    std::sort(debts.begin(), debts.end(), [](const DebtRecord& a, const DebtRecord& b) {
        return (a.chronon_debt + a.aethel_debt) < (b.chronon_debt + b.aethel_debt);
    });
...
RepaymentResult AvalancheStrategy::apply(std::vector<DebtRecord>& debts, double available_chronons,
                                         double available_aethel) const {
    // This is similar to HighestInterestStrategy but focuses on completely repaying
    // one debt before moving to the next
    RepaymentResult result;
...
RepaymentResult StabilityOptimizedStrategy::apply(std::vector<DebtRecord>& debts,
                                                  double available_chronons,
                                                  double available_aethel) const {
    RepaymentResult result;

    // Sort debts by a composite score that combines criticality, due date, and interest rate
    std::sort(debts.begin(), debts.end(), [](const DebtRecord& a, const DebtRecord& b) {
        // Create a stability score where higher means more important to repay
        double a_score =
            (a.critical ? 100.0 : 0.0) + (100.0 / (a.due_cycle + 1)) + (a.interest_rate * 20.0);
        double b_score =
            (b.critical ? 100.0 : 0.0) + (100.0 / (b.due_cycle + 1)) + (b.interest_rate * 20.0);
    ...
    for (auto& debt : debts) {
        ...
        // Higher stability improvement for critical debts and those close to due date
