...
TemporalVisitor::TemporalVisitor(Interpreter &interpreter)
...
bool TemporalVisitor::isPreventModification() const {
  return m_preventModification_flag;
}

bool TemporalVisitor::isReadOnlyMode() const { return m_readOnlyMode; }

double TemporalVisitor::getStabilizationFactor() const {
  return m_stabilizationFactor;
}
...
void TemporalVisitor::setContinueWeave(bool flag) {
  m_continueWeave_flag = flag;
}
...
void TemporalVisitor::setStabilizationFactor(double factor) {
  m_stabilizationFactor = factor;
}

void TemporalVisitor::setFlowDirection(int direction) {
...
void TemporalVisitor::visitContinueWeaveStmt(
    const ContinueWeaveStmtNode &stmt) {
  // Consume a small amount of resources
  m_interpreter.getRuntime()->consumeChronons(1.0);
...
void TemporalVisitor::visitStabilizeTimelineStmt(
  ...
  if (stmt.hasIntensity()) {
    ...
    if (intensityValue.isNumber()) {
      ...
      throw RuntimeError("Stabilization intensity must be a number.",
                         stmt.getLocation());
    }
...
void TemporalVisitor::visitPreventModificationStmt(
    const PreventModificationStmtNode &stmt) {
  // Calculate and consume resources
  double cost =
      calculateTemporalResourceCost(TemporalOperation::PREVENT_MODIFICATION);
  consumeResources(cost);
...
void TemporalVisitor::visit(const TemporalLoopNode &node) {
  ...
  for (int i = 0; i < iterations; ++i) {
    ...
    try {
      ...
      std::cerr << "Error in temporal loop iteration " << i << ": " << e.what()
                << std::endl;
      throw;
    }
    ...
    // Sleep to simulate the duration (in a real implementation, this would be
    // more sophisticated)
...
void TemporalVisitor::visit(const BranchTimelineNode &node) {
  ...
  if (!runtime) {
    throw RuntimeError(
        "No temporal runtime available for branch timeline operation");
  }
